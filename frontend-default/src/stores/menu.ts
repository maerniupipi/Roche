...
export const useMenuStore = defineStore('menuStore', () => {
  ...
  const visibleMenuArr = computed(() => {
    ...
    return menuArr.filter(item => {
      ...
      // Viewers consume enterprise knowledge through chat. Knowledge-base
      // and agent pages are department-administrator management surfaces.
      if (
      ...
      // KnowledgeDomain/runtime settings are an administrator responsibility.
      if (item.path === 'settings' && !authStore.canManageKnowledge) {
        return false
      }
      return true
    })
