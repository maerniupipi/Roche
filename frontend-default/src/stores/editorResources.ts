...
import { getKnowledgeDomainRetrievalConfig } from '@/api/retrieval'
import { loadOptionalFeatureList } from '@/utils/optionalFeature'
...
type EditorResourceKey =
  ...
  | 'promptTemplates'
  | 'placeholders'
  | 'knowledgeDomainRetrievalConfig'
  | 'parserEngines'
  | 'systemInfo'
...
export const useEditorResourcesStore = defineStore('editorResources', () => {
  ...
  const knowledgeDomainRetrievalConfig = ref<Record<string, unknown> | null>(null)
  ...
  async function ensureMcpServices(force = false): Promise<void> {
    return runOnce('mcpServices', force, async () => {
      mcpServices.value = await loadOptionalFeatureList(listMCPServices)
  ...
  async function ensureKnowledgeDomainRetrievalConfig(force = false): Promise<void> {
    return runOnce('knowledgeDomainRetrievalConfig', force, async () => {
      const retrievalRes: any = await getKnowledgeDomainRetrievalConfig()
      knowledgeDomainRetrievalConfig.value = retrievalRes?.data ?? null
      loadedAt.value.knowledgeDomainRetrievalConfig = Date.now()
    })
  }
  ...
  async function prefetchAgentEditorDeps(force = false): Promise<void> {
    await Promise.all([
      ...
      ensureKnowledgeDomainRetrievalConfig(force),
    ])
  }
  ...
  function invalidate(...keys: EditorResourceKey[]) {
    if (keys.length === 0) {
      ...
      knowledgeDomainRetrievalConfig.value = null
      parserEngines.value = []
  ...
  return {
    ...
    ensurePlaceholders,
    ensureKnowledgeDomainRetrievalConfig,
    ensureParserEngines,
    ensureSystemInfo,
