 ...
 *
 * Storage layout: RocheKAP_${userId}_${suffix}, where userId is the active
 ...
 *   - Legacy un-namespaced keys (RocheKAP_${suffix}) from earlier branch
...
export function readUserId(): string {
  try {
    const raw = localStorage.getItem('roche_kap_user')
    if (!raw) return 'anon'
...
export function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn(`[RocheKAP] failed to persist preference "${key}":`, err)
  }
}
...
export function userKey(suffix: string): string {
  return `RocheKAP_${readUserId()}_${suffix}`
}
...
export function migratePreferencesIntoUser(): void {
  ...
  for (const suffix of PREFERENCE_SUFFIXES) {
    const target = `RocheKAP_${userId}_${suffix}`
    const targetExists = safeGetItem(target) !== null

    const anonKey = `RocheKAP_anon_${suffix}`
    const legacyKey = `RocheKAP_${suffix}`
