...
const canManageAgents = computed(() => authStore.canManageKnowledge);
...
const getMcpCapability = (agent: CustomAgent): string => {
  ...
  if (config.mcp_selection_mode === 'all') return t('agent.mcpScope.mcpAll');
  if (config.mcp_services?.length) {
    return t('agent.mcpScope.mcpSelected', { count: config.mcp_services.length });
  }
