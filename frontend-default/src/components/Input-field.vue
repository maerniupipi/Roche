...
const showWebSearchButton = computed(() => {
  if (!hasAgentConfig.value) {
    return isKnowledgeDomainWebSearchReady(webSearchProviders.value);
  }
...
const allSelectedItems = computed(() => {
  ...
  return [...knowledgeItems, ...selectedMCPItems.value, ...skillMentionItems.value];
});
...
const loadMentionItems = async (q: string, resetIndex = true, append = false) => {
  ...
  // 当前用户授权，因此 Agent 的 @ 菜单只展示 MCP 和 Skill。
...
const handleSelectAgent = async (agent: CustomAgent) => {
  ...
  // 同步智能体的配置状态：模型、网络搜索、知识库由 watch 同步
  // 1. 同步网络搜索状态
