  ...
  <div ref="menuRef" class="user-menu" :class="{ collapsed: uiStore.sidebarCollapsed }">
    ...
    <Transition name="dropdown">
      <div v-if="menuVisible" class="user-dropdown" @click.stop>
        ...
        <button v-if="canSee('admin')" type="button" class="menu-item" @click="reopenGuide">
          <t-icon name="help-circle" />
          <span>{{ $t('newUserGuide.reopen') }}</span>
        </button>
        <button v-if="authStore.isSystemAdmin" type="button" class="menu-item" @click="openSettings('models')">
          <t-icon name="control-platform" />
          <span>{{ $t('settings.modelManagement') }}</span>
        </button>
        <button v-if="authStore.isSystemAdmin" type="button" class="menu-item" @click="openSettings('websearch')">
          <t-icon name="internet" />
          <span>{{ $t('settings.webSearchConfig') }}</span>
        </button>
        <button v-if="canSee('admin')" type="button" class="menu-item" @click="openSettings()">
          <t-icon name="setting" />
          <span>{{ $t('general.allSettings') }}</span>
        </button>
        <button v-if="authStore.isSystemAdmin" type="button" class="menu-item" @click="openSystemSettings">
          <t-icon name="server" />
          <span>{{ $t('settings.system') }}</span>
        </button>

        <div class="divider" />
        <button v-if="!authStore.isLiteMode" type="button" class="menu-item danger" @click="handleLogout">
          <t-icon name="logout" />
...
function openSystemSettings(): void {
  ...
  void router.push({ path: '/platform/settings', query: { section: 'system-global' } })
}
