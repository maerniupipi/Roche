<template>
  <!--
    ...
    other settings panes. Avoid bespoke layout here; the modal already
    ...
    <div class="section-header">
      <div class="section-header-row">
        ...
        <!-- Platform audit-log entry. SystemAdmin already gated the
             ...
             knowledgeDomain settings (frontend/src/views/settings/KnowledgeDomainMembers.vue). -->
        <t-button
          variant="text"
          size="small"
        ...
        <div class="setting-control">
          ...
          >
            <!--
              ...
              (knowledge_domain.default_storage_quota_gb) carries one — clicking
              writes the current setting value onto every existing
              knowledgeDomain. We do this as a separate explicit action rather
              than auto-cascade on save so a SystemAdmin who tweaks the
              default while triaging a single new-knowledgeDomain question
...
import {
  ...
  updateSystemSetting,
  resetSystemSetting,
  applyDefaultStorageQuotaToAllKnowledgeDomains,
  listSystemAdmins,
...
function highRiskConfirmBody(item: SystemSettingItem, value: unknown): string {
  const renderedValue = Array.isArray(value)
    ? value.length === 0
      ...
      : value.join(', ')
    : String(value)
  return t('system.globalSettings.confirm.defaultBody', {
...
// Currently only `knowledge_domain.default_storage_quota_gb` does — saving the
// setting only affects future knowledgeDomains, so the bulk button is the
// escape hatch for "rewrite all current knowledgeDomains too".
function hasBulkAction(item: SystemSettingItem): boolean {
  return item.key === 'knowledge_domain.default_storage_quota_gb'
}
...
function bulkActionConfirmBody(item: SystemSettingItem): string {
  ...
  // that's currently in effect for new knowledgeDomains.
  const v = item.value
...
async function runBulkAction(item: SystemSettingItem) {
  ...
  try {
    const result = await applyDefaultStorageQuotaToAllKnowledgeDomains()
...
async function loadAdmins() {
  try {
    ...
    for (const u of resp.admins ?? []) {
      ...
      // as resolveMaxOwnedKnowledgeDomainsPerUser on the backend.
      if (!u.email) continue
      map[u.email] = u.id
...
// ---- Platform audit log (system-scope, knowledge_domain_id=0) ---------------------
//
// Wired against GET /api/v1/system/admin/audit-log (SystemAdmin only).
// The drawer mirrors the structural choices of the knowledgeDomain audit drawer
// in frontend/src/views/settings/KnowledgeDomainMembers.vue: cursor-paged by
...
function auditTargetKey(row: AuditLog): string {
  ...
  if (row.action === 'system.setting_changed') {
    if (row.target_type === 'knowledge_domain_storage_quota') {
...
async function loadAuditLog(reset: boolean) {
  ...
  try {
    ...
    if (resp.success) {
      ...
      // Same convention as knowledgeDomain audit: next_cursor=0 means "no
...
/* Title + audit-log entry sit on the same row, parallel to the layout
   used in knowledgeDomain member settings — keeps secondary actions anchored to
...
/* ===== Audit drawer (mirrors KnowledgeDomainMembers.vue's audit panel) =========
...
/* t-drawer teleports its content-wrapper to body, so the height-chain
   ...
   pattern as `.knowledgeDomain-members-audit-drawer` in KnowledgeDomainMembers.vue. */
