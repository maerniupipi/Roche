                  ...
                  <div class="tools-overview">
                    <div class="tools-overview-row">
                      <div class="tools-status-chip">
                        <t-icon name="folder" />
                        <template v-if="!hasKnowledgeBase">
                          ...
                          <section v-for="group in groupedAvailableTools" :key="group.key"
                            ...
                            <div class="tool-grid">
                              <t-checkbox v-for="tool in group.tools" :key="tool.value" :value="tool.value"
                                ...
                                :class="['tool-card', { 'tool-card--disabled': tool.disabled }]">
...
const kbOptions = ref<{ label: string; value: string; type?: 'document' | 'faq'; count?: number; ragEnabled?: boolean; capabilities?: KBCapabilities }[]>([]);
...
const allTools = computed(() => [
  ...
  // Skill 专用工具
  { value: 'text_counter', label: t('agentEditor.tools.textCounter'), description: t('agentEditor.tools.textCounterDesc'), group: 'skill' },
]);
...
const toolGroups = computed(() => [
  ...
  { key: 'skill', label: t('agentEditor.tools.groupSkill') },
]);

// 当前用户是否至少能访问一个知识库。智能体本身不绑定知识范围。
const hasKnowledgeBase = computed(() => {
  return kbOptions.value.length > 0;
});
...
const scopeCapabilities = computed<ScopeCapabilities>(() => {
  const scope: ScopeCapabilities = { vector: false, keyword: false, graph: false, faq: false };
  for (const kb of kbsInScope.value) {
    ...
    if (caps) {
      ...
      // 向后兼容：capabilities 尚未加载时，退回到 ragEnabled 推断
...
// 知识库授权变化时保留用户选择的工具；运行时会对不可用工具做最终校验。
...
const loadDependencies = async () => {
  try {
    ...
    kbOptions.value = chatResources.rawKnowledgeBases.map((kb: any) => mapKbToOption(kb));
    ...
    const rc = editorResources.knowledgeDomainRetrievalConfig as Record<string, number> | null;
...
.tool-group--skill .tool-group-bar {
