...
import {
  ...
  isKnowledgeDomainWebSearchReady,
...
test('resolveAgentWebSearchProviderId falls back to knowledgeDomain default', () => {
  assert.equal(
...
test('isKnowledgeDomainWebSearchReady checks default provider only', () => {
  assert.equal(isKnowledgeDomainWebSearchReady(providers), true);
  assert.equal(
    isKnowledgeDomainWebSearchReady([{ id: 'p1', name: 'Keenable', is_default: false }]),
    false,
  );
});
