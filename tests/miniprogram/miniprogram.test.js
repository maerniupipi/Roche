...
test("API helpers send RocheKAP auth headers", async () => {
  let capturedRequest;
  global.wx = {
    getStorageSync() {
      return {
  ...
  assert.equal(capturedRequest.url, "https://rochekap.example.com/api/v1/knowledge-bases");
...
test("URL import helper posts the selected URL payload", async () => {
  ...
  await createKnowledgeFromURL("kb-1", "https://knowledge.example.com", true);

  assert.equal(capturedRequest.method, "POST");
  assert.equal(capturedRequest.url, "https://rochekap.example.com/api/v1/knowledge-bases/kb-1/knowledge/url");
...
test("chat helper includes selected knowledge base ids", async () => {
  ...
  assert.equal(capturedRequest.method, "POST");
  assert.equal(capturedRequest.url, "https://rochekap.example.com/api/v1/knowledge-chat/session-1");
  assert.deepEqual(capturedRequest.data, {
    query: "hello",
