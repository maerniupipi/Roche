...
function defaultAuthenticatedPath(_authStore: ReturnType<typeof useAuthStore>) {
  ...
  routes: [
    ...
    },
    {
      ...
      children: [
        ...
        {
          path: "system",
          redirect: { path: "/platform/settings", query: { section: "enterprise-org" } },
...
router.beforeEach(async (to, from, next) => {
  ...
  // Settings remains the only management surface accessible to
  // canManageKnowledge users; everyone else is bounced to the default
  // landing path. There is no separate admin view to gate.
