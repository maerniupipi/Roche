...
const effectiveKBPermission = computed<string>(() => {
  return props.kbInfo?.my_permission || ''
})

const accessRoleLabel = computed<string>(() => {
  const perm = effectiveKBPermission.value
  if (perm === 'manage') return t('knowledgeBase.accessInfo.roleManager')
  if (perm === 'read') return t('knowledgeBase.accessInfo.roleReader')
  return '--'
})

const accessPermissionSummary = computed<string>(() => {
  const perm = effectiveKBPermission.value
  if (perm === 'manage') return t('knowledgeBase.accessInfo.permissionManage')
  if (perm === 'read') return t('knowledgeBase.accessInfo.permissionRead')
  return '--'
})
...
.kb-info-card {
  ...
  color: var(--td-text-color-primary);
  overflow: hidden;
  /* Mobile (≤768px): shift the popup slightly right so the bottom-centered
     card sits visually anchored to its trigger button (the info icon lives
     on the right side of the KB header bar). Without this offset the
     placement="bottom" rule centers the card under the entire header and
     leaves a visible gap between trigger and content. */
