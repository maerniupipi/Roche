...
// TTooltip 保留 desktop 库（tdesign-mobile-vue 1.16.0 未导出 Tooltip）
...
// <script setup> 中已通过 import 注册 desktop Tooltip（mobile 库未导出）
...
const getKbCapability = (agent: CustomAgent): string => {
  ...
  if (config.agent_mode === 'quick-answer') return t('agent.features.knowledgeBase');
  ...
  return allowedTools.length === 0 || allowedTools.some(tool => knowledgeTools.has(tool))
    ? t('agent.features.knowledgeBase')
    : '';
};
...
const updateDetailPanelPosition = () => {
  ...
  // F3 已删除 zoom 适配，getBoundingClientRect 直接返回 CSS 像素
...
@media (max-width: 768px) {
  ...
  .agent-selector-dropdown {
    /* 覆盖 JS 算出的 top/left/bottom/width，变为底部 sheet。 */
    top: auto !important;
    bottom: 0 !important;
    left: 0 !important;
    right: 0 !important;
    ...
    animation: agentSelectorSlideUp 0.2s ease-out;
  }

  @keyframes agentSelectorSlideUp {
    from {
  ...
  /* 移动端关闭 hover-only detail panel：touch 设备没有 hover，取而代之的是直接点击选中。 */
  .agent-detail-panel {
