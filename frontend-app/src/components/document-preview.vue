...
import { MessagePlugin, Watermark } from 'tdesign-mobile-vue';
...
// ── 水印：使用当前用户标识 + 时间戳，在预览区域平铺展示 ──
const watermarkUser = computed(() => {
  const u = authStore.user as { username?: string; email?: string } | null;
...
const watermarkLine = computed(() => {
  return `${t('preview.watermarkPrefix')} ${watermarkUser.value} · ${watermarkTimestamp.value} ${t('preview.watermarkSuffix')}`;
});

// 仅在已加载到非空内容时显示水印；加载/错误/不支持类型不展示
const showWatermark = computed(() => {
  if (loading.value || error.value || previewType.value === 'unsupported') return false;
  return previewType.value !== 'audio'; // 音频不显示文字水印
});

const watermarkContent = computed(() => {
  if (!showWatermark.value) return [];
  return [
    { text: `${t('preview.watermarkPrefix')} ${watermarkUser.value} ${t('preview.watermarkSuffix')}`, fontSize: 13, },
    { text: watermarkTimestamp.value, fontSize: 13, },
  ];
});

// ── 防复制 / 防下载：阻止默认行为并提示用户 ──
function notifyCopyDisabled() {
  MessagePlugin.warning({
    content: t('preview.copyDisabled'),
    duration: 2000,
    closeBtn: false,
  });
}

function notifySaveDisabled() {
  MessagePlugin.warning({
    content: t('preview.saveDisabled'),
    duration: 2000,
    closeBtn: false,
  });
}
  ...
  try {
    ...
    switch (previewType.value) {
      case 'pdf': {
        ...
        // rawBlobUrl 记录原始 URL 供 revoke 用；blobUrl 带 fragment 隐藏 PDF 阅读器工具栏
        rawBlobUrl.value = raw;
        // #toolbar=0 隐藏 PDF 阅读器自带的工具栏/下载按钮；#navpanes=0 隐藏导航栏
        blobUrl.value = `${raw}#toolbar=0&navpanes=0`;
        break;
      }
      case 'image': {
...
function cleanup() {
  if (rawBlobUrl.value) {
    URL.revokeObjectURL(rawBlobUrl.value);
    rawBlobUrl.value = '';
  }
  if (blobUrl.value) {
    blobUrl.value = '';
...
<template>
  <div class="document-preview" :class="{ 'is-fullscreen': isFullscreen, 'preview-locked': showWatermark }"
    @contextmenu="onContextMenu" @copy="onCopy" @cut="onCut" @dragstart="onDragStart">
    <!-- Toolbar：位于水印之上，避免被覆盖 -->
    ...
    <Watermark :watermark-content="watermarkContent" :alpha="1" :rotate="-22" :width="280" :height="100" :x="0" :y="120"
      :line-space="22" :movable="false" :removable="true" :z-index="5" class="preview-watermark-host">
      <!-- Loading -->
      ...
      <div v-else-if="previewType === 'image' && blobUrl" class="preview-image">
        <div class="image-wrapper">
          <img :src="blobUrl" :alt="fileName" draggable="false" @load="onImageLoad" @contextmenu.stop="onContextMenu" />
      ...
      <div v-else-if="previewType === 'pptx' && pptxData" class="preview-pptx">
        <vue-office-pptx :src="pptxData" @rendered="() => { }"
          @error="(e: any) => { error = e?.message || $t('preview.loadFailed'); }" />
      </div>
      ...
      <div v-else-if="previewType === 'audio' && blobUrl" class="preview-audio">
        <div class="audio-wrapper">
          ...
          <audio controls :src="blobUrl" class="audio-element" controlsList="nodownload">
...
// <t-watermark> 包裹容器：负责承载水印 canvas 与默认 slot 的预览内容
.preview-watermark-host {
  position: relative;
  display: block;
  // t-watermark 内部设置了 width: 100%; overflow: hidden —— 与 preview 内置滚动容器配合使用
  min-height: 200px;
}
...
.preview-toolbar {
  ...
  top: 8px;
  right: 24px;
  // z-index 高于 <t-watermark> 的 canvas (z-index: 5)，保证工具栏不被水印遮挡
  z-index: 20;
  background: var(--td-bg-color-container);
...
.preview-image {
  ...
  .image-wrapper {
    ...
    img {
      ...
      border-radius: @border-radius;
      box-shadow: 0 2px 12px rgba(11, 65, 205, 0.08);
      object-fit: contain;
    }
