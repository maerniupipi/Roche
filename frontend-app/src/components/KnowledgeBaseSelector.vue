...
const updateDropdownPosition = () => {
  ...
  // F3 已删除 zoom 适配，window.innerWidth/innerHeight 直接可用
  const vwFallback = window.innerWidth
  const vhFallback = window.innerHeight
  ...
  // F3 已删除 zoom 适配，rawRect 已经是 CSS 像素
  const rect = rawRect
...
@media (max-width: 768px) {
  ...
  .kb-dropdown {
    /* 覆盖 JS 算出的 top/left/bottom/width → 变底部 sheet。 */
    top: auto !important;
    bottom: 0 !important;
    left: 0 !important;
    right: 0 !important;
    ...
    animation: kbSelectorSlideUp 0.2s ease-out;
  }

  @keyframes kbSelectorSlideUp {
    from {
