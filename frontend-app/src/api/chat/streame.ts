...
export function useStream() {
  ...
  const startStream = async (params: { session_id: any; query: any; knowledge_base_ids?: string[]; knowledge_ids?: string[]; tag_ids?: string[]; agent_enabled?: boolean; agent_id?: string; enable_memory?: boolean; summary_model_id?: string; skill_names?: string[]; mentioned_items?: Array<{id: string; name: string; type: string; kb_type?: string; kb_id?: string; kb_name?: string; service_id?: string; skill_name?: string}>; images?: Array<{data: string}>; attachment_uploads?: Array<{data: string; file_name: string; file_size: number}>; method: string; url: string }) => {
    ...
    // 知识域作用域请求头：只要 setSelectedKnowledgeDomain 写过当前知识域，就附
    // X-KnowledgeDomain-ID。早期版本会 short-circuit "selectedKnowledgeDomainId ===
    // defaultKnowledgeDomainId 时不附" 来减少 header 体积，但任何把 roche_kap_knowledgeDomain
    // 写成当前知识域的代码（UserMenu loadUserInfo / router
    // hydrate）都会让两者相等，使得后续流式请求悄悄丢 header、落到
    // 默认知识域上，导致 SSE 接口返回 404。直接附即可。
    ...
    try {
      ...
      const agentEnabled = params.agent_enabled !== undefined ? params.agent_enabled : true;

      // Prepare the request body. Ordinary RAG keeps the user's selected
      // knowledge scope. Agent mode intentionally omits it because every agent
      // derives its knowledge scope exclusively from the current user's grants.
      // Persist the input-bar selection state for reopening the session.
      const postBody: any = { 
        query: params.query,
        agent_enabled: agentEnabled
      };
      if (!agentEnabled && params.knowledge_base_ids !== undefined && params.knowledge_base_ids.length > 0) {
        postBody.knowledge_base_ids = params.knowledge_base_ids;
      }
      if (!agentEnabled && params.knowledge_ids !== undefined && params.knowledge_ids.length > 0) {
      ...
      if (!agentEnabled && params.tag_ids !== undefined && params.tag_ids.length > 0) {
