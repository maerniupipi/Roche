...
const protectedFileCacheState = (() => {
  ...
  const scope = window as typeof window & {
    __rochekapProtectedFileCacheV1__?: ProtectedFileCacheState;
  };
  scope.__rochekapProtectedFileCacheV1__ ||= fresh();
  return scope.__rochekapProtectedFileCacheV1__;
})();
...
function getProtectedFileRequestHeaders(): Record<string, string> {
  ...
  try {
    const token = (localStorage.getItem('roche_kap_token') || '').trim();
    if (token) {
