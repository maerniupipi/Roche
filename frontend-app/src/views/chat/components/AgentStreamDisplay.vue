<template>
  <div ref="rootElement" class="agent-stream-display" :class="{ 'is-rag-mode': ragMode }">
    ...
    <div v-if="!ragMode || displayEvents.length > 0 || showAgentActivityIndicator" ref="streamingStepsContainer"
      ...
      :class="{ 'streaming-steps-constrained': !answerEverStarted && !isConversationDone, 'is-streaming-timeline': showStreamingTimeline }">
      <template v-for="(event, index) in displayEvents" :key="getEventKey(event, index)">
        <div v-if="event && event.type" class="event-item"
          :class="{ 'event-answer': event.type === 'answer', 'tree-child': isStreamingTimelineEvent(event), 'tree-child-last': isStreamingTimelineEvent(event) && !showAgentActivityIndicator && index === lastStreamingTimelineEventIndex }">
          ...
          <div :class="{ 'tree-child-content': isStreamingTimelineEvent(event) }">
            ...
            <div v-else-if="event.type === 'answer' && (event.done || (event.content && event.content.trim()))"
              ...
              <div v-if="event.done && event.content && event.content.trim()" class="answer-toolbar">
                <div size="small" variant="outline" shape="round" :disabled="!props.userQuery"
                  @click.stop="handleRegenerate" :title="$t('chat.regenerate')">
                  <t-icon name="refresh" />
                </div>
                <!-- 点赞 / 点踩：先在前端落库状态；接口占位见 src/api/chat/feedback.ts -->
                <div size="small" variant="outline" shape="round" class="feedback-btn feedback-btn--like"
                  :class="{ 'is-active': currentFeedback === 'like', 'is-pulsing': pulseRating === 'like' }"
                  :disabled="feedbackSubmitting"
                  :title="currentFeedback === 'like' ? $t('agentStream.feedback.alreadyLiked') : $t('agentStream.feedback.likeTip')"
                  :aria-label="$t('agentStream.feedback.like')" @click.stop="handleLike(event)">
                  <t-icon name="thumb-up" />
                </div>
                <div size="small" variant="outline" shape="round" class="feedback-btn feedback-btn--dislike"
                  :class="{ 'is-active': currentFeedback === 'dislike', 'is-pulsing': pulseRating === 'dislike' }"
                  :disabled="feedbackSubmitting"
                  :title="currentFeedback === 'dislike' ? $t('agentStream.feedback.alreadyDisliked') : $t('agentStream.feedback.dislikeTip')"
                  :aria-label="$t('agentStream.feedback.dislike')" @click.stop="openDislikeDialog(event)">
                  <t-icon name="thumb-down" />
                </div>
                <div size="small" variant="outline" shape="round" :loading="copyingAnswer"
            ...
            <div v-else-if="event.type === 'tool_call'" class="tool-event">
              <div class="action-card"
                :class="{ 'action-pending': event.pending, 'action-error': event.success === false, 'reference-trigger': canOpenToolReferences(event) }"
                :role="canOpenToolReferences(event) ? 'button' : undefined"
                :tabindex="canOpenToolReferences(event) ? 0 : undefined" @click="handleActionCardClick(event)"
                @keydown.enter="handleActionCardClick(event)" @keydown.space.prevent="handleActionCardClick(event)">
                <div class="action-header" @click.stop="handleActionHeaderClick(event)"
  ...
  <DislikeFeedbackDialog v-model:visible="dislikeDialogVisible" :message-id="dislikeDialogMessageId"
...
/** 点赞：同一时刻最多记一次；与点踩可双向切换（接口成功后样式才更新）。 */
const handleLike = async (answerEvent: any) => {
  if (feedbackSubmitting.value) return
  // 已点赞再点：视为 no-op，避免重复提交
  if (currentFeedback.value === 'like') return

  feedbackSubmitting.value = true
  try {
    const result = await submitMessageFeedback({
      message_id: resolveMessageIdForFeedback(answerEvent) ?? '',
      rating: 'like',
    })
    // ★ 关键：仅在接口成功后才落样式 & 触发动效
    if (!result.success) {
...
/** 点踩：打开弹窗收集原因；用户点确认后由弹窗内部 submit。
 *  允许「已点赞」后再次点踩——切换动作走 dislikeDialog，最终也由
 *  handleFeedbackSubmitted 决定是否落样式。 */
const openDislikeDialog = (answerEvent: any) => {
  if (feedbackSubmitting.value) return
  if (currentFeedback.value === 'dislike') return
  dislikeDialogMessageId.value = resolveMessageIdForFeedback(answerEvent)
  dislikeDialogVisible.value = true
}

/** 弹窗提交成功后的统一收尾。
 *  ★ 仅当 result.success 时才更新 currentFeedback（驱动样式改变）
 *    并触发 pop 动效；失败时弹窗保留、样式不变。 */
const handleFeedbackSubmitted = (result: FeedbackSubmitResult) => {
  if (!result.success) return
  // 当前弹窗仅承载 dislike；如未来扩展 like 走弹窗，需要在这里区分 rating
