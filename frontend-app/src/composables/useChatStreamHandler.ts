...
export function useChatStreamHandler(options: UseChatStreamHandlerOptions) {
  ...
  const restoreQuickAnswerFlags = (item: ChatMessage) => {
    ...
    // agent_steps is set by the backend only for Agent-pipeline messages.
    // Honour it as the source of truth — do NOT flip the message to RAG mode
    // just because the user is currently in quick-answer mode, otherwise
    // botmsg would route an Agent-generated history through the RAG branch
    // and pass :rag-mode="true" into AgentStreamDisplay, which hides the
    // intermediate-steps tree (shouldShowCollapsedSteps short-circuits on
    // props.ragMode).
    const wasAgentMessage =
      Array.isArray(item.agent_steps) && item.agent_steps.length > 0
