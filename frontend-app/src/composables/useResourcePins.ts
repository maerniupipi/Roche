// useResourcePins – per-(user, knowledgeDomain) favorites + per-user recents for
// KBs and custom agents.
//
// Favorites are DB-backed (see migration 000047) so they sync across
// devices and survive a logout. They are scoped per (user, knowledgeDomain) on
// the server, which means switching knowledgeDomains automatically re-points the
// reads — see `refresh()` and the knowledgeDomain-change watcher inside the
// composable.
//
...
// Recents are scoped by (user, knowledgeDomain) — same rationale as favorites. The
// hydration step in the list views joins recent entries against the
// current knowledgeDomain's KB/Agent index, so a recent from knowledgeDomain A would
// silently drop out of the rendered list but still count in the sidebar
// badge ("最近 (2)" but empty body). Keying the storage by knowledgeDomain
// prevents that mismatch entirely.
//
// `knowledgeDomainSegmentForKey` returns "" when no knowledgeDomain is resolvable (e.g.
...
// recents. Once the knowledgeDomain id resolves, the knowledgeDomain watcher below bumps
// the revision so views pick up the right namespace.
function recentsKey(): string {
  return `RocheKAP_${readUserId()}_${RECENTS_SUFFIX}`
}
...
// Invalidate caches when the active knowledgeDomain changes — favorites are
// (user, knowledgeDomain)-scoped on the server, so a knowledgeDomain switch means the
// currently loaded list belongs to the *previous* knowledgeDomain. We trigger one
