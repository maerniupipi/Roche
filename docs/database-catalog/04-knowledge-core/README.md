...
目录不是独立物理知识库，也不创建单独 Milvus Collection。它可以作为
`knowledge_resource_grants.resource_type=folder` 的授权对象。删除目录时，Service
递归删除其子目录、文档及派生数据，并清理相关 ACL。
...
权限不会在 Milvus 中逐条保存。检索前先根据 `knowledge_resource_grants` 计算用户的
有效文档集合：

```text
整库可读 -> 只按 knowledge_base_id 过滤
目录/文档部分可读 -> 按 knowledge_base_id + knowledge_id IN (...) 过滤
目录/文档 deny -> 从可读 knowledge_id 集合中移除对应文档
```
