...
type mockChat struct {
	...
	callCount    int
	toolsByCall  [][]string
	onChatStream func(callIndex int)
}
...
func (m *mockChat) ChatStream(_ context.Context, _ []chat.Message, opts *chat.ChatOptions) (<-chan types.StreamResponse, error) {
	...
	m.toolsByCall = append(m.toolsByCall, toolNames)
	if m.onChatStream != nil {
		m.onChatStream(m.callCount)
	}
...
func (t *engineTestTool) Description() string         { return t.name }
func (t *engineTestTool) Parameters() json.RawMessage { return json.RawMessage(`{"type":"object"}`) }
func (t *engineTestTool) Execute(context.Context, json.RawMessage) (*types.ToolResult, error) {
	return &types.ToolResult{Success: true, Output: "ok"}, nil
}

func TestAgentEngine_RefreshesToolsAfterSkillActivation(t *testing.T) {
	registry := agenttools.NewToolRegistry()
	registry.RegisterTool(&engineTestTool{name: "base_tool"})
	require.NoError(t, registry.RegisterDormantTool(&engineTestTool{name: "dynamic_tool"}))
	require.NoError(t, registry.BindSkillTools("dynamic-skill", []string{"dynamic_tool"}))
	...
	mock.onChatStream = func(callIndex int) {
		if callIndex == 0 {
			_, err := registry.ActivateSkillTools("dynamic-skill")
	...
	require.NoError(t, err)
	require.Len(t, mock.toolsByCall, 2)
	assert.Equal(t, []string{"base_tool"}, mock.toolsByCall[0])
	assert.Equal(t, []string{"base_tool", "dynamic_tool"}, mock.toolsByCall[1])
}
...
func TestExecuteLoop_EmptyContentWithStop_ShouldNotCompleteWithEmpty(t *testing.T) {
	...
	_, err := engine.executeLoop(ctx, state, "test query", emptyMessages(), "sess-1", "msg-1")
