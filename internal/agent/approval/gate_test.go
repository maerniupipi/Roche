...
func (s *stubChecker) IsRequired(ctx context.Context, knowledgeDomainID uint64, serviceID, toolName string) (bool, error) {
...
func TestGate_Resolve_KnowledgeDomainMismatch(t *testing.T) {
	bus := event.NewEventBus()
	g := NewGate(&config.Config{Agent: &config.AgentConfig{ToolApprovalTimeoutSeconds: 2}}, &stubChecker{required: true}, nil)
	req := PendingRequest{
		KnowledgeDomainID: 1, EventBus: bus, SessionID: "s1", AssistantMessageID: "m1",
		ServiceID: "svc", MCPToolName: "t", Args: json.RawMessage(`{}`),
	}
	bus.On(event.EventToolApprovalRequired, func(_ context.Context, evt event.Event) error {
		data := evt.Data.(event.ToolApprovalRequiredData)
		go func() {
			require.ErrorIs(t, g.Resolve(999, "", data.PendingID, Decision{Approved: true}), ErrKnowledgeDomainMismatch)
...
func TestGate_Resolve_UserMismatch(t *testing.T) {
	...
	g := NewGate(&config.Config{Agent: &config.AgentConfig{ToolApprovalTimeoutSeconds: 2}}, &stubChecker{required: true}, nil)
	req := PendingRequest{
		KnowledgeDomainID: 1, UserID: "alice", EventBus: bus,
...
// per-user check entirely (allowing same-knowledgeDomain cross-user approval).
...
func TestGate_Resolve_AlreadyResolvedAfterTimeout(t *testing.T) {
	...
	d, err := g.RequestAndWait(context.Background(), PendingRequest{
		KnowledgeDomainID: 1, EventBus: bus, SessionID: "s",
		ServiceID: "svc", MCPToolName: "t", Args: json.RawMessage(`{}`),
	})
	require.NoError(t, err)
	require.True(t, d.TimedOut)
