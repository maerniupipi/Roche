...
// channel is suffixed with ROCHE_KAP_REDIS_NAMESPACE (when set) so multiple
// deployments sharing the same Redis don't cross-talk.
const pubsubChannelBase = "rochekap:mcp_approval:resolve"
...
type resolveAck struct {
	PendingID    string `json:"pending_id"`
	Status       string `json:"status"` // ok | not_found | knowledge_domain_mismatch | user_mismatch | already_resolved
	OriginID     string `json:"origin_id,omitempty"`
	RequestNonce string `json:"request_nonce,omitempty"`
}
...
func pubsubChannel() string {
	if ns := strings.TrimSpace(os.Getenv("ROCHE_KAP_REDIS_NAMESPACE")); ns != "" {
...
type MCPApproval interface {
	NeedsApproval(ctx context.Context, knowledgeDomainID uint64, serviceID, toolName string) bool
...
var (
	...
	// ErrKnowledgeDomainMismatch is returned when Resolve knowledgeDomain does not match the pending request.
	ErrKnowledgeDomainMismatch = errors.New("knowledgeDomain mismatch for tool approval")
...
func NewGate(cfg *config.Config, checker Checker, rdb *redis.Client) *Gate {
	...
	// HITL feature). Set ROCHE_KAP_AGENT_TOOL_APPROVAL_FAIL_OPEN=true to revert.
	failClose := !strings.EqualFold(strings.TrimSpace(os.Getenv("ROCHE_KAP_AGENT_TOOL_APPROVAL_FAIL_OPEN")), "true")
	g := &Gate{
...
func (g *Gate) runSubscriber() {
	...
	for {
		...
		for msg := range ch {
			...
			err := g.deliverLocal(m.KnowledgeDomainID, m.UserID, m.PendingID, Decision{
				Approved:        m.Approved,
			...
			if m.ReplyChannel != "" {
				...
				case err == nil:
					status = "ok"
				case errors.Is(err, ErrKnowledgeDomainMismatch):
					status = "knowledge_domain_mismatch"
...
func (g *Gate) NeedsApproval(ctx context.Context, knowledgeDomainID uint64, serviceID, toolName string) bool {
	if g == nil || g.checker == nil || knowledgeDomainID == 0 || serviceID == "" || toolName == "" {
		return false
	}
	ok, err := g.checker.IsRequired(ctx, knowledgeDomainID, serviceID, toolName)
	if err != nil {
		...
		// ROCHE_KAP_AGENT_TOOL_APPROVAL_FAIL_OPEN=true.
		if g.failClose {
...
func (g *Gate) RequestAndWait(ctx context.Context, req PendingRequest) (Decision, error) {
	...
	w := &waiter{
		...
		knowledgeDomainID: req.KnowledgeDomainID,
		userID:            req.UserID,
	}
...
// Resolve completes a pending approval. knowledgeDomainID must match the knowledgeDomain that
...
func (g *Gate) Resolve(knowledgeDomainID uint64, userID, pendingID string, d Decision) error {
	if g == nil {
		return fmt.Errorf("gate is nil")
	}
	switch err := g.deliverLocal(knowledgeDomainID, userID, pendingID, d); {
	case err == nil:
		return nil
	case errors.Is(err, ErrKnowledgeDomainMismatch),
	...
	case errors.Is(err, ErrPendingNotFound):
		...
		return g.resolveCrossInstance(knowledgeDomainID, userID, pendingID, d)
	default:
		return err
	}
}
...
func (g *Gate) resolveCrossInstance(knowledgeDomainID uint64, userID, pendingID string, d Decision) error {
	...
	for {
		...
		case "ok":
			return nil
		case "knowledge_domain_mismatch":
			return ErrKnowledgeDomainMismatch
		case "user_mismatch":
...
func (g *Gate) deliverLocal(knowledgeDomainID uint64, userID, pendingID string, d Decision) error {
	g.mu.Lock()
	...
	if w.knowledgeDomainID != knowledgeDomainID {
		g.mu.Unlock()
		return ErrKnowledgeDomainMismatch
	}
...
func (a *Adapter) IsRequired(ctx context.Context, knowledgeDomainID uint64, serviceID, toolName string) (bool, error) {
	if a == nil || a.Svc == nil {
		return false, nil
	}
	return a.Svc.IsRequired(ctx, knowledgeDomainID, serviceID, toolName)
}
