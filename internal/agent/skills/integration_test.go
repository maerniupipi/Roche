...
func TestPreloadedTextCounterSkillIntegration(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("Failed to get current file path")
	}
	...
	skill, err := manager.LoadSkill(ctx, "文本统计器")
	if err != nil {
		t.Fatalf("Failed to load text counter skill: %v", err)
	}
	if skill.Instructions == "" {
		t.Fatal("Expected text counter instructions to be non-empty")
	}

	bindings, err := manager.GetAllSkillToolBindings(ctx)
	if err != nil {
		t.Fatalf("Failed to load tool bindings: %v", err)
	}
	toolNames := bindings["文本统计器"]
	if len(toolNames) != 1 || toolNames[0] != "text_counter" {
		t.Fatalf("Expected text_counter binding, got %v", toolNames)
	}
}
