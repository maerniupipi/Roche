...
// toolNamePattern matches the portable function name subset accepted by the
// chat providers used by RocheKAP.
...
func (s *Skill) Validate() error {
	...
	for _, toolName := range s.Tools {
		if toolName == "" {
			return fmt.Errorf("skill %s tool name cannot be empty", s.Name)
		}
		if len(toolName) > MaxToolNameLength || !toolNamePattern.MatchString(toolName) {
			return fmt.Errorf("skill %s has invalid tool name %q", s.Name, toolName)
		}
		if _, exists := seenTools[toolName]; exists {
			return fmt.Errorf("skill %s has duplicate tool name %q", s.Name, toolName)
		}
