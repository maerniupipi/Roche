...
func NewDataSchemaTool(knowledgeService interfaces.KnowledgeService, chunkRepo interfaces.ChunkRepository, targetChunkTypes ...types.ChunkType) *DataSchemaTool {
	return newDataSchemaTool(knowledgeService, chunkRepo, nil, false, targetChunkTypes...)
}

func NewScopedDataSchemaTool(
	knowledgeService interfaces.KnowledgeService,
	chunkRepo interfaces.ChunkRepository,
	searchTargets types.SearchTargets,
	targetChunkTypes ...types.ChunkType,
) *DataSchemaTool {
	return newDataSchemaTool(knowledgeService, chunkRepo, searchTargets, true, targetChunkTypes...)
}
...
func (t *DataSchemaTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	// Resolve the document to obtain its knowledgeDomain and knowledge-base IDs.
	...
	if t.enforceScope {
		...
		if scopeErr != nil {
			return &types.ToolResult{
				Success: false,
				Error:   fmt.Sprintf("Failed to validate knowledge scope: %v", scopeErr),
			}, scopeErr
		}
		if !allowed {
			err := fmt.Errorf("knowledge %s is not accessible in the current session", knowledge.ID)
			return &types.ToolResult{
				Success: false,
				Error:   err.Error(),
			}, err
