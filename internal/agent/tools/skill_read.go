...
func NewReadSkillTool(skillManager *skills.Manager, toolRegistry *ToolRegistry) *ReadSkillTool {
	...
	if input.FilePath != "" {
		...
		if len(skill.Tools) > 0 {
			if t.toolRegistry == nil {
				return &types.ToolResult{
					Success: false,
					Error:   "Skill tool activation is not available",
				}, nil
			}
			if err := t.toolRegistry.BindSkillTools(skill.Name, skill.Tools); err != nil {
				return &types.ToolResult{
					Success: false,
					Error:   fmt.Sprintf("Failed to bind tools for skill %s: %v", skill.Name, err),
				}, nil
			}
			activatedTools, err = t.toolRegistry.ActivateSkillTools(skill.Name)
			if err != nil {
				return &types.ToolResult{
					Success: false,
					Error:   fmt.Sprintf("Failed to activate tools for skill %s: %v", skill.Name, err),
				}, nil
			}
		}
		...
		if len(activatedTools) > 0 {
			builder.WriteString("\n\n## Activated Tools\n\n")
			for _, toolName := range activatedTools {
			...
			for _, file := range files {
				if file != skills.SkillFileName { // Don't list SKILL.md again
					if skills.IsScript(file) {
						builder.WriteString(fmt.Sprintf("- `%s` (script source - read-only)\n", file))
					} else {
						builder.WriteString(fmt.Sprintf("- `%s`\n", file))
					}
				}
		...
		resultData["activated_tools"] = activatedTools
	}
