...
func TestGetFunctionDefinitions_JSONByteStable(t *testing.T) {
	r := NewToolRegistry()
	registerMany(t, r, []string{"search", "fetch", "code_run", "answer", "docs", "kb_query"})
...
func definitionNames(defs []types.FunctionDefinition) []string {
...
func TestToolRegistry_DormantToolRequiresActivation(t *testing.T) {
	r := NewToolRegistry()
	r.RegisterTool(&mockTool{name: "base", description: "base"})
	require.NoError(t, r.RegisterDormantTool(&mockTool{name: "hidden", description: "hidden"}))
	require.NoError(t, r.BindSkillTools("text-tools", []string{"hidden"}))

	assert.Equal(t, []string{"base"}, definitionNames(r.GetFunctionDefinitions()))
	assert.False(t, r.IsToolActive("hidden"))

	result, err := r.ExecuteTool(context.Background(), "hidden", json.RawMessage(`{}`))
	require.Error(t, err)
	require.NotNil(t, result)
	assert.Contains(t, result.Error, "not active")

	activated, err := r.ActivateSkillTools("text-tools")
	require.NoError(t, err)
	assert.Equal(t, []string{"hidden"}, activated)
	assert.True(t, r.IsToolActive("hidden"))
	assert.Equal(t, []string{"base", "hidden"}, definitionNames(r.GetFunctionDefinitions()))
}

func TestToolRegistry_BindingHidesPreviouslyActiveTool(t *testing.T) {
	r := NewToolRegistry()
	r.RegisterTool(&mockTool{name: "shared_tool", description: "shared"})
	require.NoError(t, r.BindSkillTools("analysis", []string{"shared_tool"}))

	assert.False(t, r.IsToolActive("shared_tool"))
	assert.Empty(t, r.GetFunctionDefinitions())
}

func TestToolRegistry_ActivationIsIdempotentAndSorted(t *testing.T) {
	r := NewToolRegistry()
	require.NoError(t, r.RegisterDormantTool(&mockTool{name: "zeta", description: "z"}))
	require.NoError(t, r.RegisterDormantTool(&mockTool{name: "alpha", description: "a"}))
	require.NoError(t, r.BindSkillTools("analysis", []string{"zeta", "alpha", "zeta"}))

	first, err := r.ActivateSkillTools("analysis")
	require.NoError(t, err)
	second, err := r.ActivateSkillTools("analysis")
	require.NoError(t, err)

	assert.Equal(t, []string{"alpha", "zeta"}, first)
	assert.Equal(t, first, second)
	assert.Equal(t, []string{"alpha", "zeta"}, definitionNames(r.GetFunctionDefinitions()))
}

func TestToolRegistry_ActivationFailsAtomicallyWhenToolIsUnavailable(t *testing.T) {
	r := NewToolRegistry()
	require.NoError(t, r.RegisterDormantTool(&mockTool{name: "available", description: "available"}))
	require.NoError(t, r.BindSkillTools("analysis", []string{"available", "missing"}))

	activated, err := r.ActivateSkillTools("analysis")
	require.Error(t, err)
	assert.Nil(t, activated)
	assert.Contains(t, err.Error(), "missing")
	assert.False(t, r.IsToolActive("available"), "activation must not partially expose tools")
}

func TestToolRegistry_DuplicateDormantRegistrationReturnsError(t *testing.T) {
	r := NewToolRegistry()
	require.NoError(t, r.RegisterDormantTool(&mockTool{name: "hidden", description: "original"}))
	err := r.RegisterDormantTool(&mockTool{name: "hidden", description: "impostor"})
	require.Error(t, err)
