...
func TestScopeClause_KnowledgeAndFullKBOredTogether(t *testing.T) {
	...
	if !strings.Contains(sql, "chunks.knowledge_base_id = ? AND chunks.knowledge_domain_id = ?") {
		t.Fatalf("expected kb-knowledgeDomain predicate, got %q", sql)
	}
	if !strings.Contains(sql, " OR ") {
		t.Fatalf("expected the two scopes to be OR'd, got %q", sql)
	}
	// args: knowledgeIDs slice + kbID + knowledgeDomainID
	if len(args) != 3 {
...
func TestScopeClause_TagScopeUsesExistsInsteadOfExpandedKnowledgeIDs(t *testing.T) {
	...
	if len(args) != 3 {
		t.Fatalf("expected kbID, knowledgeDomainID, tagIDs args, got %d: %v", len(args), args)
	}
}

func TestScopeClause_SkipsKBWithoutKnowledgeDomain(t *testing.T) {
	sql, args := scopeClause(
		[]string{"kb-1", "kb-2"},
		nil,
		nil,
		map[string]uint64{"kb-1": 7}, // kb-2 has no knowledgeDomain mapping
	)
	if strings.Contains(sql, "kb-2") {
		t.Fatalf("kb-2 has no knowledgeDomain and must be skipped, got %q", sql)
	}
	if len(args) != 2 {
