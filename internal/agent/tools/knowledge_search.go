...
// returned chunks in a compact, de-duplicated form
...
func (t *KnowledgeSearchTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	// We used to read knowledgeDomain.ConversationConfig here as the first source of
	// truth, but that field was removed when the chat pipeline moved to
	// CustomAgent — knowledgeDomain-level KV settings now live on the agent itself.
	var topK int
	...
	if t.chunkService != nil && len(deduplicatedResults) > 0 {
		byKnowledgeDomain := make(map[uint64][]*types.SearchResult)
		for _, r := range deduplicatedResults {
			tid := t.searchTargets.GetKnowledgeDomainIDForKB(r.KnowledgeBaseID)
			if tid == 0 {
				continue
			}
			byKnowledgeDomain[tid] = append(byKnowledgeDomain[tid], r.SearchResult)
		}
		for tid, batch := range byKnowledgeDomain {
...
// and knowledge domain are grouped so the query embedding is computed once per
// (model, knowledge domain, query) tuple, and all full-KB targets in a group are
// combined into a single retrieval call. Keeping knowledge domains separate is
// required because HybridSearch authorizes its scope from the domain ID in ctx.
func (t *KnowledgeSearchTool) concurrentSearchByTargets(
	...
	// Filter out non-searchable KBs such as graph-only stores. knowledge_search
	// can only serve KBs with vector or keyword indexing; feeding a non-indexed
	...
	// should be queried via graph tools instead.
	//
	...
	for _, st := range searchTargets {
		...
		if knownKBs[st.KnowledgeBaseID] {
			logger.Infof(ctx, "[Tool][KnowledgeSearch] Skipping non-searchable KB %s (no vector/keyword index)", st.KnowledgeBaseID)
			continue
		}
	...
	if len(filteredTargets) == 0 {
		logger.Infof(ctx, "[Tool][KnowledgeSearch] No searchable KBs in scope; skipping retrieval")
		return nil
	}
	searchTargets = filteredTargets

	// Resolve actual model identities (name + endpoint) for cross-knowledgeDomain grouping
	...
	groups := make(map[searchGroupKey][]*types.SearchTarget)
	for _, st := range searchTargets {
		if st == nil {
			continue
		}
		knowledgeDomainID := st.KnowledgeDomainID
		if knowledgeDomainID == 0 {
			knowledgeDomainID, _ = types.KnowledgeDomainIDFromContext(ctx)
		}
		if knowledgeDomainID == 0 {
			logger.Warnf(ctx, "[Tool][KnowledgeSearch] Skipping KB %s because its knowledge domain is unavailable", st.KnowledgeBaseID)
			continue
		}
		key := searchGroupKey{
			modelKey:          modelKeyMap[st.KnowledgeBaseID],
	...
	for _, query := range queries {
		q := query
		for groupKey, targets := range groups {
			wg.Add(1)
			go func(q string, groupKey searchGroupKey, targets []*types.SearchTarget) {
				defer wg.Done()
				searchCtx := context.WithValue(ctx, types.KnowledgeDomainIDContextKey, groupKey.knowledgeDomainID)

				// Compute embedding once for this (model, knowledge domain, query) tuple.
				var queryEmbedding []float32
				if groupKey.modelKey != "" {
					emb, err := t.knowledgeBaseService.GetQueryEmbedding(searchCtx, targets[0].KnowledgeBaseID, q)
					if err != nil {
						logger.Warnf(ctx, "[Tool][KnowledgeSearch] Failed to pre-compute embedding for model %s in knowledge domain %d: %v", groupKey.modelKey, groupKey.knowledgeDomainID, err)
					} else {
						queryEmbedding = emb
					}
				}
				...
				if len(fullKBIDs) > 0 {
					...
					go func() {
						...
						kbResults, err := t.knowledgeBaseService.HybridSearch(searchCtx, fullKBIDs[0], searchParams)
						if err != nil {
				...
				for _, target := range knowledgeTargets {
					...
					go func() {
						...
						kbResults, err := t.knowledgeBaseService.HybridSearch(searchCtx, st.KnowledgeBaseID, searchParams)
						if err != nil {
...
func (t *KnowledgeSearchTool) formatOutput(
	...
	// other retrieval tools (`<search_results>`, per-entry element, `<query>`) so that
	...
	for i, result := range results {
		...
		if _, exists := knowledgeTotalMap[result.KnowledgeID]; !exists {
			effectiveKnowledgeDomainID := t.searchTargets.GetKnowledgeDomainIDForKB(result.KnowledgeBaseID)
			if effectiveKnowledgeDomainID == 0 {
				...
				_, total, err := t.chunkService.GetRepository().ListPagedChunksByKnowledgeID(ctx,
					effectiveKnowledgeDomainID, result.KnowledgeID,
