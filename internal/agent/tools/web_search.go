...
type WebSearchTool struct {
	...
	sessionID             string
	maxResults            int
	providerID            string // WebSearchProviderEntity ID (resolved from agent config or knowledgeDomain default)
}
...
func (t *WebSearchTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	if knowledgeDomainID == 0 {
		logger.Errorf(ctx, "[Tool][WebSearch] KnowledgeDomain ID not found in context")
		return &types.ToolResult{
			Success: false,
			Error:   "knowledgeDomain ID not found in context",
		}, fmt.Errorf("knowledgeDomain ID not found in context")
	}

	// Get knowledgeDomain info from context (same approach as search.go)
	var knowledgeDomain *types.KnowledgeDomain
	if knowledgeDomainValue := ctx.Value(types.KnowledgeDomainInfoContextKey); knowledgeDomainValue != nil {
		knowledgeDomain, _ = knowledgeDomainValue.(*types.KnowledgeDomain)
	}
	...
	if knowledgeDomain != nil {
		searchConfig = types.EffectiveWebSearchConfig(knowledgeDomain.WebSearchConfig)
	}
