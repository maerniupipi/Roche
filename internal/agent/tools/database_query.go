...
## Security Features
- Explicit grant scoping: Queries are restricted to the current user's pre-computed knowledge base and document grants
...
- knowledge_domain_id (INTEGER): Owner knowledgeDomain ID
...
## Important Notes
- Results are always intersected with the current user's explicit knowledge grants
...
type DatabaseQueryInput struct {
	SQL string `json:"sql" jsonschema:"The SELECT SQL query to execute. Results are automatically restricted to the current user's explicit knowledge grants."`
}

// DatabaseQueryTool allows the agent to run read-only SQL inside the current
// user's pre-computed knowledge grant scope.
...
func (t *DatabaseQueryTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	securedSQL, err := t.validateAndSecureSQL(input.SQL)
	if err != nil {
	...
	logger.Infof(ctx, "Executing grant-scoped SQL query - original: %s, secured: %s",
		input.SQL, securedSQL)
...
// validateAndSecureSQL validates the SQL query and injects the user's explicit
// knowledge-base/document grant scope. It fails closed when no scope exists.
func (t *DatabaseQueryTool) validateAndSecureSQL(sqlQuery string) (string, error) {
	scopes := searchScopesFromTargets(t.searchTargets)
	if len(scopes) == 0 {
		return "", fmt.Errorf("no authorized knowledge scope")
	}
	...
	securedSQL, validationResult, err := utils.ValidateAndSecureSQL(
		...
		utils.WithNoCTEs(),
		utils.WithNoSystemColumns(),
		utils.WithNoSchemaAccess(),
		utils.WithNoDangerousFunctions(),
		utils.WithDefaultSafeFunctions(),
		utils.WithAllowedTables("knowledge_bases", "knowledges", "chunks"),
		utils.WithSoftDeleteFilter("knowledge_bases", "knowledges", "chunks"),
		utils.WithHiddenKBFilter(),
		utils.WithInjectionRiskCheck(),
		utils.WithSearchScopes(scopes),
	)
	if err != nil {
		return "", err
	}
