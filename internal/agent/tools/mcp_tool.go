...
//
// Security: service names must be unique per knowledgeDomain (enforced by DB unique index on
// (knowledge_domain_id, name)). The ToolRegistry uses first-wins semantics to prevent a later
...
func (t *MCPTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	if t.gate != nil {
		if meta, ok := ToolExecFromContext(ctx); ok && meta != nil && meta.EventBus != nil {
			...
			if t.gate.NeedsApproval(ctx, knowledgeDomainID, t.service.ID, t.mcpTool.Name) {
