...
// the snippet for a compact retrieval response.
...
func (t *GrepChunksTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	kbKnowledgeDomainMap := t.searchTargets.GetKBKnowledgeDomainMap()
	...
	results, err := t.searchChunks(ctx, queries, fullKBIDs, knowledgeIDs, tagTargets, kbKnowledgeDomainMap)
	if err != nil {
	...
	for _, target := range t.searchTargets {
		...
		case len(target.TagIDs) > 0:
			...
			if knowledgeDomainID == 0 {
				knowledgeDomainID = t.searchTargets.GetKnowledgeDomainIDForKB(target.KnowledgeBaseID)
			}
			tagIDs := dedupNonEmptyStrings(target.TagIDs)
			if len(tagIDs) == 0 || knowledgeDomainID == 0 {
				continue
			}
			scopeKey := fmt.Sprintf("%s:%d:%s", target.KnowledgeBaseID, knowledgeDomainID, strings.Join(tagIDs, "\x00"))
...
// Specific knowledge IDs, tag-constrained KB scopes, and full-KB (kb, knowledgeDomain)
...
func scopeClause(
	...
	kbKnowledgeDomainMap map[string]uint64,
) (string, []interface{}) {
	var clauses []string
	var args []interface{}
	...
	for _, target := range tagTargets {
		...
		knowledgeDomainID := target.KnowledgeDomainID
		if knowledgeDomainID == 0 {
			knowledgeDomainID = kbKnowledgeDomainMap[target.KnowledgeBaseID]
		}
		if knowledgeDomainID == 0 {
			continue
		}
		clauses = append(clauses,
			"(chunks.knowledge_base_id = ? AND chunks.knowledge_domain_id = ? AND EXISTS ("+
				"SELECT 1 FROM knowledge_tag_relations ktr "+
				"WHERE ktr.knowledge_id = chunks.knowledge_id AND ktr.tag_id IN ?"+
				"))")
		args = append(args, target.KnowledgeBaseID, knowledgeDomainID, target.TagIDs)
	}
	for _, kbID := range kbIDs {
		knowledgeDomainID := kbKnowledgeDomainMap[kbID]
		if knowledgeDomainID == 0 {
			continue
		}
		clauses = append(clauses, "(chunks.knowledge_base_id = ? AND chunks.knowledge_domain_id = ?)")
		args = append(args, kbID, knowledgeDomainID)
	}
	if len(clauses) == 0 {
		return "", nil
	}
...
func (t *GrepChunksTool) searchChunks(
	...
	scopeSQL, scopeArgs := scopeClause(kbIDs, knowledgeIDs, tagTargets, kbKnowledgeDomainMap)
	if scopeSQL == "" {
		logger.Warnf(ctx, "[Tool][GrepChunks] No valid scope (kb-knowledgeDomain pairs / knowledge IDs)")
		return nil, nil
	}
	...
	// which matches the other retrieval tools in this codebase.
...
// elements, matching the shared retrieval output shape. Chunks that were already
