...
type ListKnowledgeChunksTool struct {
	...
	searchTargets    types.SearchTargets // Pre-computed unified search targets with KB-knowledgeDomain mapping
}
...
func (t *ListKnowledgeChunksTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	// Resolve the knowledge row before applying the agent scope.
	...
	// Use the knowledge row's knowledgeDomain ID for the chunk query.
	effectiveKnowledgeDomainID := knowledge.KnowledgeDomainID
	...
	chunks, total, err := t.chunkService.GetRepository().ListPagedChunksByKnowledgeID(ctx,
		effectiveKnowledgeDomainID, knowledgeID, pagination, []types.ChunkType{types.ChunkTypeText, types.ChunkTypeFAQ}, "", "", "", "", "")
	if err != nil {
		return &types.ToolResult{
			Success: false,
	...
	if fetched > 0 {
		...
		infoMap := searchutil.CollectImageInfoByChunkIDs(ctx, t.chunkService.GetRepository(), effectiveKnowledgeDomainID, chunkIDs)
		for _, c := range chunks {
			if c.ImageInfo == "" {
...
func (t *ListKnowledgeChunksTool) executeByChunkID(ctx context.Context, chunkID string) (*types.ToolResult, error) {
	...
	if chunk.ImageInfo == "" {
		effectiveKnowledgeDomainID := t.searchTargets.GetKnowledgeDomainIDForKB(chunk.KnowledgeBaseID)
		if effectiveKnowledgeDomainID > 0 {
			infoMap := searchutil.CollectImageInfoByChunkIDs(ctx, t.chunkService.GetRepository(), effectiveKnowledgeDomainID, []string{chunk.ID})
...
// Resolves the knowledge row before loading its chunks.
