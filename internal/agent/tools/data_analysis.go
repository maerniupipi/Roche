...
func (t *DataAnalysisTool) LoadFromKnowledge(ctx context.Context, knowledge *types.Knowledge) (*TableSchema, error) {
	...
	if t.enforceScope {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to validate knowledge scope: %w", err)
		}
		if !allowed {
			return nil, fmt.Errorf("knowledge %s is not accessible in the current session", knowledge.ID)
		}
	}
...
func (t *DataAnalysisTool) materializeKnowledgeFile(ctx context.Context, knowledge *types.Knowledge) (string, func(), error) {
	...
	tmp, err := os.CreateTemp("", "rochekap-data-analysis-*"+suffix)
	if err != nil {
...
func (t *DataAnalysisTool) LoadFromKnowledgeID(ctx context.Context, knowledgeID string) (*TableSchema, error) {
	// Resolve the document before validating it against the agent scope.
...
func (t *DataAnalysisTool) resolveFileServiceForKnowledge(ctx context.Context, knowledge *types.Knowledge) interfaces.FileService {
	...
	if knowledgeDomain == nil {
		...
		if tid, ok := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64); ok {
			knowledgeDomainID = tid
		}
		if knowledgeDomainID == 0 && kb != nil {
			knowledgeDomainID = knowledge.KnowledgeDomainID
		}
		if knowledgeDomainID > 0 && t.knowledgeDomainService != nil {
			resolvedKnowledgeDomain, err := t.knowledgeDomainService.GetKnowledgeDomainByID(ctx, knowledgeDomainID)
			if err != nil {
				logger.Warnf(ctx, "[Tool][DataAnalysis][storage] get knowledgeDomain failed: session_id=%s knowledge_id=%s kb_id=%s knowledge_domain_id=%d err=%v",
					t.sessionID, knowledge.ID, kbID, knowledgeDomainID, err)
			} else if resolvedKnowledgeDomain != nil {
				knowledgeDomain = resolvedKnowledgeDomain
				logger.Infof(ctx, "[Tool][DataAnalysis][storage] resolved knowledgeDomain from service: session_id=%s knowledge_id=%s kb_id=%s knowledge_domain_id=%d",
					t.sessionID, knowledge.ID, kbID, knowledgeDomainID)
			}
		}
	}
	...
	if provider == "" || knowledgeDomain == nil || knowledgeDomain.StorageEngineConfig == nil {
		hasKnowledgeDomainStorageConfig := knowledgeDomain != nil && knowledgeDomain.StorageEngineConfig != nil
		logger.Infof(ctx, "[Tool][DataAnalysis][storage] fallback default: session_id=%s knowledge_id=%s kb_id=%s provider=%q knowledge_domain_cfg=%t",
			t.sessionID, knowledge.ID, kbID, provider, hasKnowledgeDomainStorageConfig)
		return t.fileService
	}

	storageConfig := knowledgeDomain.StorageEngineConfig
