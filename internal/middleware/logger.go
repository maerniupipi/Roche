...
var sensitiveFieldRegex = regexp.MustCompile(
	`(?i)("(?:password|passwd|token|access[_-]?token|refresh[_-]?token|id[_-]?token|` +
		...
		`client[_-]?secret|private[_-]?key|secret|saml[_-]?response|relay[_-]?state|` +
		`authorization[_-]?url|nonce)")\s*:\s*"[^"]*"`,
)
...
func RequestID() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Set request ID in the global context for logging, and stamp the
		// 操作人发起请求的 IP so every audit write path (Log / LogDenied /
		// business audit) can record it without each call site reaching
		// into gin. gin's c.ClientIP() honours the trusted-proxy config,
		// so this is the same address the Logger middleware logs.
...
func Logger() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if c.Request.Method == "POST" || c.Request.Method == "PUT" || c.Request.Method == "PATCH" {
			if path == samlACSPath {
				// A SAML response contains the signed assertion and user attributes.
				// Never copy it into application logs. The body remains untouched for
				// the ACS handler to parse and validate.
				requestBody = "[SAML assertion redacted]"
			} else {
