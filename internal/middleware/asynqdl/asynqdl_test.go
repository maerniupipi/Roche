...
func TestMiddleware_NilHandlerErr_NoInsert(t *testing.T) {
	repo := &fakeRepo{}
	if err := runMiddleware(repo, "any:task", []byte(`{"knowledge_domain_id":1}`), nil); err != nil {
...
func TestMiddleware_FailureWithoutAsynqCtx_RecordsRow(t *testing.T) {
	...
	payload, _ := json.Marshal(map[string]any{
		"knowledge_domain_id": 42,
		"knowledge_base_id":   "kb-abc",
		"knowledge_id":        "k-1",
	})
	wantErr := errors.New("boom")
	gotErr := runMiddleware(repo, "summary:generation", payload, wantErr)
	...
	if row.TaskType != "summary:generation" {
		t.Errorf("task_type: got %q", row.TaskType)
	}
	if row.KnowledgeDomainID != 42 {
		t.Errorf("knowledge_domain_id: got %d", row.KnowledgeDomainID)
	}
...
func TestMiddleware_RepoFailure_DoesNotMaskTaskError(t *testing.T) {
	...
	gotErr := runMiddleware(repo, "any", []byte(`{"knowledge_domain_id":1}`), wantErr)
	if !errors.Is(gotErr, wantErr) {
...
func TestInferScope_Priority(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "knowledge_domain_id fallback",
			probe:     payloadProbe{KnowledgeDomainID: 7},
			wantScope: types.TaskScopeKnowledgeDomain,
			wantID:    "7",
		},
		{
