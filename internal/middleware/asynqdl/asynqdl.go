...
// SQL-query failures by task type, scope, or knowledge domain.
//
...
// well-known fields {knowledge_domain_id, knowledge_base_id, kb_id, knowledge_id,
...
func buildDeadLetter(t *asynq.Task, taskErr error, attempts int) *types.TaskDeadLetter {
	...
	return &types.TaskDeadLetter{
		KnowledgeDomainID: probe.KnowledgeDomainID,
		TaskType:          t.Type(),
		Scope:             scope,
		ScopeID:           scopeID,
		RelatedID:         relatedID,
...
//
//   - knowledge_domain_id          — uniformly the knowledge-domain uint64
//   - knowledge_base_id  — chunk / image / post-process / ...
...
// task: knowledge_base is broader than knowledge, etc. Chunk extract is
// scope="knowledge"; and so on.
//
...
func inferScope(p payloadProbe) (string, string) {
	...
	case p.KnowledgeDomainID != 0:
		return types.TaskScopeKnowledgeDomain, formatUint(p.KnowledgeDomainID)
	default:
