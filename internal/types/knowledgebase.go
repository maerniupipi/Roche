...
type KnowledgeBase struct {
	...
	// KnowledgeDomainID is the explicit knowledge-management domain that owns
	// this knowledge base. It is not derived from the current user.
	KnowledgeDomainID uint64 `yaml:"knowledge_domain_id" json:"knowledge_domain_id" gorm:"not null;index"`
	...
	// IsPublic makes the knowledge base readable by every authenticated user
	...
	IsPublic bool `yaml:"is_public"                json:"is_public"                gorm:"default:false;not null"`
	...
	// Storage provider config (new): only stores provider selection; credentials from knowledgeDomain StorageEngineConfig
	...
	// MyPermission is the current caller's effective enterprise permission.
	// It is computed at query time and never persisted.
	MyPermission KnowledgeBasePermissionLevel `yaml:"-" json:"my_permission,omitempty" gorm:"-"`
}
...
// Credentials are managed at the knowledgeDomain level (StorageEngineConfig).
...
// the supplied knowledgeDomain default when the KB does not pin one. This mirrors the
...
func (kb *KnowledgeBase) EffectiveStorageProvider(knowledgeDomainDefault string) string {
	if p := kb.GetStorageProvider(); p != "" {
		return p
	}
	return strings.ToLower(strings.TrimSpace(knowledgeDomainDefault))
}
...
// e.g. "minio://bucket/key" → "minio", "local://knowledgeDomain/file.pdf" → "local"
...
// (as opposed to the knowledgeDomain's env-store fallback).
//
