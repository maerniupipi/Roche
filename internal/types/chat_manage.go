...
type PipelineRequest struct {
	...
	// query-understanding stage. Empty values fall back to knowledgeDomain/global defaults.
	IntentPromptOverrides map[string]string `json:"-"`

	// Misc request-scoped config
	KnowledgeDomainID   uint64 `json:"-"`
	WebSearchEnabled    bool   `json:"-"`
	WebSearchProviderID string `json:"-"` // Resolved from agent config or knowledgeDomain default
	WebSearchMaxResults int    `json:"-"` // Resolved from agent config or knowledgeDomain default
...
func (c *ChatManage) Clone() *ChatManage {
	...
	return &ChatManage{
		PipelineRequest: PipelineRequest{
			...
			KnowledgeDomainID:        c.KnowledgeDomainID,
