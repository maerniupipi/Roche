...
// Aligned with Message.Channel values while allowing finer granularity.
const (
	...
	ChannelGoogleDrive      = "google_drive"      // Google Drive
)
...
type KnowledgeListFilter struct {
	// AllowedKnowledgeIDs is an internal authorization boundary. When non-empty,
	// only these document IDs may be returned. It is never populated directly
	// from client input.
	AllowedKnowledgeIDs []string
	// FilterByFolder distinguishes an omitted folder filter from an explicit
	// request for root documents. FolderID is empty when root is requested.
	FilterByFolder bool
	FolderID       string
...
type Knowledge struct {
	...
	// FolderID identifies the containing folder. Nil means the KB root.
	FolderID *string `json:"folder_id" gorm:"type:varchar(36)"`
	// FolderPath is populated for responses that resolve folder placement.
	FolderPath string `json:"folder_path,omitempty" gorm:"-"`
	...
	// MyPermission is the caller's effective permission for this document.
	// It is computed by the authorization service and is never persisted.
	MyPermission KnowledgeBasePermissionLevel `json:"my_permission,omitempty" gorm:"-"`
}

// KnowledgeFolder represents a folder inside a document knowledge base.
// RelativePath is the normalized, slash-separated path from the KB root.
type KnowledgeFolder struct {
	...
	RelativePath      string    `json:"relative_path"     gorm:"type:varchar(2048);not null"`
	CreatedAt         time.Time `json:"created_at"`
	UpdatedAt         time.Time `json:"updated_at"`
	// MyPermission is the caller's effective permission for this folder.
	// It is computed per request and is never persisted.
	MyPermission KnowledgeBasePermissionLevel `json:"my_permission,omitempty" gorm:"-"`
}

// BeforeCreate generates a UUID for a new knowledge folder.
...
// KnowledgeSearchScope defines a (knowledge_domain_id, knowledge_base_id) search scope.
...
type KnowledgeCheckParams struct {
	...
	FileSize int64
	FileHash string
	// FolderID scopes duplicate-file checks to one folder. A pointer to an
	// empty string means the KB root; nil preserves legacy unscoped checks.
	FolderID *string
	// URL parameters
	URL string
