...
func TestIsEnvStoreID(t *testing.T) {
	tests := []struct {
		...
		{"env milvus ID", "__env_milvus__", true},
		...
		{"empty string", "", false},
		{"similar but not prefix", "_env_milvus__", false},
	}
...
func TestBuildEnvVectorStores(t *testing.T) {
	envMap := map[string]string{
		...
		"TENCENT_VECTORDB_DATABASE":   "rochekap",
		"TENCENT_VECTORDB_COLLECTION": "roche_kap_embeddings",
		"WEAVIATE_HOST":               "weaviate:8080",
		"DORIS_ADDR":                  "doris-fe:9030",
		"DORIS_HTTP_PORT":             "8030",
		"DORIS_DATABASE":              "rochekap",
		"DORIS_USERNAME":              "root",
		"DORIS_PASSWORD":              "doris-pass",
		"DORIS_TABLE_PREFIX":          "roche_kap_embeddings",
	}
	...
	t.Run("retired postgres driver is skipped", func(t *testing.T) {
	...
	t.Run("multiple drivers", func(t *testing.T) {
		...
		assert.Equal(t, "__env_elasticsearch_v8__", stores[0].ID)
		assert.Equal(t, "http://es:9200", stores[0].ConnectionConfig.Addr)
		assert.Equal(t, "elastic", stores[0].ConnectionConfig.Username)
		assert.Equal(t, "secret", stores[0].ConnectionConfig.Password) // unmasked
		assert.Equal(t, "my_index", stores[0].IndexConfig.IndexName)
	})
	...
	t.Run("whitespace trimmed", func(t *testing.T) {
		stores := BuildEnvVectorStores(" sqlite , elasticsearch_v8 ", lookup)
		require.Len(t, stores, 2)
	})

	t.Run("all supported drivers", func(t *testing.T) {
		stores := BuildEnvVectorStores("sqlite,elasticsearch_v8,elasticsearch_v7,qdrant,milvus,weaviate,doris,tencent_vectordb", lookup)
	...
	t.Run("tencent vectordb env store", func(t *testing.T) {
		...
		assert.Equal(t, "rochekap", stores[0].ConnectionConfig.Database)
		assert.Equal(t, "roche_kap_embeddings", stores[0].IndexConfig.CollectionName)
	})
	...
	t.Run("doris env store", func(t *testing.T) {
		...
		assert.Equal(t, "roche_kap_embeddings", stores[0].IndexConfig.CollectionPrefix)
	})
...
func TestFindEnvVectorStore(t *testing.T) {
	...
	t.Run("found", func(t *testing.T) {
		store := FindEnvVectorStore("sqlite", lookup, "__env_sqlite__")
		require.NotNil(t, store)
		assert.Equal(t, "__env_sqlite__", store.ID)
	})
	...
	t.Run("empty driver returns nil", func(t *testing.T) {
		store := FindEnvVectorStore("", lookup, "__env_sqlite__")
		assert.Nil(t, store)
	})
}
...
func TestVectorStore_Validate(t *testing.T) {
	...
	t.Run("knowledge domain is not required for a global store", func(t *testing.T) {
		s := valid
		s.KnowledgeDomainID = 0
		err := s.Validate()
...
func TestIsValidEngineType(t *testing.T) {
	...
	// SQLite is intentionally not registerable as a DB store. PostgreSQL
	// retrieval has been retired; the string remains here only to verify that
	// stale API input is rejected.
...
func TestResolveCollectionName(t *testing.T) {
	...
	t.Run("empty IndexConfig falls back to default", func(t *testing.T) {
		...
		ic := &IndexConfig{}
		assert.Equal(t, "roche_kap_embeddings", ResolveCollectionName(ic, "QDRANT_COLLECTION", "roche_kap_embeddings"))
	})
}
...
func TestValidateIndexConfig(t *testing.T) {
	...
	t.Run("valid names with underscore and hyphen", func(t *testing.T) {
		ic := IndexConfig{
			...
			CollectionPrefix: "RocheKAP_embeddings",
	...
	t.Run("doris GetIndexNameOrDefault falls back when prefix empty", func(t *testing.T) {
		ic := IndexConfig{}
		assert.Equal(t, "roche_kap_embeddings", ic.GetIndexNameOrDefault(DorisRetrieverEngineType))
	})
...
// TestVectorStore_RetiredPostgresAndSQLiteNotRegisterable pins the
// write-path and read-path behavior for a retired engine and an env-only one.
//
...
func TestVectorStore_RetiredPostgresAndSQLiteNotRegisterable(t *testing.T) {
	t.Run("Validate rejects postgres as DB store", func(t *testing.T) {
		v := &VectorStore{
			Name: "test", KnowledgeDomainID: 1,
			EngineType: RetrieverEngineType("postgres"),
		}
		err := v.Validate()
		require.Error(t, err)
		assert.Contains(t, err.Error(), "unsupported engine type")
	...
	t.Run("GetVectorStoreTypes omits postgres and sqlite", func(t *testing.T) {
		...
		assert.NotContains(t, got, "postgres",
			"retired postgres retriever must not appear in the UI dropdown")
