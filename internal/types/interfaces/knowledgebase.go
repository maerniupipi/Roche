...
type KnowledgeBaseService interface {
	...
	// GetKnowledgeBaseByIDOnly retrieves knowledge base by ID without knowledgeDomain filter
	// Used for authorization lookups before knowledgeDomain filtering.
	// Parameters:
	//   - ctx: Context information
	//   - id: Unique identifier of the knowledge base
	// Returns:
	...
	// FillKnowledgeBaseCounts fills KnowledgeCount, ChunkCount, IsProcessing, ProcessingCount for the given KB (uses kb.KnowledgeDomainID).
	FillKnowledgeBaseCounts(ctx context.Context, kb *types.KnowledgeBase) error

	// ListKnowledgeBases lists all knowledge bases under the current knowledgeDomain
	// Parameters:
	...
	// ListKnowledgeBasesByKnowledgeDomainID lists all knowledge bases for a specific knowledgeDomain.
	ListKnowledgeBasesByKnowledgeDomainID(ctx context.Context, knowledgeDomainID uint64) ([]*types.KnowledgeBase, error)
	...
	// TogglePublicKnowledgeBase toggles the public visibility of a knowledge base.
	// When public, every authenticated user can read the KB unless explicitly denied.
	TogglePublicKnowledgeBase(ctx context.Context, id string, isPublic bool) (*types.KnowledgeBase, error)
	...
	// across different knowledgeDomains will share the same key, enabling optimal grouping.
...
type KnowledgeBaseRepository interface {
	...
	// GetKnowledgeBaseByIDAndKnowledgeDomain queries a knowledge base by ID scoped to a knowledgeDomain.
	// Returns ErrKnowledgeBaseNotFound if the KB does not exist or does not belong to the knowledgeDomain.
	// Parameters:
	//   - ctx: Context information
	//   - id: Knowledge base ID
	//   - knowledgeDomainID: KnowledgeDomain ID (enforces knowledgeDomain isolation)
	// Returns:
	//   - Knowledge base object, if found and owned by knowledgeDomain
	//   - Possible errors such as record not existing or wrong knowledgeDomain, database errors, etc.
	GetKnowledgeBaseByIDAndKnowledgeDomain(ctx context.Context, id string, knowledgeDomainID uint64) (*types.KnowledgeBase, error)
	...
	// within a knowledgeDomain scope. Accepts a *gorm.DB handle so callers can share a
	...
	CountByVectorStoreID(ctx context.Context, db *gorm.DB, knowledgeDomainID uint64, storeID string) (int64, error)

	// CountByModelID counts active KBs that reference the given global
	...
	// (knowledgeDomain, user, kb) triple. Returns the resulting pinned_at (nil when
	// pinned=false) and an error. The knowledge_domain_id is captured to support
	// efficient "wipe a knowledgeDomain" cleanups even though (user_id, kb_id)
	...
	// given user has personally pinned in this knowledgeDomain. Used by the list
