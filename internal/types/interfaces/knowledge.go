...
type KnowledgeService interface {
	...
	// GetKnowledgeByID retrieves knowledge by ID (uses knowledgeDomain from context).
	...
	// owning KnowledgeBase, scoped to the caller's knowledgeDomain. Retained for
	// legacy ownership-guard compatibility; current enterprise routes use
	// effective KB permission resolution instead of creator-based access.
	...
	GetKnowledgeBatch(ctx context.Context, knowledgeDomainID uint64, ids []string) ([]*types.Knowledge, error)
	// ListKnowledgeIDsByTagIDs returns document knowledge IDs carrying any of
	// the specified KB-local tags.
	ListKnowledgeIDsByTagIDs(ctx context.Context, knowledgeDomainID uint64, kbID string, tagIDs []string) ([]string, error)
	...
	// ListKnowledgeFolders lists the visible folder hierarchy under a KB.
	// A nil allowedKnowledgeIDs slice means full KB access; a non-nil slice
	// restricts the result to ancestors of the listed documents.
	ListKnowledgeFolders(
		ctx context.Context,
		kbID string,
		allowedKnowledgeIDs []string,
	) ([]*types.KnowledgeFolder, error)
	// DeleteKnowledgeFolder recursively deletes a logical folder, descendant
	// documents and their indexed/storage resources, and direct ACLs.
	DeleteKnowledgeFolder(ctx context.Context, kbID string, folderID string) error
	...
	// SearchKnowledgeForScopes searches knowledge within explicit (knowledge_domain_id, kb_id) scopes.
...
type KnowledgeRepository interface {
	...
	GetKnowledgeByID(ctx context.Context, knowledgeDomainID uint64, id string) (*types.Knowledge, error)
	// GetKnowledgeByIDOnly returns knowledge by ID without knowledgeDomain filter (for permission resolution).
	GetKnowledgeByIDOnly(ctx context.Context, id string) (*types.Knowledge, error)
	ListKnowledgeByKnowledgeBaseID(ctx context.Context, knowledgeDomainID uint64, kbID string) ([]*types.Knowledge, error)
	...
	ListPagedKnowledgeByKnowledgeBaseID(ctx context.Context,
		knowledgeDomainID uint64, kbID string, page *types.Pagination, filter types.KnowledgeListFilter,
	...
	DeleteKnowledge(ctx context.Context, knowledgeDomainID uint64, id string) error
	DeleteKnowledgeList(ctx context.Context, knowledgeDomainID uint64, ids []string) error
	GetKnowledgeBatch(ctx context.Context, knowledgeDomainID uint64, ids []string) ([]*types.Knowledge, error)
	...
	AminusB(ctx context.Context, AKnowledgeDomainID uint64, A string, BKnowledgeDomainID uint64, B string) ([]string, error)
	...
	CountKnowledgeByKnowledgeBaseID(ctx context.Context, knowledgeDomainID uint64, kbID string) (int64, error)
	// CountKnowledgeByStatus counts the number of knowledge items with the specified parse status.
	CountKnowledgeByStatus(ctx context.Context, knowledgeDomainID uint64, kbID string, parseStatuses []string) (int64, error)
	// SearchKnowledge searches knowledge items by keyword across the knowledgeDomain.
	// fileTypes: optional list of file extensions to filter by (e.g., ["csv", "xlsx"])
	SearchKnowledge(ctx context.Context, knowledgeDomainID uint64, keyword string, offset, limit int, fileTypes []string) ([]*types.Knowledge, bool, error)
	...
	FindByMetadataKey(ctx context.Context, knowledgeDomainID uint64, kbID string, key string, value string) (*types.Knowledge, error)
	// SearchKnowledgeInScopes searches knowledge items by keyword within the given (knowledge_domain_id, kb_id) scopes (own + shared).
	...
	ListIDsByTagIDs(ctx context.Context, knowledgeDomainID uint64, kbID string, tagIDs []string) ([]string, error)
