...
type FileService interface {
	...
	SaveFile(ctx context.Context, file *multipart.FileHeader, knowledgeDomainID uint64, knowledgeID string) (string, error)
	...
	SaveBytes(ctx context.Context, data []byte, knowledgeDomainID uint64, fileName string, temp bool) (string, error)
	...
	// (knowledgeDomainID, knowledgeID), returning the new provider:// path. The copy is
	...
	CopyFile(ctx context.Context, srcPath string, knowledgeDomainID uint64, knowledgeID string) (string, error)
}
