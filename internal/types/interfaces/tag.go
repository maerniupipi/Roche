...
type KnowledgeTagRepository interface {
	...
	GetByID(ctx context.Context, knowledgeDomainID uint64, id string) (*types.KnowledgeTag, error)
	// GetBySeqID retrieves a tag by its seq_id.
	GetBySeqID(ctx context.Context, knowledgeDomainID uint64, seqID int64) (*types.KnowledgeTag, error)
	// GetByIDs retrieves multiple tags by their IDs in a single query.
	GetByIDs(ctx context.Context, knowledgeDomainID uint64, ids []string) ([]*types.KnowledgeTag, error)
	GetByIDsOnly(ctx context.Context, ids []string) ([]*types.KnowledgeTag, error)
	// GetBySeqIDs retrieves multiple tags by their seq_ids in a single query.
	GetBySeqIDs(ctx context.Context, knowledgeDomainID uint64, seqIDs []int64) ([]*types.KnowledgeTag, error)
	GetByName(ctx context.Context, knowledgeDomainID uint64, kbID string, name string) (*types.KnowledgeTag, error)
	ListByKB(
		ctx context.Context,
	...
	DeleteUnusedTags(ctx context.Context, knowledgeDomainID uint64, kbID string) (int64, error)
}
