...
type ChunkRepository interface {
	...
	GetChunkByID(ctx context.Context, knowledgeDomainID uint64, id string) (*types.Chunk, error)
	// GetChunkByIDOnly gets a chunk by id without knowledgeDomain filter (for permission resolution)
	GetChunkByIDOnly(ctx context.Context, id string) (*types.Chunk, error)
	// GetChunkBySeqID gets a chunk by seq_id
	GetChunkBySeqID(ctx context.Context, knowledgeDomainID uint64, seqID int64) (*types.Chunk, error)
	// ListChunksByID lists chunks by ids
	ListChunksByID(ctx context.Context, knowledgeDomainID uint64, ids []string) ([]*types.Chunk, error)
	// ListChunksByIDOnly lists chunks by IDs before authorization.
	ListChunksByIDOnly(ctx context.Context, ids []string) ([]*types.Chunk, error)
	// ListChunksBySeqID lists chunks by seq_ids
	ListChunksBySeqID(ctx context.Context, knowledgeDomainID uint64, seqIDs []int64) ([]*types.Chunk, error)
	// ListChunksByKnowledgeID lists chunks by knowledge id
	ListChunksByKnowledgeID(ctx context.Context, knowledgeDomainID uint64, knowledgeID string) ([]*types.Chunk, error)
	...
	ListChunkByParentID(ctx context.Context, knowledgeDomainID uint64, parentID string) ([]*types.Chunk, error)
	// ListChunksByParentIDs lists chunks whose parent_chunk_id is in the given list
	ListChunksByParentIDs(ctx context.Context, knowledgeDomainID uint64, parentIDs []string) ([]*types.Chunk, error)
	...
	DeleteChunk(ctx context.Context, knowledgeDomainID uint64, id string) error
	// DeleteChunks deletes chunks by IDs in batch
	DeleteChunks(ctx context.Context, knowledgeDomainID uint64, ids []string) error
	// DeleteChunksByKnowledgeID deletes chunks by knowledge id
	DeleteChunksByKnowledgeID(ctx context.Context, knowledgeDomainID uint64, knowledgeID string) error
	// DeleteByKnowledgeList deletes all chunks for a knowledge list
	DeleteByKnowledgeList(ctx context.Context, knowledgeDomainID uint64, knowledgeIDs []string) error
	// ListImageInfoByKnowledgeIDs returns non-empty (knowledge_id, image_info) pairs for image cleanup.
	ListImageInfoByKnowledgeIDs(ctx context.Context, knowledgeDomainID uint64, knowledgeIDs []string) ([]ChunkImageInfo, error)
	// MoveChunksByKnowledgeID updates knowledge_base_id for all chunks of a knowledge item
	MoveChunksByKnowledgeID(ctx context.Context, knowledgeDomainID uint64, knowledgeID string, targetKBID string) error
	// DeleteChunksByTagID deletes all chunks with the specified tag ID
	// Returns the IDs of deleted chunks for index cleanup
	DeleteChunksByTagID(ctx context.Context, knowledgeDomainID uint64, kbID string, tagID string, excludeIDs []string) ([]string, error)
	// CountChunksByKnowledgeBaseID counts the number of chunks in a knowledge base.
	CountChunksByKnowledgeBaseID(ctx context.Context, knowledgeDomainID uint64, kbID string) (int64, error)
	// DeleteUnindexedChunks deletes unindexed chunks by knowledge id and chunk index range
	DeleteUnindexedChunks(ctx context.Context, knowledgeDomainID uint64, knowledgeID string) ([]*types.Chunk, error)
	// ListAllFAQChunksByKnowledgeID lists all FAQ chunks for a knowledge ID
	// only ID and ContentHash fields for efficiency
	ListAllFAQChunksByKnowledgeID(ctx context.Context, knowledgeDomainID uint64, knowledgeID string) ([]*types.Chunk, error)
	...
	ListAllFAQChunksWithMetadataByKnowledgeBaseID(ctx context.Context, knowledgeDomainID uint64, kbID string) ([]*types.Chunk, error)
	...
	FindFAQChunkWithDuplicateQuestion(ctx context.Context, knowledgeDomainID uint64, kbID string, excludeChunkID string, questions []string) (*types.Chunk, error)
	// ListAllFAQChunksForExport lists all FAQ chunks for export with full metadata, tag_id, is_enabled, and flags
	ListAllFAQChunksForExport(ctx context.Context, knowledgeDomainID uint64, knowledgeID string) ([]*types.Chunk, error)
	...
	UpdateChunkFlagsBatch(ctx context.Context, knowledgeDomainID uint64, kbID string, setFlags map[string]types.ChunkFlags, clearFlags map[string]types.ChunkFlags) error
	...
	UpdateChunkFieldsByTagID(ctx context.Context, knowledgeDomainID uint64, kbID string, tagID string, isEnabled *bool, setFlags types.ChunkFlags, clearFlags types.ChunkFlags, newTagID *string, excludeIDs []string) ([]string, error)
	...
	FAQChunkDiff(ctx context.Context, srcKnowledgeDomainID uint64, srcKBID string, dstKnowledgeDomainID uint64, dstKBID string) (chunksToAdd []string, chunksToDelete []string, err error)
	...
	ListRecommendedFAQChunks(ctx context.Context, knowledgeDomainID uint64, kbIDs []string, knowledgeIDs []string, limit int) ([]*types.Chunk, error)

	// ListRecentDocumentChunksWithQuestions lists recent document chunks that have generated questions.
	// Filter by kbIDs and/or knowledgeIDs. At least one of them must be non-empty.
	// Returns up to `limit` chunks sorted by updated_at descending.
	ListRecentDocumentChunksWithQuestions(ctx context.Context, knowledgeDomainID uint64, kbIDs []string, knowledgeIDs []string, limit int) ([]*types.Chunk, error)
}
...
type ChunkService interface {
	// CreateChunks creates chunks
	CreateChunks(ctx context.Context, chunks []*types.Chunk) error
	// GetChunkByID gets a chunk by id (uses knowledgeDomain from context)
