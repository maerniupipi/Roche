...
type MCPServiceRepository interface {
	...
	GetByID(ctx context.Context, knowledgeDomainID uint64, id string) (*types.MCPService, error)

	// List retrieves all MCP services for a knowledgeDomain
	List(ctx context.Context, knowledgeDomainID uint64) ([]*types.MCPService, error)

	// ListEnabled retrieves all enabled MCP services for a knowledgeDomain
	ListEnabled(ctx context.Context, knowledgeDomainID uint64) ([]*types.MCPService, error)

	// ListByIDs retrieves MCP services by multiple IDs for a knowledgeDomain
	ListByIDs(ctx context.Context, knowledgeDomainID uint64, ids []string) ([]*types.MCPService, error)
...
type MCPServiceService interface {
	...
	GetMCPServiceByID(ctx context.Context, knowledgeDomainID uint64, id string) (*types.MCPService, error)

	// ListMCPServices lists all MCP services for a knowledgeDomain
	ListMCPServices(ctx context.Context, knowledgeDomainID uint64) ([]*types.MCPService, error)

	// ListMCPServicesByIDs retrieves multiple MCP services by IDs
	ListMCPServicesByIDs(ctx context.Context, knowledgeDomainID uint64, ids []string) ([]*types.MCPService, error)
	...
	DeleteMCPService(ctx context.Context, knowledgeDomainID uint64, id string) error

	// TestMCPService tests the connection to an MCP service and returns available tools/resources
	TestMCPService(ctx context.Context, knowledgeDomainID uint64, id string) (*types.MCPTestResult, error)

	// GetMCPServiceTools retrieves the list of tools from an MCP service
	GetMCPServiceTools(ctx context.Context, knowledgeDomainID uint64, id string) ([]*types.MCPTool, error)

	// GetMCPServiceResources retrieves the list of resources from an MCP service
	GetMCPServiceResources(ctx context.Context, knowledgeDomainID uint64, id string) ([]*types.MCPResource, error)
	...
	UpdateMCPCredentials(
		ctx context.Context, knowledgeDomainID uint64, id string, apiKey *string, token *string,
	...
	ClearMCPCredential(ctx context.Context, knowledgeDomainID uint64, id, field string) error
}
