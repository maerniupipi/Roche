...
type MCPOAuthRepository interface {
	...
	GetClient(ctx context.Context, knowledgeDomainID uint64, serviceID string) (*types.MCPOAuthClient, error)
	...
	DeleteClient(ctx context.Context, knowledgeDomainID uint64, serviceID string) error

	// GetToken returns the stored token for (knowledgeDomain, user, service), or
	// (nil, nil) when the user has not authorized yet.
	GetToken(ctx context.Context, knowledgeDomainID uint64, userID, serviceID string) (*types.MCPOAuthToken, error)

	// GetTokenForPrincipal returns the stored token for (knowledgeDomain, principal, service),
	...
	// re-authorize).
	DeleteToken(ctx context.Context, knowledgeDomainID uint64, userID, serviceID string) error

	// DeleteTokenForPrincipal removes the per-principal token for a service.
	DeleteTokenForPrincipal(ctx context.Context, knowledgeDomainID uint64, principal types.Principal, serviceID string) error
}
