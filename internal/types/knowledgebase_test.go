...
func TestParseProviderScheme(t *testing.T) {
	tests := []struct {
		...
		{"local://knowledgeDomain/file.pdf", "local"},
		{"minio://bucket/key", "minio"},
		{"cos://bucket/key", "cos"},
		{"tos://bucket/key", "tos"},
		{"s3://bucket/key", "s3"},
		{"s3://my-bucket/rochekap/123/exports/abc.png", "s3"},
...
func TestEffectiveStorageProvider(t *testing.T) {
	tests := []struct {
		name                   string
		kbProvider             string
		knowledgeDomainDefault string
		want                   string
	}{
		{"kb pins provider", "minio", "cos", "minio"},
		{"kb empty falls back to knowledgeDomain default", "", "cos", "cos"},
		{"both empty", "", "", ""},
		{"knowledgeDomain default cased", "", "  COS ", "cos"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			kb := &KnowledgeBase{}
			...
			if got := kb.EffectiveStorageProvider(tt.knowledgeDomainDefault); got != tt.want {
				t.Errorf("EffectiveStorageProvider(%q) with kb=%q = %q, want %q",
					tt.knowledgeDomainDefault, tt.kbProvider, got, tt.want)
			}
		})
	}
}
...
func TestEffectiveStorageProvider_CrossBackendDetection(t *testing.T) {
	knowledgeDomainDefault := "minio"
	src := &KnowledgeBase{} // inherits knowledgeDomain default -> minio
	dst := &KnowledgeBase{StorageProviderConfig: &StorageProviderConfig{Provider: "cos"}}

	sp := src.EffectiveStorageProvider(knowledgeDomainDefault)
	dp := dst.EffectiveStorageProvider(knowledgeDomainDefault)
	if sp == "" || dp == "" || sp == dp {
		t.Fatalf("expected cross-backend mismatch, got src=%q dst=%q", sp, dp)
	}

	// Same effective provider (dst empty inherits the same knowledgeDomain default) must NOT be flagged.
	dstSame := &KnowledgeBase{}
	if dstSame.EffectiveStorageProvider(knowledgeDomainDefault) != sp {
		t.Errorf("same knowledgeDomain default should match: got %q vs %q",
			dstSame.EffectiveStorageProvider(knowledgeDomainDefault), sp)
	}
}
