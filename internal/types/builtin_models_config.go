...
//     is_default on other rows of the same model type, mirroring the
//     platform-wide invariant enforced by the API path. Multiple defaults
//     for one type result in last-one-wins with a warning.
//
// Failure handling:
...
func LoadBuiltinModelsConfig(ctx context.Context, db *gorm.DB, configDir string) error {
	...
	for i := range file.BuiltinModels {
		...
		// Mirror the API path's single global default per model type.
		// invariant: clear other defaults before promoting this one.
		// Excluded our own id so we don't churn against ourselves.
		if m.IsDefault {
			if err := db.WithContext(ctx).
				Model(&Model{}).
				Where("type = ? AND id <> ? AND is_default = ?", m.Type, m.ID, true).
				Update("is_default", false).Error; err != nil {
				log.Printf("[builtin-models] WARN: clear existing default for type=%s failed: %v; continuing", m.Type, err)
			}
		}
...
// Source defaults to "remote" and status defaults to "active". IsBuiltin and
