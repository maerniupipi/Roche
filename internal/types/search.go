...
type SearchTarget struct {
	...
	KnowledgeBaseID string `json:"knowledge_base_id"`
	// KnowledgeDomainID is the knowledgeDomain ID that owns this knowledge base
...
// GetKBKnowledgeDomainMap returns a map from knowledge base ID to knowledgeDomain ID
func (st SearchTargets) GetKBKnowledgeDomainMap() map[string]uint64 {
	result := make(map[string]uint64)
	for _, t := range st {
		if t.KnowledgeBaseID != "" {
			result[t.KnowledgeBaseID] = t.KnowledgeDomainID
		}
	}
	return result
}

// GetKnowledgeDomainIDForKB returns the knowledgeDomain ID for a given knowledge base ID
// Returns 0 if not found
func (st SearchTargets) GetKnowledgeDomainIDForKB(kbID string) uint64 {
	for _, t := range st {
		if t.KnowledgeBaseID == kbID {
			return t.KnowledgeDomainID
		}
	}
	return 0
}
