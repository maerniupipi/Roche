...
func rebuildRegistryFromConfig() {
	for id := range builtinAgentEntries {
		...
		BuiltinAgentRegistry[agentID] = func() *CustomAgent {
			return buildAgentFromEntry(agentID, "")
		}
	}
}
...
func GetBuiltinAgentWithContext(ctx context.Context, id string) *CustomAgent {
...
func buildAgentFromEntry(id string, locale string) *CustomAgent {
	builtinAgentEntriesMu.RLock()
	entry, ok := builtinAgentEntries[id]
	builtinAgentEntriesMu.RUnlock()
