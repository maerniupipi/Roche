...
//
// One row per (knowledge_domain_id, service_id). The client_secret is encrypted at rest
// (AES-256-GCM) when SYSTEM_AES_KEY is configured.
type MCPOAuthClient struct {
	ID                string    `json:"id"            gorm:"type:varchar(36);primaryKey"`
	KnowledgeDomainID uint64    `json:"knowledge_domain_id"     gorm:"not null;uniqueIndex:idx_mcp_oauth_clients_knowledge_domain_svc"`
	ServiceID         string    `json:"service_id"    gorm:"type:varchar(36);not null;uniqueIndex:idx_mcp_oauth_clients_knowledge_domain_svc;index"`
...
// tokens are isolated by (knowledge_domain_id, principal_type, principal_id, service_id).
//
// AccessToken and RefreshToken are encrypted at rest (AES-256-GCM) when
// SYSTEM_AES_KEY is configured.
type MCPOAuthToken struct {
	ID                string    `json:"id"            gorm:"type:varchar(36);primaryKey"`
	KnowledgeDomainID uint64    `json:"knowledge_domain_id"     gorm:"not null;uniqueIndex:idx_mcp_oauth_tokens_knowledge_domain_principal_svc"`
	UserID            string    `json:"user_id"       gorm:"type:varchar(512);not null;index"`
	PrincipalType     string    `json:"principal_type" gorm:"type:varchar(32);not null;uniqueIndex:idx_mcp_oauth_tokens_knowledge_domain_principal_svc;index"`
	PrincipalID       string    `json:"principal_id"   gorm:"type:varchar(512);not null;uniqueIndex:idx_mcp_oauth_tokens_knowledge_domain_principal_svc;index"`
	ServiceID         string    `json:"service_id"    gorm:"type:varchar(36);not null;uniqueIndex:idx_mcp_oauth_tokens_knowledge_domain_principal_svc;index"`
