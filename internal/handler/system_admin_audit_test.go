...
// withActor builds a ctx carrying a user id (and a hydrated *User whose
// Username mirrors the id) under the canonical keys, so the helper
// resolves non-empty actor id AND actor name exactly the way the gin
// chain would in production.
func withActor(userID string) context.Context {
	ctx := context.WithValue(context.Background(), types.UserIDContextKey, userID)
	return context.WithValue(ctx, types.UserContextKey, &types.User{ID: userID, Username: userID})
}
...
// and verifies every field the handler is responsible for: knowledge_domain_id=0
...
func TestEmitAdminAudit_PopulatesCanonicalFields(t *testing.T) {
	...
	if got.ActorName != "u-actor" {
		t.Fatalf("actor_name must come from ctx user.Username, got %q", got.ActorName)
	}
...
func TestEmitAdminAudit_NilTargetStillEmitsRow(t *testing.T) {
	...
	// ActorName / ActorRole / Action still pin canonical values even
	// when the target is missing.
	if got.ActorName != "u-actor" || got.ActorRole != "system_admin" {
		t.Fatalf("actor invariants must still hold, got %+v", got)
	}
}
