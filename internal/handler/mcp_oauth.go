...
func (h *MCPOAuthHandler) AuthorizeURL(c *gin.Context) {
	...
	if knowledgeDomainID == 0 || !principal.Valid() {
	...
	service, err := h.svc.GetMCPServiceByID(ctx, knowledgeDomainID, serviceID)
	...
	authURL, err := h.oauth.StartAuthorization(ctx, service, knowledgeDomainID, principal, req.RedirectURI, req.FrontendRedirect)
	if err != nil {
...
func (h *MCPOAuthHandler) Status(c *gin.Context) {
	...
	authorized, err := h.oauth.IsAuthorized(ctx, knowledgeDomainID, principal, serviceID)
	if err != nil {
...
func (h *MCPOAuthHandler) Revoke(c *gin.Context) {
	...
	if err := h.oauth.Revoke(ctx, knowledgeDomainID, principal, serviceID); err != nil {
...
// completed; the backend verifies a token now exists for (knowledgeDomain, user,
...
func (h *MCPOAuthHandler) ResolveMCPOAuth(c *gin.Context) {
	...
	if knowledgeDomainID == 0 || !principal.Valid() || gateUserID == "" {
		c.Error(errors.NewUnauthorizedError("authentication required"))
		return
	}
	if h.gate == nil {
	...
	case "cancel", "reject", "skip":
		if err := h.gate.Resolve(knowledgeDomainID, gateUserID, pendingID, approval.Decision{
			Approved: false,
	...
	if err := h.gate.Resolve(knowledgeDomainID, gateUserID, pendingID, approval.Decision{Approved: true}); err != nil {
		switch {
		case stderrors.Is(err, approval.ErrPendingNotFound):
			c.Error(errors.NewNotFoundError("pending authorization not found or already completed"))
		case stderrors.Is(err, approval.ErrAlreadyResolved):
...
func (h *MCPOAuthHandler) CancelMCPOAuth(c *gin.Context) {
	...
	if knowledgeDomainID == 0 || strings.TrimSpace(gateUserID) == "" {
		c.Error(errors.NewUnauthorizedError("authentication required"))
		return
	}
	if h.gate == nil {
