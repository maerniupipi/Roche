...
// per-session SummaryConfig from knowledgeDomain-level ConversationConfig + config.yaml
// defaults. Both helpers became unreachable when the chat pipeline moved to
// CustomAgent (builtin-quick-answer / smart-reasoning) and the knowledgeDomain-level
