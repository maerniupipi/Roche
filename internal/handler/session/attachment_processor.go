...
func (p *AttachmentProcessor) ProcessAttachment(
	...
	storageURL, err := p.fileService.SaveBytes(ctx, data, knowledgeDomainID, uniqueFileName, false)
	if err != nil {
	...
	if p.isTextFile(ext) {
		...
		if err := p.processWithDocParser(ctx, data, baseName, ext, attachment, knowledgeDomainID); err != nil {
		...
		if err := p.processWithDocumentReader(ctx, data, baseName, ext, attachment, knowledgeDomainID); err != nil {
...
func (p *AttachmentProcessor) processWithDocParser(
	...
	if len(result.ImageRefs) > 0 && p.imageResolver != nil {
		updatedMarkdown, _, err := p.imageResolver.ResolveAndStore(ctx, result, p.fileService, knowledgeDomainID)
		if err != nil {
...
// getParserEngineOverridesFromContext returns parser engine overrides from knowledgeDomain in context.
func getParserEngineOverridesFromContext(ctx context.Context) map[string]string {
	if v := ctx.Value(types.KnowledgeDomainInfoContextKey); v != nil {
		if knowledgeDomain, ok := v.(*types.KnowledgeDomain); ok && knowledgeDomain != nil && knowledgeDomain.ParserEngineConfig != nil {
