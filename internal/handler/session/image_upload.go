...
func (h *Handler) saveImageAttachments(ctx context.Context, images []ImageAttachment, storageNamespaceID uint64) error {
	if len(images) == 0 {
		return nil
	}
	...
	for i := range images {
		...
		fileURL, err := h.fileService.SaveBytes(ctx, imgBytes, storageNamespaceID, storedName, false)
		if err != nil {
