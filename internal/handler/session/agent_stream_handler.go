...
import (
	...
	agenttools "roche.local/knowledge-agent-platform/internal/agent/tools"
...
func (h *AgentStreamHandler) Subscribe() {
	...
	h.eventBus.On(event.EventQuestionUnderstood, h.handleQuestionUnderstood)
	h.eventBus.On(event.EventKnowledgeSearch, h.handleKnowledgeSearch)
...
func (h *AgentStreamHandler) handleThought(ctx context.Context, evt event.Event) error {
	return h.handleProgress(ctx, evt, types.ResponseTypeThinking)
}

func (h *AgentStreamHandler) handleQuestionUnderstood(ctx context.Context, evt event.Event) error {
	return h.handleProgress(ctx, evt, types.ResponseTypeQuestionUnderstood)
}

func (h *AgentStreamHandler) handleKnowledgeSearch(ctx context.Context, evt event.Event) error {
	return h.handleProgress(ctx, evt, types.ResponseTypeKnowledgeRetrieved)
}

func (h *AgentStreamHandler) handleProgress(
	ctx context.Context,
	evt event.Event,
	...
	// Calculate duration if done. Structured unified-QA progress supplies its
	// own stable step ID and duration; legacy agent thoughts keep the old event
	// ID/timer behaviour.
	eventID := evt.ID
	if data.StepID != "" {
		eventID = data.StepID
	...
	if data.ResultCount > 0 {
		metadata["result_count"] = data.ResultCount
	}
	if data.ToolCalls > 0 {
		metadata["tool_calls"] = data.ToolCalls
	}
	if data.ModelCalls > 0 {
		metadata["model_calls"] = data.ModelCalls
	}
	if data.Done {
		startTime := h.eventStartTimes[evt.ID]
		durationMS := data.DurationMS
		if durationMS <= 0 && !startTime.IsZero() {
			durationMS = time.Since(startTime).Milliseconds()
		}
		metadata["duration_ms"] = durationMS
		metadata["completed_at"] = time.Now().Unix()
...
func (h *AgentStreamHandler) handleReferences(ctx context.Context, evt event.Event) error {
	...
	// Answer generators emit the named types.References slice, while some
	// legacy paths still emit the unnamed slice or []interface{}. A type
	// assertion to []*types.SearchResult does not match types.References even
	// though both have the same underlying type, so normalize every supported
	// representation before persisting the assistant message.
	h.knowledgeRefs = appendKnowledgeReferences(h.knowledgeRefs, data.References)
...
func appendKnowledgeReferences(existing []*types.SearchResult, raw any) []*types.SearchResult {
	switch refs := raw.(type) {
	case types.References:
		return append(existing, refs...)
	...
	case []interface{}:
		for _, ref := range refs {
			...
			existing = append(existing, searchResult)
		}
	}
	return existing
}
	...
	if h.finalAnswer == "" && data.FinalAnswer != "" {
		logger.GetLogger(h.ctx).Warnf(
			"No answer events were streamed, emitting fallback answer (len=%d). "+
				...
				"or (2) the model returned tool calls non-incrementally. "+
