...
// carry the effective knowledgeDomain ID, so the handler reads knowledgeDomain from
// context the way it always did.
//
// kgService is retained for legacy creator lookup helpers used by
// compatibility tests. Production chunk routes authorize through the
// enterprise KB access guard before reaching the handler.
...
func (h *ChunkHandler) GetChunkByIDOnly(c *gin.Context) {
	...
	// Get chunk by ID without knowledgeDomain filter (chunk may belong to shared
...
func (h *ChunkHandler) ListKnowledgeChunks(c *gin.Context) {
	...
	// The route-level guard has rewritten the request's knowledgeDomain context
	// to the authorized enterprise knowledgeDomain.
...
// KB; this check stops a same-knowledgeDomain attacker from passing one
...
// @Security     Bearer
// @Router       /chunks/{knowledge_id}/pages/{page_number} [get]
func (h *ChunkHandler) ListChunksByPage(c *gin.Context) {
	ctx := c.Request.Context()
	knowledgeID := secutils.SanitizeForLog(c.Param("knowledge_id"))
	pageStr := c.Param("page_number")

	pageNumber, err := strconv.Atoi(pageStr)
	if err != nil || pageNumber < 1 {
		c.Error(errors.NewBadRequestError("Invalid page number"))
		return
	}

	logger.Infof(ctx, "Querying chunks for knowledge %s, page %d", knowledgeID, pageNumber)

	// Get all chunks for this knowledge
	allChunks, err := h.service.ListChunksByKnowledgeID(ctx, knowledgeID)
	if err != nil {
		c.Error(errors.NewInternalServerError("Failed to query chunks"))
		return
	}

	// Filter chunks by page number
	var pageChunks []*types.Chunk
	for _, chunk := range allChunks {
		metadata := chunk.Metadata
		if len(metadata) > 0 {
			var meta types.DocumentChunkMetadata
			if err := json.Unmarshal(metadata, &meta); err == nil {
				// Exact page match
				if meta.PageNumber != nil && *meta.PageNumber == pageNumber {
					pageChunks = append(pageChunks, chunk)
					continue
				}

				// Cross-page range match
				if meta.PageRange != nil {
					ranges := strings.Split(*meta.PageRange, "-")
					for _, r := range ranges {
						if p, err := strconv.Atoi(strings.TrimSpace(r)); err == nil && p == pageNumber {
							pageChunks = append(pageChunks, chunk)
							break
						}
					}
	...
	logger.Infof(ctx, "Found %d chunks on page %d for knowledge %s", len(pageChunks), pageNumber, knowledgeID)
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"data":    pageChunks,
		"total":   len(pageChunks),
	})
}
