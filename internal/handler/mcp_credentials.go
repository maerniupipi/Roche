...
func (h *MCPCredentialsHandler) Put(c *gin.Context) {
	...
	if req.APIKey == nil && req.Token == nil {
		svc, err := h.svc.GetMCPServiceByID(ctx, knowledgeDomainID, serviceID)
		if err != nil || svc == nil {
	...
	updated, err := h.svc.UpdateMCPCredentials(ctx, knowledgeDomainID, serviceID, req.APIKey, req.Token)
	if err != nil {
...
func (h *MCPCredentialsHandler) DeleteField(c *gin.Context) {
	...
	if err := h.svc.ClearMCPCredential(ctx, knowledgeDomainID, serviceID, field); err != nil {
		logger.ErrorWithFields(ctx, err, map[string]interface{}{
			"service_id": secutils.SanitizeForLog(serviceID),
			"field":      field,
		})
