...
func newDataSourceTestRouter(h *DataSourceHandler) *gin.Engine {
	...
	r.Use(func(c *gin.Context) {
		if knowledgeDomainID, ok := c.Request.Context().Value(types.KnowledgeDomainIDContextKey).(uint64); ok {
			c.Set(types.KnowledgeDomainIDContextKey.String(), knowledgeDomainID)
		}
		c.Next()
	})
...
func withDSCtx(req *http.Request, knowledgeDomainID uint64) *http.Request {
	ctx := req.Context()
...
func TestDataSource_GetSyncLogs_ValidLimitWithinBounds(t *testing.T) {
	...
	kbSvc := &stubKBServiceForDS{
		getByID: func(_ context.Context, _ string) (*types.KnowledgeBase, error) {
			return &types.KnowledgeBase{ID: "kb1", KnowledgeDomainID: 1}, nil
		},
	}
