...
import (
	...
	"roche.local/knowledge-agent-platform/internal/application/service/file"
	"roche.local/knowledge-agent-platform/internal/config"
	"roche.local/knowledge-agent-platform/internal/database"
	"roche.local/knowledge-agent-platform/internal/infrastructure/docparser"
	"roche.local/knowledge-agent-platform/internal/logger"
	"roche.local/knowledge-agent-platform/internal/runtime"
...
type SystemHandler struct {
	...
	knowledgeDomainSvc interfaces.KnowledgeDomainService
...
// semantics of the previous access-audit emitter and the system settings
// audit hook.
//
...
func (h *SystemHandler) emitAdminAudit(
	...
	actorName := types.ActorNameFromContext(ctx)
...
func (h *SystemHandler) GetSystemInfo(c *gin.Context) {
	...
	if runtimeConfig, err := h.getPlatformRuntimeConfig(ctx); err == nil {
		storageConfig = runtimeConfig.StorageEngineConfig
	} else {
		logger.Warnf(ctx, "Failed to load platform storage config for system info: %v", err)
	}
	minioEnabled := h.isMinioConfigured(storageConfig)
...
func (h *SystemHandler) getPlatformRuntimeConfig(ctx context.Context) (*types.PlatformRuntimeConfig, error) {
	if h.knowledgeDomainSvc == nil {
		return &types.PlatformRuntimeConfig{}, nil
	}
	cfg, err := h.knowledgeDomainSvc.GetPlatformRuntimeConfig(ctx)
	if err != nil {
		return nil, err
	}
	if cfg == nil {
...
func (h *SystemHandler) ListParserEngines(c *gin.Context) {
	...
	runtimeConfig, err := h.getPlatformRuntimeConfig(ctx)
	if err != nil {
		logger.Errorf(ctx, "Failed to load platform parser engine config: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"code": 1, "msg": "failed to load parser engine config"})
		return
	}
	overrides := runtimeConfig.ParserEngineConfig.ToOverridesMap()
...
func (h *SystemHandler) ReconnectDocReader(c *gin.Context) {
	...
	runtimeConfig, err := h.getPlatformRuntimeConfig(c.Request.Context())
	if err != nil {
		logger.Errorf(c.Request.Context(), "Failed to load platform parser engine config: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"code": 1, "msg": "failed to load parser engine config"})
		return
	}
	overrides := runtimeConfig.ParserEngineConfig.ToOverridesMap()
...
func (h *SystemHandler) CheckParserEngines(c *gin.Context) {
	...
	merged := types.MergeParserEngineConfigForUpdate(&body, runtimeConfig.ParserEngineConfig)
...
// getMinioConfig resolves MinIO connection parameters from the global platform
// configuration (remote mode) or environment variables (docker/default mode).
func (h *SystemHandler) getMinioConfig(cfg *types.StorageEngineConfig) (endpoint, accessKeyID, secretAccessKey string) {
...
// isMinioConfigured checks whether MinIO connection info is available.
func (h *SystemHandler) isMinioConfigured(cfg *types.StorageEngineConfig) bool {
	endpoint, accessKeyID, secretAccessKey := h.getMinioConfig(cfg)
...
// isCOSConfigured checks whether COS connection info is available.
func (h *SystemHandler) isCOSConfigured(cfg *types.StorageEngineConfig) bool {
	if cfg != nil && cfg.COS != nil {
		cosConf := cfg.COS
...
// isTOSConfigured checks whether TOS connection info is available from global config or env.
func (h *SystemHandler) isTOSConfigured(cfg *types.StorageEngineConfig) bool {
	if cfg != nil && cfg.TOS != nil {
		tosConf := cfg.TOS
...
// isOSSConfigured checks whether OSS connection info is available.
func (h *SystemHandler) isOSSConfigured(cfg *types.StorageEngineConfig) bool {
...
// isKS3Configured checks whether KS3 connection info is available.
func (h *SystemHandler) isKS3Configured(cfg *types.StorageEngineConfig) bool {
	if cfg != nil && cfg.KS3 != nil {
		ks3Conf := cfg.KS3
...
func (h *SystemHandler) GetStorageEngineStatus(c *gin.Context) {
	...
	runtimeConfig, err := h.getPlatformRuntimeConfig(ctx)
	if err != nil {
		logger.Errorf(ctx, "Failed to load platform storage engine config: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"code": 1, "msg": "failed to load storage engine config"})
		return
	}
	storageConfig := runtimeConfig.StorageEngineConfig
	minioConfigured := h.isMinioConfigured(storageConfig)
	minioEnvAvailable := h.isMinioEnvAvailable()
	cosConfigured := h.isCOSConfigured(storageConfig)
	tosConfigured := h.isTOSConfigured(storageConfig)
	s3Configured := h.isS3Configured(storageConfig)
	ossConfigured := h.isOSSConfigured(storageConfig)
	ks3Configured := h.isKS3Configured(storageConfig)
...
func (h *SystemHandler) isS3Configured(cfg *types.StorageEngineConfig) bool {
...
// rare in practice but possible during a knowledgeDomain merge) would otherwise
...
// @Description  not knowledgeDomain-scope). SystemAdmin only.
// @Tags         System Admin
// @Produce      json
...
// ApplyDefaultStorageQuotaToAllKnowledgeDomains godoc
...
// @Description  GiB into storage_quota for every row in knowledgeDomains. Bypasses
// @Description  the per-knowledgeDomain PUT whitelist, which forbids storage_quota
...
// @Router       /system/admin/knowledge-domains/apply-default-storage-quota [post]
func (h *SystemHandler) ApplyDefaultStorageQuotaToAllKnowledgeDomains(c *gin.Context) {
	ctx := logger.CloneContext(c.Request.Context())

	// Resolve via the same 3-tier path the CreateKnowledgeDomain handler uses
	// so the action's effect mirrors what new knowledgeDomains would receive.
	...
	affected, err := h.knowledgeDomainSvc.BulkSetStorageQuota(ctx, quotaBytes)
	if err != nil {
	...
	if h.auditSvc != nil {
		...
		_ = h.auditSvc.Log(ctx, &types.AuditLog{
			...
			TargetType:  "knowledge_domain_storage_quota",
			TargetID:    "all",
			Outcome:     types.AuditOutcomeSuccess,
...
// ============================================================================
// User Management: ban / unban / batch roles / list (P2)
// ----------------------------------------------------------------------------
// Endpoints gated to SystemAdmin via RegisterSystemAdminRoutes.
// ============================================================================
...
// @Accept       json
// @Produce      json
// @Param        request body types.BanUserRequest true "Ban request"
// @Success      200 {object} types.UserBanActionResponse "User banned"
// @Failure      400 {object} types.UserBanActionResponse "Bad request"
// @Failure      404 {object} types.UserBanActionResponse "User not found"
// @Failure      500 {object} types.UserBanActionResponse "Internal error"
// @Router       /system/admin/users/ban [post]
func (h *SystemHandler) BanUser(c *gin.Context) {
	ctx := logger.CloneContext(c.Request.Context())

	var req types.BanUserRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, types.UserBanActionResponse{
	...
	actorID, _ := types.UserIDFromContext(ctx)
	user, err := h.userSvc.BanUser(ctx, req.UserID, actorID)
	if err != nil {
		logger.Errorf(ctx, "Error banning user %s: %v", req.UserID, err)
		...
		c.JSON(statusCode, types.UserBanActionResponse{Code: statusCode, Message: message, Status: "failed"})
		return
	}

	logger.Infof(ctx, "User %s banned by %s", user.ID, actorID)
	h.emitAdminAudit(ctx, types.AuditActionUserBanned, user, map[string]any{
		"target_email":    user.Email,
		"target_username": user.Username,
	})
	c.JSON(http.StatusOK, types.UserBanActionResponse{
		Code:    http.StatusOK,
...
// @Accept       json
// @Produce      json
// @Param        request body types.UnbanUserRequest true "Unban request"
// @Success      200 {object} types.UserBanActionResponse "User unbanned"
// @Failure      400 {object} types.UserBanActionResponse "Bad request"
// @Failure      404 {object} types.UserBanActionResponse "User not found"
// @Failure      500 {object} types.UserBanActionResponse "Internal error"
// @Router       /system/admin/users/unban [post]
func (h *SystemHandler) UnbanUser(c *gin.Context) {
	ctx := logger.CloneContext(c.Request.Context())
	...
	actorID, _ := types.UserIDFromContext(ctx)
	user, err := h.userSvc.UnbanUser(ctx, req.UserID, actorID)
	if err != nil {
		logger.Errorf(ctx, "Error unbanning user %s: %v", req.UserID, err)
		statusCode := http.StatusInternalServerError
	...
	logger.Infof(ctx, "User %s unbanned by %s", user.ID, actorID)
	h.emitAdminAudit(ctx, types.AuditActionUserUnbanned, user, map[string]any{
		"target_email":    user.Email,
		"target_username": user.Username,
	})
	c.JSON(http.StatusOK, types.UserBanActionResponse{
...
// @Description  tokens without changing the account status. The user must sign in
// @Description  again afterwards. AdminBackend only.
// @Tags         System Admin
// @Accept       json
// @Produce      json
// @Param        request body types.OfflineUserRequest true "Offline request"
// @Success      200 {object} types.UserBanActionResponse "User offline"
// @Failure      400 {object} types.UserBanActionResponse "Bad request"
// @Failure      404 {object} types.UserBanActionResponse "User not found"
// @Failure      500 {object} types.UserBanActionResponse "Internal error"
// @Router       /system/admin/users/offline [post]
func (h *SystemHandler) OfflineUser(c *gin.Context) {
	ctx := logger.CloneContext(c.Request.Context())

	var req types.OfflineUserRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, types.UserBanActionResponse{
			Code:    http.StatusBadRequest,
			Message: "Invalid request: " + err.Error(),
	...
	actorID, _ := types.UserIDFromContext(ctx)
	user, err := h.userSvc.OfflineUser(ctx, req.UserID, actorID)
	if err != nil {
		logger.Errorf(ctx, "Error forcing user %s offline: %v", req.UserID, err)
		statusCode := http.StatusInternalServerError
	...
	logger.Infof(ctx, "User %s forced offline by %s", user.ID, actorID)
	h.emitAdminAudit(ctx, types.AuditActionUserOfflined, user, map[string]any{
		"target_email":    user.Email,
		"target_username": user.Username,
	})
	c.JSON(http.StatusOK, types.UserBanActionResponse{
...
// BatchUpdateUserRoles godoc
// @Summary      Batch update knowledge officer roles
// @Description  Set role_knowledge_officer for a batch of users, optionally
// @Description  binding the given knowledge bases within a knowledge domain.
// @Description  This endpoint never modifies the operator role — use
// @Description  /system/admin/users/roles/operator for that. SystemAdmin only.
// @Tags         System Admin
// @Accept       json
// @Produce      json
// @Param        request body types.BatchUpdateUserRolesRequest true "Batch knowledge officer role update request"
// @Success      200 {object} types.ActionResponse "Roles updated"
// @Failure      400 {object} types.ActionResponse "Bad request"
// @Failure      500 {object} types.ActionResponse "Internal error"
// @Router       /system/admin/users/roles [put]
func (h *SystemHandler) BatchUpdateUserRoles(c *gin.Context) {
	ctx := logger.CloneContext(c.Request.Context())

	var req types.BatchUpdateUserRolesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, types.ActionResponse{
			Code:    http.StatusBadRequest,
			Message: "Invalid request: " + err.Error(),
			Status:  "failed",
		})
	...
	actorID, _ := types.UserIDFromContext(ctx)
	affected, err := h.userSvc.BatchUpdateUserRoles(ctx, req.UserIDs, req.Roles, actorID)
	if err != nil {
		logger.Errorf(ctx, "Error updating user roles: %v", err)
		c.JSON(http.StatusInternalServerError, types.ActionResponse{
	...
	rolesChanged := make([]string, 0)
	if req.Roles.RoleKnowledgeOfficer != nil {
		rolesChanged = append(rolesChanged, fmt.Sprintf("knowledge_officer=%d", *req.Roles.RoleKnowledgeOfficer))
	}
	if req.Roles.KnowledgeDomainID != nil {
		rolesChanged = append(rolesChanged, fmt.Sprintf("knowledge_domain_id=%d", *req.Roles.KnowledgeDomainID))
	}
	if len(req.Roles.KnowledgeBaseIDs) > 0 {
		rolesChanged = append(rolesChanged, fmt.Sprintf("knowledge_base_ids=%v", req.Roles.KnowledgeBaseIDs))
	}

	// Audit for role update
	if h.auditSvc != nil {
		actorID2, _ := types.UserIDFromContext(ctx)
		actorName := types.ActorNameFromContext(ctx)
		details, _ := json.Marshal(map[string]any{
	...
	c.JSON(http.StatusOK, types.ActionResponse{
		Code:    http.StatusOK,
		Message: "Knowledge officer roles updated successfully",
		Status:  "success",
	})
}

// BatchUpdateOperatorRole godoc
// @Summary      Batch update operator roles
// @Description  Grant/revoke the operator role for a batch of users. The
// @Description  role is stored as is_system_admin: role_operator=1 maps to
// @Description  true, 0 maps to false. This endpoint never modifies the
// @Description  knowledge officer role or its knowledge-base bindings — use
// @Description  /system/admin/users/roles for that. SystemAdmin only.
// @Tags         System Admin
// @Accept       json
// @Produce      json
// @Param        request body types.BatchUpdateOperatorRoleRequest true "Batch operator role update request"
// @Success      200 {object} types.ActionResponse "Roles updated"
// @Failure      400 {object} types.ActionResponse "Bad request"
// @Failure      500 {object} types.ActionResponse "Internal error"
// @Router       /system/admin/users/roles/operator [put]
func (h *SystemHandler) BatchUpdateOperatorRole(c *gin.Context) {
	ctx := logger.CloneContext(c.Request.Context())

	var req types.BatchUpdateOperatorRoleRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, types.ActionResponse{
			Code:    http.StatusBadRequest,
			Message: "Invalid request: " + err.Error(),
	...
	actorID, _ := types.UserIDFromContext(ctx)
	affected, err := h.userSvc.BatchUpdateOperatorRole(ctx, req.UserIDs, req.Roles, actorID)
	if err != nil {
		logger.Errorf(ctx, "Error updating operator roles: %v", err)
		c.JSON(http.StatusInternalServerError, types.ActionResponse{
			Code:    http.StatusInternalServerError,
			Message: "Failed to update operator roles",
			Status:  "failed",
		})
		return
	}
	...
	rolesChanged := make([]string, 0)
	if req.Roles.RoleOperator != nil {
		rolesChanged = append(rolesChanged, fmt.Sprintf("operator=%d", *req.Roles.RoleOperator))
	}

	// Audit for operator role update
	if h.auditSvc != nil {
		actorID2, _ := types.UserIDFromContext(ctx)
		actorName := types.ActorNameFromContext(ctx)
		details, _ := json.Marshal(map[string]any{
...
// @Param        offset               query int    false "Page offset" default(0)
// @Param        limit                query int    false "Page size (max 200)" default(50)
// @Param        query                query string false "Free-text search across username/email/account/names/employee_id"
// @Param        account              query string false "Filter by account (case-insensitive like)"
// @Param        employee_name        query string false "Filter by english_name or chinese_name"
// @Param        is_knowledge_officer query int    false "Filter by role_knowledge_officer (0/1)"
// @Param        is_operator          query int    false "Filter by operator status (1=operator, stored as is_system_admin)"
// @Param        department           query string false "Filter by department_code or department_name"
// @Param        status               query int    false "Filter by status (0=active, 1=banned, 2=resigned)"
// @Success      200 {object} types.ListUsersResponse "Paginated user list"
...
func (h *SystemHandler) ListUsers(c *gin.Context) {
	...
	if v := strings.TrimSpace(c.Query("employee_name")); v != "" {
		filter.EmployeeName = v
	}
	if v := strings.TrimSpace(c.Query("department")); v != "" {
		filter.Department = v
	}
	if v := c.Query("is_knowledge_officer"); v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			filter.IsKnowledgeOfficer = &n
		}
	}
	if v := c.Query("is_operator"); v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			filter.IsOperator = &n
		}
	}
	...
	users, total, err := h.userSvc.ListUsers(ctx, offset, limit, filter)
	if err != nil {
	...
	c.JSON(http.StatusOK, types.ListUsersResponse{
		Total: total,
		Users: infos,
	})
}
...
// @Success      200 {object} types.UserDetailResponse "User details"
// @Failure      404 {object} map[string]interface{} "User not found"
...
func (h *SystemHandler) GetUser(c *gin.Context) {
	...
	detail, err := h.userSvc.GetUserDetail(ctx, userID)
	if err != nil {
		logger.Errorf(ctx, "Error getting user detail %s: %v", userID, err)
		c.JSON(http.StatusNotFound, gin.H{"error": "User not found"})
		return
	}
...
// @Description  single identifier in `id_or_email`; the server decides whether it
// @Description  is an email address (stored in `email`) or an employee id
// @Description  (stored in `employee_id`). An input containing '@' or '.com' is
// @Description  treated as an email address; any other input — including
// @Description  alphanumeric ids such as E10002 — is stored as an employee id.
// @Description  The other identifier remains empty until the user signs in via
// @Description  SSO and back-fills it. Created accounts are tagged
// @Description  provider="white_list". username and the initial password are
// @Description  derived/generated server-side. SystemAdmin only.
// @Tags         System Admin
// @Accept       json
// @Produce      json
// @Param        request body types.CreateUserRequest true "Create user request"
// @Success      201 {object} types.CreateUserResponse "User created"
// @Failure      400 {object} types.CreateUserResponse "Bad request"
// @Failure      500 {object} types.CreateUserResponse "Internal error"
...
func (h *SystemHandler) CreateUser(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, types.CreateUserResponse{
			Code:    http.StatusBadRequest,
			Message: "Invalid request: " + err.Error(),
			Status:  "failed",
		})
	...
	user, err := h.userSvc.CreateUser(ctx, &req)
	if err != nil {
		logger.Errorf(ctx, "Error creating user: %v", err)
		c.JSON(http.StatusBadRequest, types.CreateUserResponse{
			Code:    http.StatusBadRequest,
			Message: err.Error(),
			Status:  "failed",
	...
	actorID, _ := types.UserIDFromContext(ctx)
	h.emitAdminAudit(ctx, types.AuditActionUserCreated, user, map[string]any{
		"target_email":    user.Email,
		"target_username": user.Username,
	})
	logger.Infof(ctx, "User %s created by %s", user.ID, actorID)
...
// @Description  the given knowledge bases within a knowledge domain.
// @Description  This endpoint never modifies the operator role — use
// @Description  /system/admin/users/roles/operator/single for that. SystemAdmin only.
// @Tags         System Admin
// @Accept       json
// @Produce      json
// @Param        request body types.UpdateUserRolesRequest true "Update user roles request"
// @Success      200 {object} map[string]interface{} "affected count"
// @Failure      400 {object} map[string]interface{} "Bad request"
// @Router       /system/admin/users/roles/single [put]
func (h *SystemHandler) UpdateUserRoles(c *gin.Context) {
	ctx := logger.CloneContext(c.Request.Context())
	...
	actorID, _ := types.UserIDFromContext(ctx)
	affected, err := h.userSvc.UpdateUserRoles(ctx, req.UserID, req.Roles, actorID)
	if err != nil {
		logger.Errorf(ctx, "Error updating user %s roles: %v", req.UserID, err)
...
// UpdateOperatorRole godoc
// @Summary      Update a single user's operator role
// @Description  Grant/revoke the operator role for one user. The role is
// @Description  stored as is_system_admin: role_operator=1 maps to true,
// @Description  0 maps to false. This endpoint never modifies the knowledge
// @Description  officer role or its knowledge-base bindings — use
// @Description  /system/admin/users/roles/single for that. SystemAdmin only.
// @Tags         System Admin
// @Accept       json
// @Produce      json
// @Param        request body types.UpdateOperatorRoleRequest true "Update operator role request"
// @Success      200 {object} map[string]interface{} "affected count"
// @Failure      400 {object} map[string]interface{} "Bad request"
// @Router       /system/admin/users/roles/operator/single [put]
func (h *SystemHandler) UpdateOperatorRole(c *gin.Context) {
	ctx := logger.CloneContext(c.Request.Context())

	var req types.UpdateOperatorRoleRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid request: " + err.Error()})
		return
	}
	...
	actorID, _ := types.UserIDFromContext(ctx)
	affected, err := h.userSvc.UpdateOperatorRole(ctx, req.UserID, req.Roles, actorID)
	if err != nil {
		logger.Errorf(ctx, "Error updating user %s operator role: %v", req.UserID, err)
