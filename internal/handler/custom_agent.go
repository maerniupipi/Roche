...
func (h *CustomAgentHandler) ListAgents(c *gin.Context) {
	...
	// agents (IsBuiltin=true, CreatedBy="") are platform fixtures rather than
	// user creations; we always keep them regardless of the
	...
	// 批量回填 creator_name，作用同 KB 列表：让前端能区分「我创建」与「同知识域其他成员」。
...
func (h *CustomAgentHandler) UpdateAgent(c *gin.Context) {
	...
	if err != nil {
		...
		case service.ErrEnterpriseAccessDenied:
			c.Error(errors.NewForbiddenError("No permission to manage this agent"))
...
// @Produce      json
// @Param        id                  path      string  true   "智能体ID"
// @Param        knowledge_base_ids  query     string  false  "知识库ID列表（逗号分隔），限定推荐问题来源且必须在当前用户授权范围内"
