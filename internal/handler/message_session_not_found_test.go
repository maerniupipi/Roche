...
func TestLoadMessagesDefaultsToFiftyMessages(t *testing.T) {
	var capturedLimit int
	svc := &stubMessageService{
		getRecent: func(_ context.Context, _ string, limit int) ([]*types.Message, error) {
			capturedLimit = limit
	...
	if w.Code != http.StatusOK {
		t.Fatalf("loading messages without a limit must succeed, got %d body=%s", w.Code, w.Body.String())
	}
	if capturedLimit != 50 {
		t.Fatalf("default message limit = %d, want 50", capturedLimit)
	}
}
...
// owning session (non-owner / wrong knowledgeDomain), the handler used to
