...
func (e *EvaluationHandler) Evaluation(c *gin.Context) {
	...
	knowledgeDomainID, exists := c.Get(string(types.KnowledgeDomainIDContextKey))
	if !exists {
		logger.Error(ctx, "Failed to get knowledgeDomain ID")
		c.Error(errors.NewUnauthorizedError("Unauthorized"))
		return
	}

	logger.Infof(ctx, "Executing evaluation, knowledgeDomain: %v, dataset: %s, knowledge_base: %s, chat: %s, rerank: %s",
		knowledgeDomainID,
