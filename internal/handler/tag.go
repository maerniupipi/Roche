...
// carry the effective knowledgeDomain ID, so handlers below just use
...
// Uses knowledgeDomain from c's context — which the route-level KB-access guard
// has already validated the enterprise knowledgeDomain.
func (h *TagHandler) resolveTagID(c *gin.Context) (string, error) {
	return h.resolveTagIDWithCtx(c, c.Request.Context())
}

// resolveTagIDWithCtx resolves tag_id using the given context for knowledgeDomain.
func (h *TagHandler) resolveTagIDWithCtx(c *gin.Context, ctx context.Context) (string, error) {
	...
	if seqID, err := strconv.ParseInt(tagIDParam, 10, 64); err == nil {
		...
		tag, err := h.tagRepo.GetBySeqID(ctx, knowledgeDomainID, seqID)
		if err != nil {
...
func (h *TagHandler) getChunksBySeqIDs(ctx context.Context, knowledgeDomainID uint64, seqIDs []int64) ([]*types.Chunk, error) {
	return h.chunkRepo.ListChunksBySeqID(ctx, knowledgeDomainID, seqIDs)
}
...
func (h *TagHandler) DeleteTag(c *gin.Context) {
	...
	if len(req.ExcludeIDs) > 0 {
		knowledgeDomainID := types.MustKnowledgeDomainIDFromContext(ctx)
		chunks, err := h.getChunksBySeqIDs(ctx, knowledgeDomainID, req.ExcludeIDs)
		if err != nil {
