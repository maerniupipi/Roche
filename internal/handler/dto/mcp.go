...
// invariant rather than a runtime redaction step. If a future maintainer
...
type MCPServiceResponse struct {
	...
	// per-knowledgeDomain credentials).
...
//
// Builtin MCP services have their knowledgeDomain-specific transport details (URL,
// Headers, EnvVars, StdioConfig) stripped — these reveal how the knowledgeDomain
// configured an upstream provider and must not be visible to other knowledgeDomains
// that see the same builtin row via the cross-knowledgeDomain list.
func NewMCPServiceResponse(ctx context.Context, svc *types.MCPService) *MCPServiceResponse {
	...
	resp := &MCPServiceResponse{
		ID:                svc.ID,
		KnowledgeDomainID: svc.KnowledgeDomainID,
		Name:              svc.Name,
	...
	if svc.IsBuiltin {
		// Builtin services are shared across knowledgeDomains — strip everything that
		// could leak how this knowledgeDomain configured the underlying provider.
		resp.URL = nil
		resp.Headers = nil
		resp.EnvVars = nil
		resp.StdioConfig = nil
