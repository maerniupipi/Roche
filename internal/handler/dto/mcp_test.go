...
func TestMCPServiceResponse_BuiltinStripsKnowledgeDomainConfig(t *testing.T) {
	url := "https://knowledgeDomain-private.example.com"
	svc := &types.MCPService{
		ID:        "builtin-1",
		IsBuiltin: true,
		URL:       &url,
		Headers:   types.MCPHeaders{"X-KnowledgeDomain-Secret": "shhh"},
		AuthConfig: &types.MCPAuthConfig{
			APIKey: "should-not-leak-via-builtin",
		},
	}
	resp := NewMCPServiceResponse(adminContext(), svc)
	assert.Nil(t, resp.URL, "builtin must not leak per-knowledgeDomain URL")
	assert.Nil(t, resp.Headers, "builtin must not leak per-knowledgeDomain headers")
	...
	assert.False(t, strings.Contains(string(body), "X-KnowledgeDomain-Secret"))
}
