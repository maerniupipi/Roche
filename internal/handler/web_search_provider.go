...
// getKnowledgeDomainID extracts knowledgeDomain ID from gin context (set by auth middleware).
func (h *WebSearchProviderHandler) getKnowledgeDomainID(c *gin.Context) uint64 {
	return c.GetUint64(types.KnowledgeDomainIDContextKey.String())
}

// getOwnedProvider loads a provider and verifies it belongs to the given knowledgeDomain.
// Returns (nil, status, msg) on failure so callers can respond immediately.
func (h *WebSearchProviderHandler) getOwnedProvider(
	ctx context.Context, knowledgeDomainID uint64, id string,
...
func (h *WebSearchProviderHandler) CreateProvider(c *gin.Context) {
	...
	if knowledgeDomainID == 0 {
		c.JSON(http.StatusUnauthorized, gin.H{"success": false, "error": "unauthorized: knowledgeDomain context missing"})
		return
	}
	...
	logger.Infof(ctx, "Creating web search provider: knowledgeDomain=%d, name=%s, type=%s",
		knowledgeDomainID, secutils.SanitizeForLog(req.Name), secutils.SanitizeForLog(string(req.Provider)))
...
// ListProviders lists all web search providers for the current knowledgeDomain
func (h *WebSearchProviderHandler) ListProviders(c *gin.Context) {
	...
	providers, err := h.repo.List(ctx, knowledgeDomainID)
	if err != nil {
...
func (h *WebSearchProviderHandler) GetProvider(c *gin.Context) {
	...
	id := c.Param("id")
	provider, status, msg := h.getOwnedProvider(ctx, knowledgeDomainID, id)
...
func (h *WebSearchProviderHandler) UpdateProvider(c *gin.Context) {
	...
	// Ownership check
	existing, status, msg := h.getOwnedProvider(ctx, knowledgeDomainID, id)
	if status != http.StatusOK {
		c.JSON(status, gin.H{"success": false, "error": msg})
		return
	}
	...
	// partial PUT would silently drop knowledgeDomain-configured extras.
	...
	updated, _ := h.repo.GetByID(ctx, knowledgeDomainID, id)
	if updated != nil {
...
func (h *WebSearchProviderHandler) DeleteProvider(c *gin.Context) {
	...
	// Ownership check
	if _, status, msg := h.getOwnedProvider(ctx, knowledgeDomainID, id); status != http.StatusOK {
		c.JSON(status, gin.H{"success": false, "error": msg})
		return
	}

	if err := h.service.DeleteProvider(ctx, knowledgeDomainID, id); err != nil {
