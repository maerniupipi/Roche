...
import (
	...
	"roche.local/knowledge-agent-platform/internal/middleware/asynqdl"
...
// readRedisOpTimeoutMs reads ROCHE_KAP_REDIS_OP_TIMEOUT_MS, falling back to
...
func readRedisOpTimeoutMs() int {
	if v := strings.TrimSpace(os.Getenv("ROCHE_KAP_REDIS_OP_TIMEOUT_MS")); v != "" {
...
// ROCHE_KAP_ASYNQ_CONCURRENCY is unset. The asynq library defaults to
...
func NewAsynqServer(svc interfaces.SystemSettingService) *asynq.Server {
	...
	if svc != nil {
		n := svc.GetInt(context.Background(), "asynq.concurrency", "ROCHE_KAP_ASYNQ_CONCURRENCY", defaultAsynqConcurrency)
		if n > 0 {
			concurrency = int(n)
		}
	}
...
func RunAsynqServer(params AsynqTaskParams) *asynq.ServeMux {
	...
	// failures by task type, scope, or knowledge domain without scraping logs.
	...
	mux.HandleFunc(types.TypeWorkdaySync, params.IntegrationService.ProcessWorkdaySync)
