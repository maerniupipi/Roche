# RocheKAP Data Source Sync Framework

## Overview

The data source sync framework enables RocheKAP to automatically import and synchronize content from external platforms (Feishu, Notion, Confluence, etc.) into knowledge bases. This is the foundational layer upon which all specific connectors are built.
...
- Indexes: knowledge_domain_id, knowledge_base_id, type, status
- Encrypted config field (AES-256-GCM)

### sync_logs table
- Stores sync operation history
- Indexes: data_source_id, knowledge_domain_id, status, started_at
...
5. **Explicit authorization**: All operations enforce user and organization grants
