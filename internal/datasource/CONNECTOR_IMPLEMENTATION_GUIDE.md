...
A connector is an adapter that translates between RocheKAP's data model and an external platform's API. It handles:
...
func (c *YourConnector) ListResources(ctx context.Context, config *types.DataSourceConfig) ([]types.Resource, error) {
    ...
    // Convert to RocheKAP Resource format
...
import (
    // ... existing imports
    yourconnector "roche.local/knowledge-agent-platform/internal/datasource/connector/yourtype"
)
...
2. RocheKAP already has `internal/datasource/connector/feishu/` for reference
...
- `internal/datasource/connector/feishu/client.go` - Feishu API patterns
- `internal/datasource/connector/feishu/connector.go` - Resource discovery and content synchronization
