...
5. Use security defaults (recommended for production):
   parseResult, validation := ValidateSQL(
       ...
       WithSecurityDefaults(knowledgeDomainID),
   )

6. Validate and secure SQL with knowledgeDomain isolation:
   securedSQL, validation, err := ValidateAndSecureSQL(
       "SELECT * FROM knowledge_bases",
       WithSecurityDefaults(knowledgeDomainID),
   )
   // securedSQL will have knowledge_domain_id automatically injected:
   // "SELECT * FROM knowledge_bases WHERE knowledge_bases.knowledge_domain_id = 123"
...
7. Custom validation options:
   parseResult, validation := ValidateSQL(
       ...
       WithKnowledgeDomainIsolation(knowledgeDomainID, "sessions"),
   )
*/
...
type sqlValidator struct {
	...
	// KnowledgeDomain isolation
	enableKnowledgeDomainInjection bool
	knowledgeDomainID              uint64
	tablesWithKnowledgeDomainID    map[string]bool
...
// WithKnowledgeDomainIsolation enables automatic knowledge_domain_id injection for multi-knowledgeDomain security
func WithKnowledgeDomainIsolation(knowledgeDomainID uint64, tables ...string) SQLValidationOption {
	return func(v *sqlValidator) {
		v.enableKnowledgeDomainInjection = true
		v.knowledgeDomainID = knowledgeDomainID
		v.tablesWithKnowledgeDomainID = make(map[string]bool)
		if len(tables) == 0 {
			// Default tables with knowledge_domain_id
			// SECURITY: All tables with knowledge_domain_id column must be listed here
			// to ensure proper knowledgeDomain isolation and prevent cross-knowledgeDomain data access
			v.tablesWithKnowledgeDomainID = map[string]bool{
			...
			for _, table := range tables {
				v.tablesWithKnowledgeDomainID[strings.ToLower(table)] = true
			}
		}
	}
}
...
// from query results so they are not visible to end users.
...
func WithSecurityDefaults(knowledgeDomainID uint64) SQLValidationOption {
	return func(v *sqlValidator) {
		...
		WithKnowledgeDomainIsolation(knowledgeDomainID)(v)

		// Default allowed tables
		// SECURITY: Only tables with knowledge_domain_id column should be listed here
		// Tables without knowledge_domain_id (for example, messages) are excluded to prevent
		// cross-knowledgeDomain data access vulnerabilities (CVE: Broken Access Control)
		WithAllowedTables(
			"knowledge_bases",
			"knowledges",
			"chunks",
...
func ValidateSQL(sql string, opts ...SQLValidationOption) (*SQLParseResult, *SQLValidationResult) {
	...
	validator := &sqlValidator{
		...
		tablesWithKnowledgeDomainID: make(map[string]bool),
...
// ValidateAndSecureSQL validates SQL and returns a secured version with knowledgeDomain isolation
...
func ValidateAndSecureSQL(sql string, opts ...SQLValidationOption) (string, *SQLValidationResult, error) {
	...
	// Find validator config to check if knowledgeDomain injection is enabled
	validator := &sqlValidator{
		tablesWithKnowledgeDomainID: make(map[string]bool),
		tablesWithDeletedAt:         make(map[string]bool),
	}
	...
	if !validator.enableKnowledgeDomainInjection && !validator.enableSoftDeleteInjection && !validator.enableHiddenKBFilter && !validator.enableSearchScopeFilter {
	...
	// Inject knowledgeDomain conditions
	securedSQL := validator.injectKnowledgeDomainConditions(normalizedSQL, tablesInQuery)
...
// injectKnowledgeDomainConditions adds knowledge_domain_id filtering to the query
func (v *sqlValidator) injectKnowledgeDomainConditions(sql string, tablesInQuery map[string]string) string {
	if !v.enableKnowledgeDomainInjection {
		return sql
	}

	// Build knowledgeDomain conditions
	var conditions []string
	for tableName, alias := range tablesInQuery {
		if v.tablesWithKnowledgeDomainID[tableName] {
			if tableName == "knowledge_domains" {
				conditions = append(conditions, fmt.Sprintf("%s.id = %d", alias, v.knowledgeDomainID))
			} else {
				conditions = append(conditions, fmt.Sprintf("%s.knowledge_domain_id = %d", alias, v.knowledgeDomainID))
			}
		}
	}
	...
	knowledgeDomainFilter := strings.Join(conditions, " AND ")
	return InjectAndConditions(sql, knowledgeDomainFilter)
}
