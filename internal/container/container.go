...
import (
	...
	memoryRepo "roche.local/knowledge-agent-platform/internal/application/repository/memory/neo4j"
	dorisRepo "roche.local/knowledge-agent-platform/internal/application/repository/retriever/doris"
	elasticsearchRepoV7 "roche.local/knowledge-agent-platform/internal/application/repository/retriever/elasticsearch/v7"
	elasticsearchRepoV8 "roche.local/knowledge-agent-platform/internal/application/repository/retriever/elasticsearch/v8"
	milvusRepo "roche.local/knowledge-agent-platform/internal/application/repository/retriever/milvus"
	neo4jRepo "roche.local/knowledge-agent-platform/internal/application/repository/retriever/neo4j"
	openSearchRepo "roche.local/knowledge-agent-platform/internal/application/repository/retriever/opensearch"
	qdrantRepo "roche.local/knowledge-agent-platform/internal/application/repository/retriever/qdrant"
	sqliteRetrieverRepo "roche.local/knowledge-agent-platform/internal/application/repository/retriever/sqlite"
	tencentVectorDBRepo "roche.local/knowledge-agent-platform/internal/application/repository/retriever/tencentvectordb"
	weaviateRepo "roche.local/knowledge-agent-platform/internal/application/repository/retriever/weaviate"
	...
	memoryService "roche.local/knowledge-agent-platform/internal/application/service/memory"
	...
	feishuConnector "roche.local/knowledge-agent-platform/internal/datasource/connector/feishu"
	googleDriveConnector "roche.local/knowledge-agent-platform/internal/datasource/connector/googledrive"
	notionConnector "roche.local/knowledge-agent-platform/internal/datasource/connector/notion"
	rssConnector "roche.local/knowledge-agent-platform/internal/datasource/connector/rss"
	yuqueConnector "roche.local/knowledge-agent-platform/internal/datasource/connector/yuque"
	...
	infra_web_search "roche.local/knowledge-agent-platform/internal/infrastructure/web_search"
	workdayIntegration "roche.local/knowledge-agent-platform/internal/integration/workday"
	...
	"roche.local/knowledge-agent-platform/internal/router"
	"roche.local/knowledge-agent-platform/internal/stream"
...
func BuildContainer(container *dig.Container) *dig.Container {
	...
	must(container.Provide(workdayIntegration.NewProvider))
	...
	must(container.Provide(repository.NewKnowledgeDomainRepository))
	must(container.Provide(repository.NewKnowledgeDomainAdminRepository))
	must(container.Provide(repository.NewEnterpriseAccessRepository))
	must(container.Provide(repository.NewEnterpriseIntegrationRepository))
	must(container.Provide(repository.NewSSOIdentityRepository))
	...
	must(container.Provide(repository.NewBlacklistEntryRepository))
	must(container.Provide(repository.NewSystemSettingRepository))
	must(container.Provide(repository.NewSuggestedQuestionRepository))
	must(container.Provide(repository.NewExchangeRateRepository))
	must(container.Provide(repository.NewAdminAnswerRecordRepository))
	...
	must(container.Provide(repository.NewUnifiedQARunRepository))
	must(container.Provide(repository.NewDashboardRepository))
	...
	must(container.Provide(service.NewKnowledgeDomainService))
	must(container.Provide(service.NewKnowledgeDomainAdminService))
	must(container.Provide(service.NewAuditLogService))
	must(container.Provide(service.NewBusinessAuditRecorder))
	must(container.Provide(service.NewAuditLogRetentionRunner))
	must(container.Provide(service.NewEnterpriseAccessService))
	...
	must(container.Provide(func(kbService interfaces.KnowledgeBaseService, domainService interfaces.KnowledgeDomainService) *unifiedqa.AuthorizedKBResolver {
		return unifiedqa.NewAuthorizedKBResolver(kbService, domainService)
	}))
	must(container.Provide(unifiedqa.LoadAgentCatalog))
	must(container.Provide(func(cfg *config.Config, modelService interfaces.ModelService) unifiedqa.RouteModel {
		var routeModel *config.UnifiedQARouteModelConfig
		if cfg != nil && cfg.UnifiedQA != nil {
			routeModel = cfg.UnifiedQA.RouteModel
		}
		return unifiedqa.NewChatRouteModel(modelService, routeModel)
	}))
	must(container.Provide(func(
		cfg *config.Config,
		model unifiedqa.RouteModel,
		catalog *unifiedqa.AgentCatalog,
	) (*unifiedqa.MasterAgentRouter, error) {
		prompt := config.FindTemplateByID(cfg.PromptTemplates, "unified-master-route-v1")
		if prompt == nil || strings.TrimSpace(prompt.Content) == "" {
			return nil, fmt.Errorf("unified QA route prompt is not configured")
		}
		grpoPrompt := config.FindTemplateByID(cfg.PromptTemplates, "unified-master-route-grpo-v1")
		if grpoPrompt == nil || strings.TrimSpace(grpoPrompt.Content) == "" {
			return nil, fmt.Errorf("unified QA GRPO route prompt is not configured")
		}
		return unifiedqa.NewMasterAgentRouter(model, catalog, prompt.Content, grpoPrompt.Content), nil
	}))
	must(container.Provide(func(manager *langfuse.Manager) *unifiedqa.NodeRunner {
		return unifiedqa.NewNodeRunner(unifiedqa.NewLangfuseNodeObserver(manager))
	}))
	must(container.Provide(func(
		cfg *config.Config,
		kbService interfaces.KnowledgeBaseService,
		knowledgeDomainService interfaces.KnowledgeDomainService,
	) *unifiedqa.RetrievalAdapter {
		settings := unifiedqa.RetrievalSettings{}
		if cfg != nil && cfg.Conversation != nil {
			settings.MatchCount = cfg.Conversation.EmbeddingTopK
			settings.VectorThreshold = cfg.Conversation.VectorThreshold
			settings.KeywordThreshold = cfg.Conversation.KeywordThreshold
			settings.RerankTopK = cfg.Conversation.RerankTopK
			settings.RerankThreshold = cfg.Conversation.RerankThreshold
		}
		return unifiedqa.NewRetrievalAdapter(kbService, settings, knowledgeDomainService)
	}))
	must(container.Provide(func(modelService interfaces.ModelService) *unifiedqa.RerankModelResolver {
		return unifiedqa.NewRerankModelResolver(modelService)
	}))
	must(container.Provide(func(modelService interfaces.ModelService) unifiedqa.FAQFastPathModel {
		return unifiedqa.NewChatFAQFastPathModel(modelService)
	}))
	must(container.Provide(func(cfg *config.Config, model unifiedqa.FAQFastPathModel) unifiedqa.FAQFastPathReviewer {
		return unifiedqa.NewFAQFastPathValidator(model, func(id string) string {
			if prompt := config.FindTemplateByID(cfg.PromptTemplates, id); prompt != nil {
				return prompt.Content
			}
			return ""
		})
	}))
	must(container.Provide(func(modelService interfaces.ModelService) unifiedqa.ReviewModel {
		return unifiedqa.NewChatReviewModel(modelService)
	}))
	must(container.Provide(func(cfg *config.Config, model unifiedqa.ReviewModel) *unifiedqa.DomainEvidenceReviewer {
		return unifiedqa.NewDomainEvidenceReviewer(model, func(id string) string {
			if prompt := config.FindTemplateByID(cfg.PromptTemplates, id); prompt != nil {
				return prompt.Content
			}
			return ""
	...
	must(container.Provide(func(retrieval *unifiedqa.RetrievalAdapter) *unifiedqa.EvidenceRecoveryExecutor {
		return unifiedqa.NewEvidenceRecoveryExecutor(retrieval, 5)
	}))
	must(container.Provide(func(
		catalog *unifiedqa.AgentCatalog,
		retrieval *unifiedqa.RetrievalAdapter,
		reviewer *unifiedqa.DomainEvidenceReviewer,
		recovery *unifiedqa.EvidenceRecoveryExecutor,
	) *unifiedqa.DomainAgentExecutor {
		return unifiedqa.NewDomainAgentExecutor(catalog, retrieval, reviewer, recovery)
	}))
	must(container.Provide(unifiedqa.NewObservationAggregator))
	must(container.Provide(unifiedqa.LoadTerminologyCatalog))
	must(container.Provide(func(
		cfg *config.Config,
		modelService interfaces.ModelService,
		exchangeRates interfaces.ExchangeRateService,
	) *unifiedqa.AnswerGenerator {
		return unifiedqa.NewAnswerGenerator(modelService, func(id string) string {
			if prompt := config.FindTemplateByID(cfg.PromptTemplates, id); prompt != nil {
				return prompt.Content
			}
			return ""
	...
	must(container.Provide(unifiedqa.NewUnifiedQAService, dig.As(new(unifiedqa.Executor))))
	must(container.Provide(service.NewUnifiedQAObservationService))
	...
	must(container.Provide(service.NewSuggestedQuestionService))
	must(container.Provide(service.NewExchangeRateService))
	must(container.Provide(service.NewAdminAnswerRecordService))
	...
	// KnowledgeDomainStoreOwnership adapter used by the retriever factory functions
	// to verify that a resolved VectorStore belongs to the caller's knowledgeDomain.
	...
	must(container.Provide(service.NewEnterpriseIntegrationService))
	...
	must(container.Provide(service.NewDashboardService))
	...
	must(container.Provide(middleware.NewGlobalAuditRecorder))
	must(container.Invoke(startGlobalAudit))
	logger.Debugf(ctx, "[Container] Global audit recorder registered")
	...
	must(container.Provide(service.NewDashboardStatsService))
	must(container.Invoke(startDashboardStatsService))
	logger.Debugf(ctx, "[Container] Dashboard daily stats service registered")
	...
	must(container.Provide(handler.NewKnowledgeDomainHandler))
	must(container.Provide(handler.NewKnowledgeDomainAdminHandler))
	must(container.Provide(handler.NewAuditLogHandler))
	must(container.Provide(handler.NewKnowledgeBaseHandler))
	must(container.Provide(handler.NewEnterpriseAccessHandler))
	must(container.Provide(handler.NewEnterpriseIntegrationHandler))
	...
	must(container.Provide(handler.NewSuggestedQuestionHandler))
	must(container.Provide(handler.NewExchangeRateHandler))
	must(container.Provide(handler.NewAdminAnswerRecordHandler))
	...
	must(container.Provide(handler.NewUnifiedQAObservationHandler))
	must(container.Provide(handler.NewDashboardHandler))
	must(container.Provide(handler.NewMenuHandler))
	logger.Debugf(ctx, "[Container] HTTP handlers registered")

	// Wire the chat package's local image resolver so multimodal chat can read
	// local:// images that live under a knowledgeDomain's configured storage PathPrefix
...
// do NOT encode the owning knowledgeDomain's configured PathPrefix, so resolving them to
// disk bytes requires rebuilding the FileService from that knowledgeDomain's storage
// config. The owning knowledgeDomain is parsed from the URL's first path segment, which
// resolves stored local images using the knowledgeDomain encoded in their path.
func registerChatLocalImageResolver(knowledgeDomainRepo interfaces.KnowledgeDomainRepository) {
	chat.LocalImageResolver = func(storageURL string) ([]byte, bool) {
		knowledgeDomainID := secutils.ParseKnowledgeDomainIDFromStoragePath(storageURL)
		if knowledgeDomainID == 0 {
			return nil, false
		}
		ctx := context.Background()
		knowledgeDomain, err := knowledgeDomainRepo.GetKnowledgeDomainByID(ctx, knowledgeDomainID)
		if err != nil || knowledgeDomain == nil {
			return nil, false
		}
		baseDir := strings.TrimSpace(os.Getenv("LOCAL_STORAGE_BASE_DIR"))
		fileSvc, _, err := file.NewFileServiceFromStorageConfig("local", knowledgeDomain.StorageEngineConfig, baseDir)
		if err != nil {
			return nil, false
		}
...
type databaseMigrationRunner func(string, database.MigrationOptions) error

// runRequiredMigrations applies all pending migrations before the database is
// exposed to the rest of the application. A schema that failed to migrate is
// unsafe to serve, so the connection is closed and the startup error is
// propagated to dig.
func runRequiredMigrations(
	db *gorm.DB,
	dsn string,
	opts database.MigrationOptions,
	run databaseMigrationRunner,
) error {
	if err := run(dsn, opts); err != nil {
		if sqlDB, dbErr := db.DB(); dbErr != nil {
			logger.Warnf(context.Background(), "Failed to access database connection after migration error: %v", dbErr)
		} else if closeErr := sqlDB.Close(); closeErr != nil {
			logger.Warnf(context.Background(), "Failed to close database connection after migration error: %v", closeErr)
		}
		return fmt.Errorf("database migration failed; refusing to start application: %w", err)
	}
	return nil
}
...
func initDatabase(cfg *config.Config) (*gorm.DB, error) {
	...
	case "postgres":
		...
		migrateDSN = fmt.Sprintf(
			"postgres://%s:%s@%s:%s/%s?sslmode=disable&options=-c%%20app.skip_embedding=true",
			os.Getenv("DB_USER"),
	...
	if os.Getenv("AUTO_MIGRATE") != "false" {
		...
		// Run all versioned migrations. The legacy PostgreSQL embeddings
		// migration is always skipped; Milvus is the vector retrieval engine.
		if err := runRequiredMigrations(
			db,
			migrateDSN,
			migrationOpts,
			database.RunMigrationsWithOptions,
		); err != nil {
...
// Supports multiple retrieval engines (Milvus, Elasticsearch, Qdrant, and others)
// Parameters:
...
func initRetrieveEngineRegistry(
	...
	// events fire under a knowledgeDomain-scoped ctx at indexing time; the env-path
	// registration ctx below has no knowledgeDomain, so those emits self-skip.
...
// Provider instances are created on-demand when knowledgeDomains configure them.
...
func initConnectorRegistry() (*datasource.ConnectorRegistry, error) {
	...
	if err := registry.Register(googleDriveConnector.NewConnector()); err != nil {
		errs = errors.Join(errs, fmt.Errorf("register Google Drive connector: %w", err))
	}
...
// startDashboardStatsService starts the daily dashboard aggregation cron (plus
// the startup backfill) and registers shutdown cleanup. Best-effort: failures
// are logged but do not abort the container — the dashboard endpoints simply
// return empty data until the next successful aggregation.
func startDashboardStatsService(svc *service.DashboardStatsService, cleaner interfaces.ResourceCleaner) {
	if svc == nil {
		return
	}
	if err := svc.Start(context.Background()); err != nil {
		logger.Warnf(context.Background(), "[Container] dashboard stats service start failed: %v", err)
	}
	cleaner.RegisterWithName("DashboardStatsService", func() error {
		svc.Stop()
		return nil
	})
}
...
// startGlobalAudit registers the GlobalAuditRecorder shutdown hook so
// buffered audit entries are flushed on graceful termination. When the
// recorder is a no-op (channel is nil) the cleanup is still registered
// but is an immediate no-op.
func startGlobalAudit(
	recorder *middleware.GlobalAuditRecorder, cleaner interfaces.ResourceCleaner,
) {
	cleaner.RegisterWithName("GlobalAuditRecorder", func() error {
		recorder.Shutdown()
		return nil
	})
}
