...
func (c *MinerUReader) Read(ctx context.Context, req *types.ReadRequest) (*types.ReadResult, error) {
	...
	mdContent, imagesB64, pageMapping, err := c.callFileParse(ctx, content)
	if err != nil {
	...
	metadata := map[string]string{
		"pages": fmt.Sprintf("%d", len(pageMapping)),
	}
	if len(pageMapping) > 0 {
		mappingBytes, err := json.Marshal(pageMapping)
		if err == nil {
			metadata["page_mapping"] = string(mappingBytes)
		}
	}
...
type mineruFileParseResponse struct {
	Results struct {
		Document struct {
			...
			PageMapping []PagePosition     `json:"page_mapping,omitempty"`
		} `json:"document"`
		Files struct {
			MDContent   string            `json:"md_content"`
			Images      map[string]string `json:"images"`
...
func (c *MinerUReader) callFileParse(ctx context.Context, content []byte) (string, map[string]string, []PagePosition, error) {
	...
	if result.Results.Document.MDContent != "" || len(result.Results.Document.Images) > 0 {
		...
		pageMapping := result.Results.Document.PageMapping
		return result.Results.Document.MDContent, result.Results.Document.Images, pageMapping, nil
	}
	if result.Results.Files.MDContent != "" || len(result.Results.Files.Images) > 0 {
		...
		pageMapping := result.Results.Files.PageMapping
		return result.Results.Files.MDContent, result.Results.Files.Images, pageMapping, nil
	}
