...
func (m *mockFileService) SaveFile(ctx context.Context, file *multipart.FileHeader, knowledgeDomainID uint64, knowledgeID string) (string, error) {
	return "", nil
}
func (m *mockFileService) SaveBytes(ctx context.Context, data []byte, knowledgeDomainID uint64, fileName string, temp bool) (string, error) {
	m.saved = append(m.saved, savedEntry{Data: data, KnowledgeDomainID: knowledgeDomainID, FileName: fileName})
...
func (m *mockFileService) CopyFile(ctx context.Context, srcPath string, knowledgeDomainID uint64, knowledgeID string) (string, error) {
	return "", nil
}
...
func TestResolveRemoteImages_NormalDownload(t *testing.T) {
	...
	if fSvc.saved[0].KnowledgeDomainID != 42 {
		t.Errorf("expected knowledgeDomainID 42, got %d", fSvc.saved[0].KnowledgeDomainID)
	}
}
