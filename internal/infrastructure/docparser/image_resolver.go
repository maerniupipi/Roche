...
type ImageResolver struct {
	// KnowledgeDomainID for storage path namespacing
...
func (r *ImageResolver) ResolveAndStore(
	...
	md2, imgDataURIs, _ := r.ResolveDataURIImages(ctx, markdown, fileSvc, knowledgeDomainID)
	markdown = md2
	images = append(images, imgDataURIs...)

	md3, imgHTML, _ := r.ResolveHTMLDataURIImages(ctx, markdown, fileSvc, knowledgeDomainID)
	markdown = md3
	images = append(images, imgHTML...)

	md4, imgBare, _ := r.ResolveBareBase64Content(ctx, markdown, fileSvc, knowledgeDomainID)
	markdown = md4
	...
	for i := len(matches) - 1; i >= 0; i-- {
		...
		stored, ok := r.saveReferencedImage(ctx, fileSvc, knowledgeDomainID, refPath, refMap, savedRefs)
		if !ok {
			continue
		}
	...
	md5, imgRelativeHTML, _ := r.ResolveRelativeHTMLImages(ctx, markdown, fileSvc, knowledgeDomainID, refMap, savedRefs)
	markdown = md5
...
func (r *ImageResolver) saveReferencedImage(
	...
	servingURL, saveErr := fileSvc.SaveBytes(ctx, ref.ImageData, knowledgeDomainID, fileName, false)
	if saveErr != nil {
...
func (r *ImageResolver) ResolveHTMLDataURIImages(
	...
	for i := len(matches) - 1; i >= 0; i-- {
		...
		fileName := uuid.New().String() + ext
		servingURL, saveErr := fileSvc.SaveBytes(ctx, data, knowledgeDomainID, fileName, false)
		if saveErr != nil {
...
func (r *ImageResolver) ResolveRelativeHTMLImages(
	...
	for i := len(matches) - 1; i >= 0; i-- {
		...
		stored, ok := r.saveReferencedImage(ctx, fileSvc, knowledgeDomainID, src, refMap, savedRefs)
		if !ok {
			continue
		}
		images = appendStoredImage(images, stored)
...
func (r *ImageResolver) ResolveBareBase64Content(
	...
	md, imgs1 := r.resolveBareDataURIs(ctx, markdown, fileSvc, knowledgeDomainID)
	markdown = md
	images = append(images, imgs1...)

	md2, imgs2 := r.resolveBareBase64Prefix(ctx, markdown, fileSvc, knowledgeDomainID)
	markdown = md2
...
func downloadImage(ctx context.Context, client *http.Client, remoteURL string) (data []byte, mimeType string, err error) {
	...
	req.Header.Set("User-Agent", "Mozilla/5.0 (compatible; RocheKAP/1.0)")
