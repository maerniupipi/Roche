...
const (
	...
	// KnowledgeDomain related error codes (2000-2099)
	ErrKnowledgeDomainNotFound      ErrorCode = 2000
	ErrKnowledgeDomainAlreadyExists ErrorCode = 2001
	ErrKnowledgeDomainInactive      ErrorCode = 2002
	ErrKnowledgeDomainNameRequired  ErrorCode = 2003
	ErrKnowledgeDomainInvalidStatus ErrorCode = 2004
	...
	// clients distinguish "wrong UUID / cross-knowledgeDomain" from "store exists
...
// guards (e.g. per-user self-service knowledgeDomain creation cap).
...
// KnowledgeDomain related errors
func NewKnowledgeDomainNotFoundError() *AppError {
	return &AppError{
		Code:     ErrKnowledgeDomainNotFound,
		Message:  "知识域不存在",
		HTTPCode: http.StatusNotFound,
	}
}

// NewKnowledgeDomainAlreadyExistsError creates a knowledgeDomain already exists error
func NewKnowledgeDomainAlreadyExistsError() *AppError {
	return &AppError{
		Code:     ErrKnowledgeDomainAlreadyExists,
		Message:  "知识域已存在",
		HTTPCode: http.StatusConflict,
	}
}

// NewKnowledgeDomainInactiveError creates a knowledgeDomain inactive error
func NewKnowledgeDomainInactiveError() *AppError {
	return &AppError{
		Code:     ErrKnowledgeDomainInactive,
		Message:  "知识域已停用",
		HTTPCode: http.StatusForbidden,
	}
}
...
// knowledgeDomain (or carried a malformed UUID). The user-facing message is
// intentionally generic to avoid enumeration oracles — see the structured
// log at the call site for the knowledgeDomain/store pair.
