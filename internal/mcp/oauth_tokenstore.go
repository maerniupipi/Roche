...
// scoped to a single (knowledgeDomain, principal, service) tuple. The mcp-go OAuth handler
...
func newDBTokenStore(
	repo interfaces.MCPOAuthRepository, knowledgeDomainID uint64, principal types.Principal, serviceID string,
) *dbTokenStore {
	return &dbTokenStore{
		repo:              repo,
...
func (s *dbTokenStore) GetToken(ctx context.Context) (*transport.Token, error) {
	...
	row, err := s.repo.GetTokenForPrincipal(ctx, s.knowledgeDomainID, s.principal, s.serviceID)
	if err != nil {
		return nil, err
	}
