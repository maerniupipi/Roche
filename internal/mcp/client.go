...
type ClientConfig struct {
	...
	// The token store is scoped to (KnowledgeDomainID, Principal, Service.ID) so each
...
func buildOAuthConfig(config *ClientConfig, httpClient *http.Client) (transport.OAuthConfig, bool, error) {
	...
	oauthCfg := transport.OAuthConfig{
		...
		TokenStore:            newDBTokenStore(config.OAuthRepo, config.KnowledgeDomainID, principal, svc.ID),
		PKCEEnabled:           true,
		AuthServerMetadataURL: svc.AuthConfig.AuthServerMetadataURL,
		HTTPClient:            httpClient,
	}
	if regClient, err := config.OAuthRepo.GetClient(context.Background(), config.KnowledgeDomainID, svc.ID); err == nil && regClient != nil {
