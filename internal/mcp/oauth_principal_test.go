...
func fakeOAuthKey(knowledgeDomainID uint64, principal types.Principal, serviceID string) string {
	return fmt.Sprintf("%d|%s|%s", knowledgeDomainID, principal.Normalize().StorageID(), serviceID)
}

func (r *fakeOAuthRepo) GetClient(
	_ context.Context, knowledgeDomainID uint64, serviceID string,
) (*types.MCPOAuthClient, error) {
	return r.clients[fmt.Sprintf("%d|%s", knowledgeDomainID, serviceID)], nil
}

func (r *fakeOAuthRepo) SaveClient(_ context.Context, client *types.MCPOAuthClient) error {
	r.clients[fmt.Sprintf("%d|%s", client.KnowledgeDomainID, client.ServiceID)] = client
	return nil
}

func (r *fakeOAuthRepo) DeleteClient(_ context.Context, knowledgeDomainID uint64, serviceID string) error {
	delete(r.clients, fmt.Sprintf("%d|%s", knowledgeDomainID, serviceID))
	return nil
}

func (r *fakeOAuthRepo) GetToken(
	ctx context.Context, knowledgeDomainID uint64, userID, serviceID string,
) (*types.MCPOAuthToken, error) {
	return r.GetTokenForPrincipal(ctx, knowledgeDomainID, types.Principal{Type: types.PrincipalWebUser, ID: userID}, serviceID)
}

func (r *fakeOAuthRepo) GetTokenForPrincipal(
	_ context.Context, knowledgeDomainID uint64, principal types.Principal, serviceID string,
) (*types.MCPOAuthToken, error) {
	return r.tokens[fakeOAuthKey(knowledgeDomainID, principal, serviceID)], nil
}
...
func (r *fakeOAuthRepo) SaveTokenForPrincipal(_ context.Context, token *types.MCPOAuthToken) error {
	...
	r.tokens[fakeOAuthKey(token.KnowledgeDomainID, principal, token.ServiceID)] = token
	return nil
}
...
func (r *fakeOAuthRepo) DeleteToken(
	...
	return r.DeleteTokenForPrincipal(ctx, knowledgeDomainID, types.Principal{Type: types.PrincipalWebUser, ID: userID}, serviceID)
}

func (r *fakeOAuthRepo) DeleteTokenForPrincipal(
	_ context.Context, knowledgeDomainID uint64, principal types.Principal, serviceID string,
) error {
	delete(r.tokens, fakeOAuthKey(knowledgeDomainID, principal, serviceID))
	return nil
}

func TestDBTokenStoreUsesPrincipal(t *testing.T) {
	repo := newFakeOAuthRepo()
	principal := types.Principal{Type: types.PrincipalWebUser, ID: "external-42"}
	...
	require.Equal(t, types.PrincipalWebUser, row.PrincipalType)
	require.Equal(t, "external-42", row.PrincipalID)
