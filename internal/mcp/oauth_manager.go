...
const clientRegistrationName = "RocheKAP"
...
// callback code exchange. Tokens are persisted per (knowledgeDomain, principal, service);
// the registered client is persisted per (knowledgeDomain, service) and reused.
...
func (m *OAuthManager) newHandler(
	ctx context.Context, service *types.MCPService, knowledgeDomainID uint64, principal types.Principal, redirectURI string,
	...
	cfg := transport.OAuthConfig{
		...
		TokenStore:            newDBTokenStore(m.repo, knowledgeDomainID, principal, service.ID),
		PKCEEnabled:           true,
		AuthServerMetadataURL: service.AuthConfig.AuthServerMetadataURL,
	}
	if existing, err := m.repo.GetClient(ctx, knowledgeDomainID, service.ID); err == nil && existing != nil {
...
func (m *OAuthManager) StartAuthorization(
	...
	h, err := m.newHandler(ctx, service, knowledgeDomainID, principal, redirectURI)
	if err != nil {
		return "", err
	}

	// Register a client dynamically if we don't have one yet for this service.
	existing, _ := m.repo.GetClient(ctx, knowledgeDomainID, service.ID)
	if existing == nil {
...
// convenience for callers that only hold a service ID and
...
func (m *OAuthManager) StartAuthorizationForService(
	...
	service, err := m.serviceRepo.GetByID(ctx, knowledgeDomainID, serviceID)
	if err != nil {
	...
	return m.StartAuthorization(ctx, service, knowledgeDomainID, principal, redirectURI, frontendRedirect)
}
...
func (m *OAuthManager) CompleteAuthorization(
	...
	service, err := m.serviceRepo.GetByID(ctx, st.KnowledgeDomainID, st.ServiceID)
	if err != nil {
	...
	h, err := m.newHandler(ctx, service, st.KnowledgeDomainID, principal, st.RedirectURI)
	if err != nil {
...
func (m *OAuthManager) IsAuthorized(
	ctx context.Context, knowledgeDomainID uint64, principal types.Principal, serviceID string,
) (bool, error) {
	tok, err := m.repo.GetTokenForPrincipal(ctx, knowledgeDomainID, principal, serviceID)
	if err != nil {
		return false, err
	}
...
func (m *OAuthManager) Revoke(
	...
	return m.repo.DeleteTokenForPrincipal(ctx, knowledgeDomainID, principal, serviceID)
}
