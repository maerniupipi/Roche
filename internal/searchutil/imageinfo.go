...
func CollectImageInfoByChunkIDs(
	...
	children, err := chunkRepo.ListChunksByParentIDs(ctx, knowledgeDomainID, chunkIDs)
	...
	if len(textChildIDs) > 0 {
		grandChildren, err := chunkRepo.ListChunksByParentIDs(ctx, knowledgeDomainID, textChildIDs)
		if err == nil {
...
func EnrichSearchResultsImageInfo(
	...
	infoMap := CollectImageInfoByChunkIDs(ctx, chunkRepo, knowledgeDomainID, chunkIDs)
	if len(infoMap) == 0 {
		return
	}
