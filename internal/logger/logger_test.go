...
func TestCloneContextPreservesPrincipal(t *testing.T) {
	...
	got, ok := types.PrincipalFromContext(cloned)
	if !ok || got.Type != types.PrincipalWebUser || got.ID != "user-1" {
		t.Fatalf("PrincipalFromContext(cloned) = %#v, %v", got, ok)
	}
}

func TestGetLoggerAcceptsNilContext(t *testing.T) {
	entry := GetLogger(nil)
	if entry == nil {
