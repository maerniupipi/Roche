...
// 这允许在 main() 中加载环境文件后，让 LOG_LEVEL / LOG_PATH 立即生效。
...
// audit-relevant events (cross-knowledgeDomain probes, invariant violations) so that
// log aggregators can index the knowledgeDomain/resource identifiers without
...
func CloneContext(ctx context.Context) context.Context {
	...
	for _, k := range []types.ContextKey{
		...
		types.KnowledgeDomainInfoContextKey,
		...
		// 操作人发起请求的 IP：审计写入路径依赖它给每条记录盖章，
		// 业务 handler 经 CloneContext 传递时必须保留。
		types.ClientIPContextKey,
	} {
