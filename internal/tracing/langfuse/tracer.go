...
func (m *Manager) StartTrace(ctx context.Context, opts TraceOptions) (context.Context, *Trace) {
	...
	sampled := m.sample()
	id := newID()
	t := &Trace{ID: id, manager: m, sampled: sampled, metadata: cloneTraceMetadata(opts.Metadata)}
	...
	if sampled {
		...
		body := traceBody{
			...
			Input:       opts.Input,
			Metadata:    t.metadataSnapshot(nil),
			Tags:        opts.Tags,
			Environment: env,
			Release:     release,
		}
...
func (t *Trace) Finish(output interface{}, metadata map[string]interface{}) {
	...
	body := traceBody{
		ID:       t.ID,
		Output:   output,
		Metadata: t.metadataSnapshot(metadata),
	}
...
// UpdateMetadata adds business correlation fields to the active trace without
// finishing it. Repeated updates retain previously recorded metadata so the
// HTTP middleware's final status update cannot discard unified-QA run fields.
func (t *Trace) UpdateMetadata(metadata map[string]interface{}) {
	if t == nil || t.manager == nil || !t.sampled || len(metadata) == 0 {
		return
	}
	t.manager.enqueue(ingestionEvent{
		ID:        newID(),
...
func (t *Trace) metadataSnapshot(extra map[string]interface{}) map[string]interface{} {
	if t == nil {
		return cloneTraceMetadata(extra)
	}
	t.mu.Lock()
	defer t.mu.Unlock()
	if t.metadata == nil {
		t.metadata = make(map[string]interface{}, len(extra))
	}
	for key, value := range extra {
		t.metadata[key] = value
	}
	return cloneTraceMetadata(t.metadata)
}

func cloneTraceMetadata(source map[string]interface{}) map[string]interface{} {
	if len(source) == 0 {
		return nil
	}
