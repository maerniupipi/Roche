...
func TestTrace_UpdateMetadataIsRetainedByFinish(t *testing.T) {
	srv, drain := newTestServer(t)
	defer srv.Close()
	m := newTestManager(t, srv.URL)

	_, trace := m.StartTrace(context.Background(), TraceOptions{
		Name:     "knowledge-chat",
		Metadata: map[string]interface{}{"request_id": "request-1"},
	})
	trace.UpdateMetadata(map[string]interface{}{
		"unified_qa.run_id": "run-1",
	})
	trace.Finish(nil, map[string]interface{}{
	...
	for _, ev := range drain() {
		...
		var current traceBody
		_ = json.Unmarshal(body, &current)
		if current.ID == trace.ID {
			final = current
		}
	}
	if final.Metadata["request_id"] != "request-1" ||
		final.Metadata["unified_qa.run_id"] != "run-1" ||
		final.Metadata["http.status_code"] != float64(200) {
