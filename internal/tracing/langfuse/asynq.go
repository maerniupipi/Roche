...
// logic: explicit UserID → knowledgeDomain:<id> → empty.
func userIDFromCtx(ctx context.Context) string {
	if v, ok := ctx.Value(types.UserIDContextKey).(string); ok && v != "" {
		return v
	}
	if v, ok := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64); ok && v != 0 {
		return "knowledgeDomain:" + strconv.FormatUint(v, 10)
	}
	return ""
}
