...
// GetChunkByID retrieves a chunk by its ID and knowledgeDomain ID
func (r *chunkRepository) GetChunkByID(ctx context.Context, knowledgeDomainID uint64, id string) (*types.Chunk, error) {
	var chunk types.Chunk
	if err := r.db.WithContext(ctx).Where("knowledge_domain_id = ? AND id = ?", knowledgeDomainID, id).First(&chunk).Error; err != nil {
...
// GetChunkByIDOnly retrieves a chunk by ID without knowledgeDomain filter (for permission resolution).
...
// GetChunkBySeqID retrieves a chunk by its seq_id and knowledgeDomain ID
func (r *chunkRepository) GetChunkBySeqID(ctx context.Context, knowledgeDomainID uint64, seqID int64) (*types.Chunk, error) {
	var chunk types.Chunk
	if err := r.db.WithContext(ctx).Where("knowledge_domain_id = ? AND seq_id = ?", knowledgeDomainID, seqID).First(&chunk).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrChunkNotFound
		}
		return nil, err
...
// ListChunksByIDOnly retrieves chunks by ID before authorization.
...
func (r *chunkRepository) ListChunksBySeqID(
	ctx context.Context, knowledgeDomainID uint64, seqIDs []int64,
) ([]*types.Chunk, error) {
	if len(seqIDs) == 0 {
	...
	var chunks []*types.Chunk
	if err := r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND seq_id IN ?", knowledgeDomainID, seqIDs).
		Find(&chunks).Error; err != nil {
		return nil, err
	}
	return chunks, nil
...
func (r *chunkRepository) ListChunksByKnowledgeID(
	...
	var chunks []*types.Chunk
	if err := r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND knowledge_id = ? and chunk_type = ?", knowledgeDomainID, knowledgeID, "text").
...
func (r *chunkRepository) ListPagedChunksByKnowledgeID(
	...
	baseFilter := func(db *gorm.DB) *gorm.DB {
		db = db.Where("knowledge_domain_id = ? AND knowledge_id = ? AND chunk_type IN (?) AND status in (?)",
			knowledgeDomainID, knowledgeID, chunkType, []int{int(types.ChunkStatusIndexed), int(types.ChunkStatusDefault)})
		if tagID != "" {
...
func (r *chunkRepository) ListChunkByParentID(
	...
	var chunks []*types.Chunk
	if err := r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND parent_chunk_id = ?", knowledgeDomainID, parentID).
		Find(&chunks).Error; err != nil {
		return nil, err
	}
	return chunks, nil
...
func (r *chunkRepository) ListChunksByParentIDs(
	...
	var chunks []*types.Chunk
	if err := r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND parent_chunk_id IN ?", knowledgeDomainID, parentIDs).
		Find(&chunks).Error; err != nil {
		return nil, err
	}
	return chunks, nil
...
func (r *chunkRepository) DeleteChunk(ctx context.Context, knowledgeDomainID uint64, id string) error {
	return r.db.WithContext(ctx).Where("knowledge_domain_id = ? AND id = ?", knowledgeDomainID, id).Delete(&types.Chunk{}).Error
}

// DeleteChunks deletes chunks by IDs in batch.
// To avoid MySQL Error 1390 (too many placeholders), IDs are split into batches.
func (r *chunkRepository) DeleteChunks(ctx context.Context, knowledgeDomainID uint64, ids []string) error {
	if len(ids) == 0 {
		return nil
	}
	...
	for i := 0; i < len(ids); i += batchSize {
		...
		if err := r.db.WithContext(ctx).Where("knowledge_domain_id = ? AND id IN ?", knowledgeDomainID, ids[i:end]).Delete(&types.Chunk{}).Error; err != nil {
			return err
		}
	}
	return nil
...
func (r *chunkRepository) DeleteChunksByKnowledgeID(ctx context.Context, knowledgeDomainID uint64, knowledgeID string) error {
	return r.db.WithContext(ctx).Where(
		"knowledge_domain_id = ? AND knowledge_id = ?", knowledgeDomainID, knowledgeID,
...
func (r *chunkRepository) ListImageInfoByKnowledgeIDs(
	ctx context.Context, knowledgeDomainID uint64, knowledgeIDs []string,
) ([]interfaces.ChunkImageInfo, error) {
	var results []interfaces.ChunkImageInfo
	err := r.db.WithContext(ctx).
		Model(&types.Chunk{}).
		Select("knowledge_id, image_info").
		Where("knowledge_domain_id = ? AND knowledge_id IN ? AND image_info != ''", knowledgeDomainID, knowledgeIDs).
		Scan(&results).Error
	return results, err
}

// DeleteByKnowledgeList deletes all chunks for a knowledge list
func (r *chunkRepository) DeleteByKnowledgeList(ctx context.Context, knowledgeDomainID uint64, knowledgeIDs []string) error {
	return r.db.WithContext(ctx).Where(
		"knowledge_domain_id = ? AND knowledge_id in ?", knowledgeDomainID, knowledgeIDs,
	).Delete(&types.Chunk{}).Error
}

// MoveChunksByKnowledgeID updates knowledge_base_id for all chunks of a knowledge item
func (r *chunkRepository) MoveChunksByKnowledgeID(ctx context.Context, knowledgeDomainID uint64, knowledgeID string, targetKBID string) error {
	return r.db.WithContext(ctx).Model(&types.Chunk{}).
		Where("knowledge_domain_id = ? AND knowledge_id = ?", knowledgeDomainID, knowledgeID).
...
func (r *chunkRepository) DeleteChunksByTagID(ctx context.Context, knowledgeDomainID uint64, kbID string, tagID string, excludeIDs []string) ([]string, error) {
	...
	if err := r.db.WithContext(ctx).Model(&types.Chunk{}).
		Where("knowledge_domain_id = ? AND knowledge_base_id = ? AND tag_id = ?", knowledgeDomainID, kbID, tagID).
...
func (r *chunkRepository) DeleteUnindexedChunks(
	...
	var chunks []*types.Chunk
	if err := r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND knowledge_id = ? AND status = ?", knowledgeDomainID, knowledgeID, types.ChunkStatusStored).
		Find(&chunks).Error; err != nil {
		return nil, err
	}
	if len(chunks) > 0 {
...
func (r *chunkRepository) ListAllFAQChunksByKnowledgeID(
	...
	for {
		...
		if err := r.db.WithContext(ctx).
			...
			Where("knowledge_domain_id = ? AND knowledge_id = ? AND chunk_type = ?", knowledgeDomainID, knowledgeID, types.ChunkTypeFAQ).
			Offset(offset).
			Limit(batchSize).
...
func (r *chunkRepository) ListAllFAQChunksWithMetadataByKnowledgeBaseID(
	...
	for {
		...
		if err := r.db.WithContext(ctx).
			Select("id, metadata").
			Where("knowledge_domain_id = ? AND knowledge_base_id = ? AND chunk_type = ? AND status = ?",
				knowledgeDomainID, kbID, types.ChunkTypeFAQ, types.ChunkStatusIndexed).
			Offset(offset).
			Limit(batchSize).
			Find(&batchChunks).Error; err != nil {
			return nil, err
...
func (r *chunkRepository) FindFAQChunkWithDuplicateQuestion(
	...
	db := r.db.WithContext(ctx).
		Select("id, metadata").
		Where("knowledge_domain_id = ? AND knowledge_base_id = ? AND chunk_type = ? AND status = ? AND id != ?",
			knowledgeDomainID, kbID, types.ChunkTypeFAQ, types.ChunkStatusIndexed, excludeChunkID)
...
func (r *chunkRepository) ListAllFAQChunksForExport(
	...
	for {
		...
		if err := r.db.WithContext(ctx).
			...
			Where("knowledge_domain_id = ? AND knowledge_id = ? AND chunk_type = ? AND status = ?",
				knowledgeDomainID, knowledgeID, types.ChunkTypeFAQ, types.ChunkStatusIndexed).
...
func (r *chunkRepository) UpdateChunkFlagsBatch(
    ...
    SET flags = (flags | (%s)) & ~(%s),
        updated_at = %s
    WHERE knowledge_domain_id = ?
      AND knowledge_base_id = ?
      AND id IN (%s)
`, setExpr, clearExpr, nowFunc, strings.Join(inPlaceholders, ","))

	args = append(args, knowledgeDomainID, kbID)
	for _, id := range allIDs {
		args = append(args, id)
	}
...
func (r *chunkRepository) UpdateChunkFieldsByTagID(
	...
	if isEnabled != nil {
		...
		query := r.db.WithContext(ctx).
			Select("id").
			Where("knowledge_domain_id = ? AND knowledge_base_id = ? AND chunk_type = ?",
				knowledgeDomainID, kbID, types.ChunkTypeFAQ)
		if tagID != "" {
...
func (r *chunkRepository) FAQChunkDiff(
	ctx context.Context,
	srcKnowledgeDomainID uint64, srcKBID string,
	dstKnowledgeDomainID uint64, dstKBID string,
	...
	dstHashSubQuery := r.db.Model(&types.Chunk{}).
		Where("knowledge_domain_id = ? AND knowledge_base_id = ? AND chunk_type = ?", dstKnowledgeDomainID, dstKBID, types.ChunkTypeFAQ).
		Select("content_hash")

	// Find chunks in source that don't exist in destination (by content_hash)
	err = r.db.WithContext(ctx).Model(&types.Chunk{}).
		Where("knowledge_domain_id = ? AND knowledge_base_id = ? AND chunk_type = ?", srcKnowledgeDomainID, srcKBID, types.ChunkTypeFAQ).
...
func (r *chunkRepository) ListRecommendedFAQChunks(
	...
	var chunks []*types.Chunk
	query := r.db.WithContext(ctx).
		Select("id, seq_id, knowledge_id, knowledge_base_id, chunk_type, metadata, flags, updated_at").
		Where("knowledge_domain_id = ? AND chunk_type = ? AND status IN ? AND is_enabled = ? AND flags & ? != 0",
			knowledgeDomainID, types.ChunkTypeFAQ, []int{int(types.ChunkStatusIndexed), int(types.ChunkStatusDefault)}, true, int(types.ChunkFlagRecommended))
...
func (r *chunkRepository) ListRecentDocumentChunksWithQuestions(
	...
	baseQuery := r.db.WithContext(ctx).
		...
		Where("knowledge_domain_id = ? AND chunk_type = ? AND status IN ? AND is_enabled = ?",
			knowledgeDomainID, types.ChunkTypeText, []int{int(types.ChunkStatusIndexed), int(types.ChunkStatusDefault)}, true)
