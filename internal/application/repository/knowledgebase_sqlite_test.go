...
// — which RocheKAP's UpdateKnowledgeBase repository path uses today — does
...
// VectorStore delete guard. Verifies global binding counts, the GORM auto soft-delete
...
func TestCountByVectorStoreID(t *testing.T) {
	...
	// KnowledgeDomain 1: two active rows bound to store-A, one row bound to store-B.
	...
	// KnowledgeDomain 2: one row bound to store-A (must not count for knowledgeDomain 1).
	kbCross := makeKB(kbStrPtr("store-A"))
	kbCross.KnowledgeDomainID = 2
	...
	t.Run("nil db handle uses default", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, int64(3), count, "three active KBs use the global store-A instance")
	})
	...
	t.Run("knowledge domain argument does not scope a global store", func(t *testing.T) {
