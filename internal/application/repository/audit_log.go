...
// List returns audit log rows, newest-first, applying the
...
func (r *auditLogRepository) List(
	...
	tx := r.db.WithContext(ctx).Model(&types.AuditLog{})
	if q != nil {
		if q.AfterID > 0 {
...
// dedup. The remaining filters are applied as additional WHERE clauses
// on the indexed result set.
//
...
//
// KnowledgeDomain scope is intentionally not part of this signature: retention
// is a global ops policy, not a per-knowledgeDomain choice. If we ever need
// per-knowledgeDomain retention, we'd add a separate DeleteOlderThanForKnowledgeDomain
