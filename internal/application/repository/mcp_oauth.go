...
func (r *mcpOAuthRepository) GetClient(
	ctx context.Context, knowledgeDomainID uint64, serviceID string,
) (*types.MCPOAuthClient, error) {
	var client types.MCPOAuthClient
	err := r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND service_id = ?", knowledgeDomainID, serviceID).
		First(&client).Error
	if err != nil {
...
func (r *mcpOAuthRepository) SaveClient(ctx context.Context, client *types.MCPOAuthClient) error {
	...
	return r.db.WithContext(ctx).
		Clauses(clause.OnConflict{
			Columns:   []clause.Column{{Name: "knowledge_domain_id"}, {Name: "service_id"}},
			DoUpdates: clause.AssignmentColumns([]string{"client_id", "client_secret", "redirect_uri", "updated_at"}),
		}).
		Create(client).Error
}

func (r *mcpOAuthRepository) DeleteClient(ctx context.Context, knowledgeDomainID uint64, serviceID string) error {
	return r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND service_id = ?", knowledgeDomainID, serviceID).
...
func (r *mcpOAuthRepository) GetToken(
	...
	return r.GetTokenForPrincipal(ctx, knowledgeDomainID, types.Principal{
...
func (r *mcpOAuthRepository) GetTokenForPrincipal(
	...
	err := r.db.WithContext(ctx).
		Where(
			"knowledge_domain_id = ? AND principal_type = ? AND principal_id = ? AND service_id = ?",
			knowledgeDomainID, principal.Type, principal.ID, serviceID,
		).
		First(&token).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (r *mcpOAuthRepository) DeleteToken(
	...
	return r.DeleteTokenForPrincipal(ctx, knowledgeDomainID, types.Principal{
		Type: types.PrincipalWebUser,
		ID:   userID,
	}, serviceID)
}
