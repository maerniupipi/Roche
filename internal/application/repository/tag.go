...
func (r *knowledgeTagRepository) GetByID(ctx context.Context, knowledgeDomainID uint64, id string) (*types.KnowledgeTag, error) {
...
func (r *knowledgeTagRepository) GetByIDs(ctx context.Context, knowledgeDomainID uint64, ids []string) ([]*types.KnowledgeTag, error) {
	if len(ids) == 0 {
...
func (r *knowledgeTagRepository) GetByIDsOnly(ctx context.Context, ids []string) ([]*types.KnowledgeTag, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	var tags []*types.KnowledgeTag
...
func (r *knowledgeTagRepository) GetBySeqID(ctx context.Context, knowledgeDomainID uint64, seqID int64) (*types.KnowledgeTag, error) {
	var tag types.KnowledgeTag
	if err := r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND seq_id = ?", knowledgeDomainID, seqID).
		First(&tag).Error; err != nil {
		return nil, err
	}
	return &tag, nil
...
func (r *knowledgeTagRepository) GetBySeqIDs(ctx context.Context, knowledgeDomainID uint64, seqIDs []int64) ([]*types.KnowledgeTag, error) {
	if len(seqIDs) == 0 {
		return []*types.KnowledgeTag{}, nil
	}
	var tags []*types.KnowledgeTag
...
func (r *knowledgeTagRepository) GetByName(ctx context.Context, knowledgeDomainID uint64, kbID string, name string) (*types.KnowledgeTag, error) {
	var tag types.KnowledgeTag
	if err := r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND knowledge_base_id = ? AND name = ?", knowledgeDomainID, kbID, name).
		First(&tag).Error; err != nil {
		return nil, err
	}
	return &tag, nil
...
func (r *knowledgeTagRepository) Delete(ctx context.Context, knowledgeDomainID uint64, id string) error {
...
func (r *knowledgeTagRepository) CountReferences(
	...
	if err = r.db.WithContext(ctx).
		...
		Joins("JOIN knowledges AS k ON ktr.knowledge_id = k.id AND k.deleted_at IS NULL AND k.knowledge_domain_id = ? AND k.knowledge_base_id = ?", knowledgeDomainID, kbID).
...
func (r *knowledgeTagRepository) BatchCountReferences(
	...
	if err := r.db.WithContext(ctx).
		...
		Where("knowledge_domain_id = ? AND knowledge_base_id = ? AND tag_id IN (?)", knowledgeDomainID, kbID, tagIDs).
		Group("tag_id").
...
func (r *knowledgeTagRepository) DeleteUnusedTags(ctx context.Context, knowledgeDomainID uint64, kbID string) (int64, error) {
	// Delete tags that have no references in both knowledges and chunks tables (excluding soft-deleted records)
	result := r.db.WithContext(ctx).
		Where("knowledge_domain_id = ? AND knowledge_base_id = ?", knowledgeDomainID, kbID).
		Where("id NOT IN (SELECT DISTINCT ktr.tag_id FROM knowledge_tag_relations ktr JOIN knowledges k ON ktr.knowledge_id = k.id AND k.deleted_at IS NULL AND k.knowledge_domain_id = ? AND k.knowledge_base_id = ?)", knowledgeDomainID, kbID).
		Where("id NOT IN (SELECT DISTINCT tag_id FROM chunks WHERE knowledge_domain_id = ? AND knowledge_base_id = ? AND tag_id IS NOT NULL AND tag_id != '' AND deleted_at IS NULL)", knowledgeDomainID, kbID).
