...
func (r *MCPToolApprovalRepository) ListByService(ctx context.Context, knowledgeDomainID uint64, serviceID string) ([]*types.MCPToolApproval, error) {
...
func (r *MCPToolApprovalRepository) IsRequired(ctx context.Context, knowledgeDomainID uint64, serviceID, toolName string) (bool, error) {
	var row types.MCPToolApproval
	err := r.db.WithContext(ctx).
		Select("require_approval").
		Where("knowledge_domain_id = ? AND service_id = ? AND tool_name = ?", knowledgeDomainID, serviceID, toolName).
		First(&row).Error
...
// Uses ON CONFLICT against the (knowledge_domain_id, service_id, tool_name) unique index
