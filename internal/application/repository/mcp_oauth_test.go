...
func TestMCPOAuthRepositoryTokenForPrincipalIsolated(t *testing.T) {
	...
	otherWebPrincipal := types.Principal{Type: types.PrincipalWebUser, ID: "u2"}
	...
	require.NoError(t, repo.SaveTokenForPrincipal(ctx, &types.MCPOAuthToken{
		KnowledgeDomainID: 7,
		UserID:            otherWebPrincipal.StorageID(),
		PrincipalType:     otherWebPrincipal.Type,
		PrincipalID:       otherWebPrincipal.ID,
		ServiceID:         "svc1",
	...
	apiToken, err := repo.GetTokenForPrincipal(ctx, 7, otherWebPrincipal, "svc1")
	require.NoError(t, err)
