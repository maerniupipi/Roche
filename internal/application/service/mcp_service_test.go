...
func (r *fakeMCPRepo) ListEnabled(ctx context.Context, knowledgeDomainID uint64) ([]*types.MCPService, error) {
	return r.List(ctx, knowledgeDomainID)
}
...
func TestUpdateMCPService_CustomHeadersPreserveOnNil(t *testing.T) {
	...
	svc, repo := newTestService()
	id := seedService(t, repo, "stored-api", "stored-token")
	repo.store[id].AuthConfig.CustomHeaders = map[string]string{"X-KnowledgeDomain": "acme"}
	...
	got := repo.store[id]
	assert.Equal(t, "acme", got.AuthConfig.CustomHeaders["X-KnowledgeDomain"],
