...
func (s *knowledgeBaseService) processSearchResults(ctx context.Context,
	...
	// Batch fetch knowledge data for the authorized search targets.
	logger.Infof(ctx, "Fetching knowledge data for %d IDs", len(index.knowledgeIDs))
	knowledgeMap, err := s.fetchKnowledgeData(ctx, knowledgeDomainID, index.knowledgeIDs)
	if err != nil {
		return nil, err
	}

	// Batch fetch chunks for the authorized search targets.
	logger.Infof(ctx, "Fetching chunk data for %d IDs", len(index.chunkIDs))
	allChunks, err := s.listChunksByID(ctx, knowledgeDomainID, index.chunkIDs)
	if err != nil {
	...
	if !skipEnrichment {
		...
		if len(additionalChunkIDs) > 0 {
			...
			additionalChunks, err := s.listChunksByID(ctx, knowledgeDomainID, additionalChunkIDs)
			if err != nil {
				...
				if s.hasImageChunks(allChunks) {
					...
					if len(parentIDs) > 0 {
						...
						parentChunks, err := s.listChunksByID(ctx, knowledgeDomainID, parentIDs)
						if err != nil {
	...
	searchutil.EnrichSearchResultsImageInfo(ctx, s.chunkRepo, knowledgeDomainID, searchResults)
