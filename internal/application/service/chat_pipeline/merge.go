...
func (p *PluginMerge) resolveParentChunks(
	...
	parentChunks, err := p.chunkRepo.ListChunksByID(ctx, knowledgeDomainID, ids)
	if err != nil {
	...
	if hasImageResults {
		...
		if len(grandparentIDs) > 0 {
			gpChunks, err := p.chunkRepo.ListChunksByID(ctx, knowledgeDomainID, grandparentIDs)
			if err != nil {
	...
	if len(textChildIDs) > 0 {
		scopedImageInfo = searchutil.CollectImageInfoByChunkIDs(ctx, p.chunkRepo, knowledgeDomainID, textChildIDs)
	}
