...
func (p *PluginSearch) OnEvent(ctx context.Context,
	...
	pipelineInfo(ctx, "Search", "input", map[string]interface{}{
		...
		"knowledge_domain_id": chatManage.KnowledgeDomainID,
...
func (p *PluginSearch) searchByTargets(
	...
	// Resolve actual model identities (name + endpoint). Targets are still split
	// by knowledge domain because HybridSearch authorizes its scope from ctx.
	...
	for _, t := range chatManage.SearchTargets {
		...
		if knowledgeDomainID == 0 {
			pipelineWarn(ctx, "Search", "missing_knowledge_domain", map[string]interface{}{
				"kb_id": t.KnowledgeBaseID,
			})
			continue
		}
		key := searchGroupKey{
			modelKey:          modelKeyMap[t.KnowledgeBaseID], // empty string if unresolved
	...
	for groupKey, targets := range groups {
		wg.Add(1)
		go func(groupKey searchGroupKey, targets []*types.SearchTarget) {
			defer wg.Done()
			...
			if groupKey.modelKey != "" {
				emb, err := p.knowledgeBaseService.GetQueryEmbedding(searchCtx, targets[0].KnowledgeBaseID, queryText)
				if err != nil {
					pipelineWarn(ctx, "Search", "group_embed_error", map[string]interface{}{
						"model_key":           groupKey.modelKey,
						"knowledge_domain_id": groupKey.knowledgeDomainID,
			...
			if len(fullKBIDs) > 0 {
				...
				go func() {
					...
					res, err := p.knowledgeBaseService.HybridSearch(searchCtx, fullKBIDs[0], params)
					if err != nil {
			...
			for _, target := range knowledgeTargets {
				...
				go func(t *types.SearchTarget) {
					defer innerWg.Done()
					p.searchSingleTarget(searchCtx, chatManage, t, queryText, queryEmbedding, &mu, &results)
				}(target)
			}
...
func (p *PluginSearch) searchSingleTarget(
	...
	if t.Type == types.SearchTargetTypeKnowledge && !t.DisableDirectLoad {
		directResults, skippedIDs := p.tryDirectChunkLoading(ctx, chatManage.KnowledgeDomainID, t.KnowledgeIDs)
...
func (p *PluginSearch) tryDirectChunkLoading(ctx context.Context, knowledgeDomainID uint64, knowledgeIDs []string) ([]*types.SearchResult, []string) {
	...
	if len(uniqueKIDs) > 0 {
		knowledges, err := p.knowledgeService.GetKnowledgeBatch(ctx, knowledgeDomainID, uniqueKIDs)
		if err != nil {
	...
	searchutil.EnrichSearchResultsImageInfo(ctx, p.chunkService.GetRepository(), knowledgeDomainID, results)
...
func (p *PluginSearch) searchWebIfEnabled(ctx context.Context, chatManage *types.ChatManage) []*types.SearchResult {
	if !chatManage.WebSearchEnabled || p.webSearchService == nil || p.knowledgeDomainService == nil {
		return nil
	}
	knowledgeDomain, _ := types.KnowledgeDomainInfoFromContext(ctx)
	...
	if knowledgeDomain != nil {
		webConfig = types.EffectiveWebSearchConfig(knowledgeDomain.WebSearchConfig)
	}
