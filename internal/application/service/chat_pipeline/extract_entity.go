...
type PluginExtractEntity struct {
	...
	knowledgeService  interfaces.KnowledgeService // For document-to-KB resolution
...
func (p *PluginExtractEntity) OnEvent(ctx context.Context,
	...
	if len(chatManage.KnowledgeIDs) > 0 {
		knowledges, err := p.knowledgeService.GetKnowledgeBatch(ctx, chatManage.KnowledgeDomainID, chatManage.KnowledgeIDs)
		if err != nil {
