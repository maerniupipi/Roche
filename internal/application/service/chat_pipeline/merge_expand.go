...
func (p *PluginMerge) expandShortContextWithNeighbors(
	...
	if knowledgeDomainID == 0 && chatManage != nil {
		knowledgeDomainID = chatManage.KnowledgeDomainID
	}
	if knowledgeDomainID == 0 {
		pipelineWarn(ctx, "Merge", "expand_skip", map[string]interface{}{
			"reason": "missing_knowledge_domain",
		})
		return results
	}
	...
	chunks, err := p.chunkRepo.ListChunksByID(ctx, knowledgeDomainID, baseIDs)
	if err != nil {
	...
	if len(neighborIDsSet) > 0 {
		...
		neighbors, err := p.chunkRepo.ListChunksByID(ctx, knowledgeDomainID, neighborIDs)
		if err != nil {
	...
	for _, target := range targets {
		res := target.result
		p.fetchChunksIfMissing(ctx, knowledgeDomainID, chunkMap, res.ID)
		...
		p.fetchChunksIfMissing(ctx, knowledgeDomainID, chunkMap, prevCursor, nextCursor)
		...
		for {
			...
			expanded := false
			if prevCursor != "" {
				p.fetchChunksIfMissing(ctx, knowledgeDomainID, chunkMap, prevCursor)
			...
			if nextCursor != "" {
				p.fetchChunksIfMissing(ctx, knowledgeDomainID, chunkMap, nextCursor)
...
func (p *PluginMerge) fetchChunksIfMissing(
	...
	chunks, err := p.chunkRepo.ListChunksByID(ctx, knowledgeDomainID, missing)
	if err != nil {
