...
func (p *PluginSearchEntity) OnEvent(ctx context.Context,
	...
	chunks, err := p.chunkRepo.ListChunksByID(ctx, types.MustKnowledgeDomainIDFromContext(ctx), chunkIDs)
	if err != nil {
	...
	knowledges, err := p.knowledgeRepo.GetKnowledgeBatch(
		ctx,
		types.MustKnowledgeDomainIDFromContext(ctx),
		knowledgeIDs,
	)
	if err != nil {
	...
	searchutil.EnrichSearchResultsImageInfo(ctx, p.chunkRepo, types.MustKnowledgeDomainIDFromContext(ctx), entityResults)
