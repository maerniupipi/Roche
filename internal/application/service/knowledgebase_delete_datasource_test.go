...
func TestDeleteDataSourcesForKnowledgeBase(t *testing.T) {
	...
	kbRepo.rows[kbID] = &types.KnowledgeBase{ID: kbID, KnowledgeDomainID: 1, Name: "test"}
	...
	svc.deleteDataSourcesForKnowledgeBase(ctxWithKnowledgeDomain(1), kbID)
...
func TestDeleteKnowledgeBaseCleansUpDataSources(t *testing.T) {
	...
	ctx := ctxWithKnowledgeDomainStorage(1, "local")
...
func TestDeleteKnowledgeBaseContinuesWhenDataSourceCleanupFails(t *testing.T) {
	...
	err := svc.DeleteKnowledgeBase(ctxWithKnowledgeDomainStorage(1, "local"), kbID)
	require.NoError(t, err)
	assert.Equal(t, kbID, kbRepo.deletedID)
}
