...
type knowledgeService struct {
	...
	// businessAudit records critical knowledge document lifecycle
...
//
// source is a free-form tag (e.g. "question_batch[3]" or "summary")
...
// graph) call this on entry so their actual processing time shows
...
// getParserEngineOverridesFromContext returns parser engine overrides from knowledgeDomain in context (e.g. MinerU endpoint, API key).
...
func (s *knowledgeService) getParserEngineOverridesFromContext(ctx context.Context) map[string]string {
	if v := ctx.Value(types.KnowledgeDomainInfoContextKey); v != nil {
		if knowledgeDomain, ok := v.(*types.KnowledgeDomain); ok && knowledgeDomain != nil {
			return knowledgeDomain.ParserEngineConfig.ToOverridesMap()
		}
	}
	return nil
}
...
func (s *knowledgeService) isKnowledgeDeleting(ctx context.Context, knowledgeDomainID uint64, knowledgeID string) bool {
...
func (s *knowledgeService) isKnowledgeAborted(
	ctx context.Context, knowledgeDomainID uint64, knowledgeID string,
) (bool, string) {
	knowledge, err := s.repo.GetKnowledgeByID(ctx, knowledgeDomainID, knowledgeID)
	if err != nil {
...
// (either at the KB level or via the knowledgeDomain default).
//
// 内部版兜底语义：当 KB 与知识域都未配置 storage provider 时，如果服务实例持有
...
// 仅当 KB/知识域/全局三处都拿不到任何可用 FileService 时才报错。
func (s *knowledgeService) checkStorageEngineConfigured(ctx context.Context, kb *types.KnowledgeBase) error {
	provider := kb.GetStorageProvider()
	if provider == "" {
		knowledgeDomain, _ := ctx.Value(types.KnowledgeDomainInfoContextKey).(*types.KnowledgeDomain)
		if knowledgeDomain != nil && knowledgeDomain.StorageEngineConfig != nil {
	...
	if s != nil && s.fileSvc != nil {
		logger.Warnf(ctx,
			"[storage] checkStorageEngineConfigured: no KB/knowledgeDomain provider, fallback to global fileSvc (kb=%s)",
			kbIDOrEmpty(kb))
		return nil
	}
...
func (s *knowledgeService) GetKnowledgeByID(ctx context.Context, id string) (*types.Knowledge, error) {
	...
	knowledge, err := s.repo.GetKnowledgeByID(ctx, knowledgeDomainID, id)
	if err != nil {
...
// GetKnowledgeByIDOnly retrieves knowledge by ID without knowledgeDomain filter (for permission resolution).
...
// legacy ownership-guard compatibility. Current enterprise authorization
// resolves effective KB permissions through the access service.
// Both fetches are knowledgeDomain-scoped (GetKnowledgeByID reads knowledgeDomain from
// ctx; GetKnowledgeBaseByID is then constrained to the same knowledgeDomain by
// the KB service), so a cross-knowledgeDomain id surfaces as the underlying
...
func (s *knowledgeService) GetOwningKBCreatorID(ctx context.Context, knowledgeID string) (string, error) {
	// Resolve via the repository directly: the compatibility lookup only needs the
	...
	// minimal and knowledgeDomain-scoped.
	knowledgeDomainID, ok := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64)
	if !ok {
		return "", werrors.NewUnauthorizedError("KnowledgeDomain ID not found in context")
	}
	knowledge, err := s.repo.GetKnowledgeByID(ctx, knowledgeDomainID, knowledgeID)
	if err != nil {
		return "", err
...
func (s *knowledgeService) ListKnowledgeByKnowledgeBaseID(ctx context.Context,
	...
	return s.repo.ListKnowledgeByKnowledgeBaseID(ctx, ctx.Value(types.KnowledgeDomainIDContextKey).(uint64), kbID)
}

// ListKnowledgeFolders returns the visible folder hierarchy for a KB.
func (s *knowledgeService) ListKnowledgeFolders(
	ctx context.Context,
	kbID string,
	allowedKnowledgeIDs []string,
) ([]*types.KnowledgeFolder, error) {
	return s.repo.ListKnowledgeFolders(
		ctx,
		ctx.Value(types.KnowledgeDomainIDContextKey).(uint64),
		kbID,
		allowedKnowledgeIDs,
	)
}

func (s *knowledgeService) DeleteKnowledgeFolder(
	ctx context.Context,
	kbID string,
	folderID string,
) error {
	knowledgeDomainID, ok := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64)
	if !ok || knowledgeDomainID == 0 {
		return werrors.NewUnauthorizedError("knowledge domain context missing")
	}

	root, err := s.repo.GetKnowledgeFolderByID(ctx, knowledgeDomainID, folderID)
	if err != nil {
		return err
	}
	if root.KnowledgeBaseID != kbID {
		return repository.ErrKnowledgeFolderNotFound
	}
	folders, err := s.repo.ListKnowledgeFolders(ctx, knowledgeDomainID, kbID, nil)
	if err != nil {
		return err
	}
	...
	knowledges, err := s.repo.ListKnowledgeByKnowledgeBaseID(ctx, knowledgeDomainID, kbID)
	if err != nil {
		return err
	}
	knowledgeIDs := make([]string, 0)
	for _, knowledge := range knowledges {
		if knowledge == nil || knowledge.FolderID == nil {
			continue
		}
		if _, ok := folderSet[*knowledge.FolderID]; ok {
			knowledgeIDs = append(knowledgeIDs, knowledge.ID)
		}
	}
	if len(knowledgeIDs) > 0 {
		knowledgeDomain, loadErr := s.knowledgeDomainRepo.GetKnowledgeDomainByID(ctx, knowledgeDomainID)
		if loadErr != nil {
			return loadErr
		}
		deleteCtx := context.WithValue(ctx, types.KnowledgeDomainInfoContextKey, knowledgeDomain)
		if err := s.DeleteKnowledgeList(deleteCtx, knowledgeIDs); err != nil {
			return err
		}
	}
	return s.repo.DeleteKnowledgeFolder(ctx, knowledgeDomainID, kbID, folderID)
}
...
func (s *knowledgeService) ListPagedKnowledgeByKnowledgeBaseID(ctx context.Context,
	...
	knowledges, total, err := s.repo.ListPagedKnowledgeByKnowledgeBaseID(ctx,
		ctx.Value(types.KnowledgeDomainIDContextKey).(uint64), kbID, page, filter)
	if err != nil {
		return nil, err
	}
...
func (s *knowledgeService) UpdateKnowledge(ctx context.Context, knowledge *types.Knowledge) error {
	record, err := s.repo.GetKnowledgeByID(ctx, ctx.Value(types.KnowledgeDomainIDContextKey).(uint64), knowledge.ID)
	if err != nil {
	...
	// Audit: knowledge document modification
	...
	s.businessAudit.RecordKnowledgeUpdated(ctx,
		knowledge.ID, record.Title, record.KnowledgeBaseID,
		changedFields, nil, nil)
...
func (s *knowledgeService) GetKnowledgeBatch(ctx context.Context,
	knowledgeDomainID uint64, ids []string,
) ([]*types.Knowledge, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	return s.repo.GetKnowledgeBatch(ctx, knowledgeDomainID, ids)
}
...
func (s *knowledgeService) ListKnowledgeIDsByTagIDs(
	...
	return s.repo.ListIDsByTagIDs(ctx, knowledgeDomainID, kbID, tagIDs)
}
...
func (s *knowledgeService) validateKnowledgeTagIDs(
	...
	tags, err := s.tagRepo.GetByIDs(ctx, knowledgeDomainID, unique)
	if err != nil {
		return err
	}
...
func (s *knowledgeService) setAndAttachKnowledgeTags(
	...
	if err := s.validateKnowledgeTagIDs(ctx, knowledgeDomainID, kbID, tagIDs); err != nil {
		return err
	}
	if len(tagIDs) > 0 {
...
func (s *knowledgeService) UpdateKnowledgeTag(ctx context.Context, knowledgeID string, tagIDs []string) error {
	...
	if err := s.validateKnowledgeTagIDs(ctx, knowledgeDomainID, knowledge.KnowledgeBaseID, tagIDs); err != nil {
		return err
	}
...
func (s *knowledgeService) UpdateKnowledgeTagBatch(ctx context.Context, authorizedKBID string, updates map[string][]string) error {
	...
	knowledgeDomainIDVal := ctx.Value(types.KnowledgeDomainIDContextKey)
	if knowledgeDomainIDVal == nil {
		return werrors.NewUnauthorizedError("knowledgeDomain ID not found in context")
	}
	knowledgeDomainID, ok := knowledgeDomainIDVal.(uint64)
	if !ok {
		return werrors.NewUnauthorizedError("invalid knowledgeDomain ID in context")
	}
	...
	knowledgeList, err := s.repo.GetKnowledgeBatch(ctx, knowledgeDomainID, knowledgeIDs)
	if err != nil {
		return err
	}
	...
	if len(tagIDSet) > 0 {
		...
		tags, err := s.tagRepo.GetByIDs(ctx, knowledgeDomainID, tagIDList)
		if err != nil {
			return err
		}
		for _, tag := range tags {
...
// SearchKnowledge searches knowledge items by keyword across readable knowledge bases.
...
func (s *knowledgeService) SearchKnowledge(ctx context.Context, keyword string, offset, limit int, fileTypes []string) ([]*types.Knowledge, bool, int64, error) {
	knowledgeDomainID, ok := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64)
	if !ok {
		return nil, false, 0, werrors.NewUnauthorizedError("KnowledgeDomain ID not found in context")
	}

	scopes := make([]types.KnowledgeSearchScope, 0)

	// Own knowledgeDomain: document-type knowledge bases
	ownKBs, err := s.kbService.ListKnowledgeBases(ctx)
	if err == nil {
		for _, kb := range ownKBs {
			if kb != nil && kb.Type == types.KnowledgeBaseTypeDocument {
				scopes = append(scopes, types.KnowledgeSearchScope{KnowledgeDomainID: knowledgeDomainID, KBID: kb.ID})
			}
		}
	}
...
// SearchKnowledgeForScopes searches knowledge within explicit knowledgeDomain/KB scopes.
