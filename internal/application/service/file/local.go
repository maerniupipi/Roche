...
// externalURL is the externally-reachable base URL (e.g. "https://rochekap.example.com");
...
// The file is stored in a directory structure: baseDir/knowledgeDomainID/knowledgeID/filename
// Returns the full file path or an error if saving fails
func (s *localFileService) SaveFile(ctx context.Context,
	file *multipart.FileHeader, knowledgeDomainID uint64, knowledgeID string,
) (string, error) {
	logger.Info(ctx, "Starting to save file locally")
	logger.Infof(ctx, "File information: name=%s, size=%d, knowledgeDomain ID=%d, knowledge ID=%s",
		file.Filename, file.Size, knowledgeDomainID, knowledgeID)

	// Create storage directory with knowledgeDomain and knowledge ID
	dir := filepath.Join(s.baseDir, fmt.Sprintf("%d", knowledgeDomainID), knowledgeID)
...
// The destination uses the same layout as SaveFile (baseDir/{knowledgeDomainID}/{knowledgeID}/{unique}{ext}),
...
func (s *localFileService) CopyFile(ctx context.Context,
	srcPath string, knowledgeDomainID uint64, knowledgeID string,
) (string, error) {
...
func (s *localFileService) SaveBytes(ctx context.Context, data []byte, knowledgeDomainID uint64, fileName string, temp bool) (string, error) {
	logger.Infof(ctx, "Saving bytes data: fileName=%s, size=%d, knowledgeDomainID=%d, temp=%v", fileName, len(data), knowledgeDomainID, temp)
	...
	// Create storage directory with knowledgeDomain ID
	dir := filepath.Join(s.baseDir, fmt.Sprintf("%d", knowledgeDomainID), "exports")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		logger.Errorf(ctx, "Failed to create directory: %v", err)
		return "", fmt.Errorf("failed to create directory: %w", err)
	}
...
func (s *localFileService) GetFileURL(ctx context.Context, filePath string) (string, error) {
	...
	if s.externalURL != "" {
		// KnowledgeDomain ID is parsed from the storage path, which encodes the
		// resource owner's knowledgeDomain (not the caller's). The verifier on
		// /api/v1/files/presigned uses this ID to look up the owning
		// knowledgeDomain's StorageEngineConfig.
		knowledgeDomainID := secutils.ParseKnowledgeDomainIDFromStoragePath(normalized)
		presignedURL, err := secutils.SignFileURL(s.externalURL, normalized, knowledgeDomainID, 0)
		if err != nil {
...
// - provider scheme: "local://knowledgeDomain/.." → baseDir/knowledgeDomain/..
// - absolute path: "/data/files/knowledgeDomain/.."
// - path under base dir: "knowledgeDomain/.."
// - legacy relative with base prefix: "data/files/knowledgeDomain/.."
