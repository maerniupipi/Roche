...
func (s *ks3FileService) SaveFile(ctx context.Context, file *multipart.FileHeader, knowledgeDomainID uint64, knowledgeID string) (string, error) {
	ext := filepath.Ext(file.Filename)
	objectKey := joinKS3Key(s.pathPrefix, fmt.Sprintf("%d", knowledgeDomainID), knowledgeID, uuid.New().String()+ext)
...
func (s *ks3FileService) SaveBytes(ctx context.Context, data []byte, knowledgeDomainID uint64, fileName string, temp bool) (string, error) {
	safeName, err := utils.SafeFileName(fileName)
	if err != nil {
		return "", fmt.Errorf("invalid file name: %w", err)
	}
	ext := filepath.Ext(safeName)
	objectKey := joinKS3Key(s.pathPrefix, fmt.Sprintf("%d", knowledgeDomainID), "exports", uuid.New().String()+ext)
...
func (s *ks3FileService) CopyFile(ctx context.Context,
	...
	destKey := joinKS3Key(s.pathPrefix, fmt.Sprintf("%d", knowledgeDomainID), knowledgeID, uuid.New().String()+ext)
