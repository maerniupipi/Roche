...
// It generates a unique name for the file and organizes it by knowledgeDomain and knowledge ID
func (s *cosFileService) SaveFile(ctx context.Context,
	...
	objectName := fmt.Sprintf("%s/%d/%s/%s%s", s.cosPathPrefix, knowledgeDomainID, knowledgeID, uuid.New().String(), ext)
	src, err := file.Open()
	if err != nil {
...
func (s *cosFileService) CopyFile(ctx context.Context,
	...
	destKey := fmt.Sprintf("%s/%d/%s/%s%s", s.cosPathPrefix, knowledgeDomainID, knowledgeID, uuid.New().String(), ext)
...
func (s *cosFileService) SaveBytes(ctx context.Context, data []byte, knowledgeDomainID uint64, fileName string, temp bool) (string, error) {
	...
	if temp && s.tempClient != nil {
		objectName := fmt.Sprintf("exports/%d/%s%s", knowledgeDomainID, uuid.New().String(), ext)
	...
	// 写入主桶
	objectName := fmt.Sprintf("%s/%d/exports/%s%s", s.cosPathPrefix, knowledgeDomainID, uuid.New().String(), ext)
