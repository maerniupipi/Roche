...
func (s *ossFileService) SaveFile(ctx context.Context,
	...
	objectName := fmt.Sprintf("%s%d/%s/%s%s", s.pathPrefix, knowledgeDomainID, knowledgeID, uuid.New().String(), ext)
...
func (s *ossFileService) SaveBytes(ctx context.Context, data []byte, knowledgeDomainID uint64, fileName string, temp bool) (string, error) {
	...
	client := s.client
	objectName := fmt.Sprintf("%s%d/exports/%s%s", s.pathPrefix, knowledgeDomainID, uuid.New().String(), ext)
