...
func (s *tosFileService) SaveFile(ctx context.Context, file *multipart.FileHeader, knowledgeDomainID uint64, knowledgeID string) (string, error) {
	ext := filepath.Ext(file.Filename)
	objectName := joinTOSObjectKey(
		s.pathPrefix,
		fmt.Sprintf("%d", knowledgeDomainID),
		knowledgeID,
		uuid.New().String()+ext,
	)
...
func (s *tosFileService) SaveBytes(ctx context.Context, data []byte, knowledgeDomainID uint64, fileName string, temp bool) (string, error) {
