...
func (s *minioFileService) SaveFile(ctx context.Context,
	...
	objectName := fmt.Sprintf("%d/%s/%s%s", knowledgeDomainID, knowledgeID, uuid.New().String(), ext)
...
func (s *minioFileService) SaveBytes(ctx context.Context, data []byte, knowledgeDomainID uint64, fileName string, temp bool) (string, error) {
	safeName, err := utils.SafeFileName(fileName)
	if err != nil {
		return "", fmt.Errorf("invalid file name: %w", err)
	}
	ext := filepath.Ext(safeName)
	objectName := fmt.Sprintf("%d/exports/%s%s", knowledgeDomainID, uuid.New().String(), ext)
