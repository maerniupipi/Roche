...
func (s *vectorStoreService) CreateStore(ctx context.Context, store *types.VectorStore) error {
	// 1. Basic validation (name and engine type).
	...
	exists, err := s.repo.ExistsByEndpointAndIndex(ctx, 0, store.EngineType, endpoint, indexName)
	if err != nil {
	...
	// 6. Persist
	logger.Infof(ctx, "Creating platform vector store: name=%s, engine=%s",
...
func (s *vectorStoreService) UpdateStore(ctx context.Context, store *types.VectorStore) error {
	...
	logger.Infof(ctx, "Updating platform vector store: id=%s", store.ID)
	return s.repo.Update(ctx, store)
}

// DeleteStore deletes a vector store by knowledgeDomain + id, after verifying that no
...
//     (knowledge_domain_id, vector_store_id). GORM auto-applies the soft-delete
...
func (s *vectorStoreService) DeleteStore(ctx context.Context, _ uint64, id string) error {
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		count, err := s.kbRepo.CountByVectorStoreID(ctx, tx, 0, id)
		if err != nil {
			return err
		}
		if count > 0 {
	...
	logger.Infof(ctx, "Deleted platform vector store: id=%s", secutils.SanitizeForLog(id))
	return nil
}
...
//  2. DB store row matching (id, knowledgeDomainID) → user-source display.
...
func (s *vectorStoreService) ResolveStoreView(
	ctx context.Context, _ uint64, storeID string,
) (types.StoreDisplay, error) {
	if storeID == "" {
		return s.defaultStoreDisplay(), nil
	}
	store, err := s.repo.GetByID(ctx, 0, storeID)
	if err != nil {
...
//
// Implementation note: the knowledgeDomain-store count is bounded by operator
// config (typically tens), so iterating the knowledgeDomain's full store list
...
func (s *vectorStoreService) BatchResolveStoreView(
	ctx context.Context, _ uint64, storeIDs []string,
	...
	if hasNonEmpty {
		dbStores, err := s.repo.List(ctx, 0)
		if err != nil {
			return nil, err
		}
...
// env-bound and user-bound KBs (e.g. "milvus" vs "qdrant") without
// branching on Source.
...
func validateConnectionAddrSSRF(engineType types.RetrieverEngineType, config types.ConnectionConfig) error {
	...
	case types.SQLiteRetrieverEngineType:
		...
		return nil
	default:
		// Fail closed. Engines without a DB-store address mapping (infinity,
		// elasticfaiss, and any future engine) must not silently
...
func (s *vectorStoreService) TestRawConnection(
	...
	//    with raw credentials; unsupported engine types cannot be used as an
	//    outbound credential oracle.
