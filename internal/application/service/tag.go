...
func (s *knowledgeTagService) ListTags(
	...
	// Use kb's knowledgeDomain ID for data access
	effectiveKnowledgeDomainID := kb.KnowledgeDomainID

	tags, total, err := s.repo.ListByKB(ctx, effectiveKnowledgeDomainID, kbID, page, keyword)
	if err != nil {
		return nil, err
	}
	...
	countsMap, err := s.repo.BatchCountReferences(ctx, effectiveKnowledgeDomainID, kbID, tagIDs)
	if err != nil {
...
func (s *knowledgeTagService) CreateTag(
	...
	existingTag, err := s.repo.GetByName(ctx, kb.KnowledgeDomainID, kbID, name)
...
func (s *knowledgeTagService) UpdateTag(
	...
	knowledgeDomainID := types.MustKnowledgeDomainIDFromContext(ctx)
	tag, err := s.repo.GetByID(ctx, knowledgeDomainID, id)
	if err != nil {
		return nil, err
	}
...
func (s *knowledgeTagService) DeleteTag(ctx context.Context, id string, force bool, contentOnly bool, excludeIDs []string) error {
	...
	kCount, cCount, err := s.repo.CountReferences(ctx, knowledgeDomainID, tag.KnowledgeBaseID, tag.ID)
	if err != nil {
		return err
	}

	// Get knowledgeDomain info for effective engines
	...
	deleteChunksAndEnqueueIndexDelete := func() error {
		...
		deletedIDs, err := s.chunkRepo.DeleteChunksByTagID(ctx, knowledgeDomainID, tag.KnowledgeBaseID, tag.ID, excludeIDs)
		if err != nil {
		...
		if len(deletedIDs) > 0 {
			s.enqueueIndexDeleteTask(ctx, knowledgeDomainID, kb.ID, kb.EmbeddingModelID, string(kb.Type), deletedIDs, knowledgeDomainInfo.GetEffectiveEngines(), kb.VectorStoreID)
		}
	...
	enqueueKnowledgeDeleteTask := func() error {
		...
		knowledgeIDs, err := s.knowledgeRepo.ListIDsByTagIDs(ctx, knowledgeDomainID, kb.ID, []string{tag.ID})
		if err != nil {
	...
	return s.repo.Delete(ctx, knowledgeDomainID, id)
}
...
func (s *knowledgeTagService) enqueueIndexDeleteTask(ctx context.Context,
	knowledgeDomainID uint64, kbID, embeddingModelID, kbType string, chunkIDs []string,
...
func (s *knowledgeTagService) ProcessIndexDelete(ctx context.Context, t *asynq.Task) error {
	...
	// The factory verifies the payload's knowledgeDomain owns the bound store, so a
	// tampered queue entry cannot reach a cross-knowledgeDomain store.
	...
	retrieveEngine, err := retriever.CreateRetrieveEngineFromPayload(
		...
		payload.KnowledgeDomainID, payload.EffectiveEngines, payload.VectorStoreID,
	)
	if errors.Is(err, retriever.ErrVectorStoreForbidden) ||
		errors.Is(err, retriever.ErrVectorStoreNotFound) {
		logger.Errorf(ctx, "Index delete task aborted: %v (knowledgeDomain=%d, kb=%s)", err, payload.KnowledgeDomainID, payload.KnowledgeBaseID)
...
func (s *knowledgeTagService) FindOrCreateTagByName(ctx context.Context, kbID string, name string) (*types.KnowledgeTag, error) {
	...
	// 先尝试查找现有标签
	tag, err := s.repo.GetByName(ctx, knowledgeDomainID, kbID, name)
	if err == nil {
		return tag, nil
	}
