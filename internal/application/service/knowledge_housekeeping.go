...
func (h *HousekeepingService) Start(ctx context.Context) error {
	...
	if !housekeepingEnabled() {
		logger.Infof(ctx, "[Housekeeping] disabled via ROCHE_KAP_HOUSEKEEPING_ENABLED=false")
		return nil
	}
...
func (h *HousekeepingService) runSweep(ctx context.Context) {
	...
	if queueSkipped > 0 {
		...
		// mean the queue is the bottleneck (raise ROCHE_KAP_ASYNQ_CONCURRENCY
...
func housekeepingEnabled() bool {
	...
	v := strings.TrimSpace(os.Getenv("ROCHE_KAP_HOUSEKEEPING_ENABLED"))
	if v == "" {
		return true
	}
