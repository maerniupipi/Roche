...
func TestValidateProcessOverrides_COSIncompleteForImage(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), types.KnowledgeDomainInfoContextKey, &types.KnowledgeDomain{
...
func TestMergeParserEngineOverrides(t *testing.T) {
	...
	// 4. Overlap priority (upload override should take priority over knowledgeDomain config)
	knowledgeDomain := map[string]string{"k1": "knowledge_domain_value", "k2": "v2"}
	upload := map[string]string{"k1": "upload_val", "k3": "v3"}
	merged = MergeParserEngineOverrides(knowledgeDomain, upload)
