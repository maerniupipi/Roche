...
func (s *KnowledgePostProcessService) Handle(ctx context.Context, task *asynq.Task) error {
	...
	// question / graph child tasks for an aborted knowledge. We
	...
	if graphChunkCount > 0 {
		...
		for i, chunk := range textChunks {
			ok, err := NewChunkExtractTask(ctx, s.taskEnqueuer, payload.KnowledgeDomainID, chunk.ID, kb.SummaryModelID,
	...
	// downstream stages (summary/question/graph) record their
