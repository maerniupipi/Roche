...
import (
	...
	werrors "roche.local/knowledge-agent-platform/internal/errors"
	"roche.local/knowledge-agent-platform/internal/infrastructure/chunker"
	...
	"roche.local/knowledge-agent-platform/internal/searchutil"
...
func (s *knowledgeService) cloneKnowledge(
	...
	targetFolderID, targetFolderPath, err := s.resolveTargetKnowledgeFolder(ctx, src, targetKB)
	if err != nil {
		return err
	}
	dst := &types.Knowledge{
		ID:                uuid.New().String(),
		KnowledgeDomainID: targetKB.KnowledgeDomainID,
	...
	if src.FilePath != "" {
		...
		newPath, copyErr := copyOwnedObject(ctx, srcSvc, dstSvc, src.FilePath, targetKB.KnowledgeDomainID, dst.ID)
		if copyErr != nil {
	...
	knowledgeDomainInfo.StorageUsed += dst.StorageSize
	if err = s.knowledgeDomainRepo.AdjustStorageUsed(ctx, knowledgeDomainInfo.ID, dst.StorageSize); err != nil {
		logger.GetLogger(ctx).WithField("error", err).Errorf("MoveKnowledge update knowledgeDomain storage used failed")
		return
	}
...
func (s *knowledgeService) processChunks(ctx context.Context,
	...
	if aborted, status := s.isKnowledgeAborted(ctx, knowledge.KnowledgeDomainID, knowledge.ID); aborted {
	...
	retrieveEngine, err := retriever.CreateRetrieveEngineForKB(
		ctx, s.retrieveEngine, s.ownership, knowledgeDomainInfo.ID, kb.VectorStoreID)
	...
	for idx, chunkData := range chunks {
		...
		// Set page metadata
		if chunkData.PageNumber != nil || chunkData.PageRange != nil {
			meta := &types.DocumentChunkMetadata{
				PageNumber: chunkData.PageNumber,
				PageRange:  chunkData.PageRange,
			}
			if err := textChunk.SetDocumentMetadata(meta); err != nil {
				logger.Warnf(ctx, "Failed to set page metadata for chunk %s: %v", textChunk.ID, err)
			}
		}
	...
	// Chunks are needed for graph extraction and summary generation
	...
	// Chunks are ALWAYS saved to DB (above) because graph extraction needs them even without vector indexing.
	...
	if kb.NeedsEmbeddingModel() && embeddingModel != nil {
		...
		if knowledgeDomainInfo.StorageQuota > 0 {
			// Re-fetch knowledgeDomain storage information
			knowledgeDomainInfo, err = s.knowledgeDomainRepo.GetKnowledgeDomainByID(ctx, knowledgeDomainInfo.ID)
			if err != nil {
				knowledge.ParseStatus = types.ParseStatusFailed
				knowledge.ErrorMessage = err.Error()
				knowledge.UpdatedAt = time.Now()
			...
			if knowledgeDomainInfo.StorageUsed+totalStorageSize > knowledgeDomainInfo.StorageQuota {
				knowledge.ParseStatus = types.ParseStatusFailed
	...
	// Update knowledgeDomain's storage usage
	knowledgeDomainInfo.StorageUsed += totalStorageSize
	if err := s.knowledgeDomainRepo.AdjustStorageUsed(ctx, knowledgeDomainInfo.ID, totalStorageSize); err != nil {
		logger.GetLogger(ctx).WithField("error", err).Errorf("processChunks update knowledgeDomain storage used failed")
	}
...
func (s *knowledgeService) getSummary(ctx context.Context,
	...
	imageInfoMap := searchutil.CollectImageInfoByChunkIDs(ctx, s.chunkRepo, knowledge.KnowledgeDomainID, chunkIDs)
...
func (s *knowledgeService) ProcessSummaryGeneration(ctx context.Context, t *asynq.Task) (retErr error) {
	...
	// Set knowledgeDomain and language context
	...
	// Get knowledge
	knowledge, err := s.repo.GetKnowledgeByID(ctx, payload.KnowledgeDomainID, payload.KnowledgeID)
	if err != nil {
	...
	// KBs without embedding-based indexing don't need summary chunks in the vector index.
	if strings.TrimSpace(summary) != "" && kb.NeedsEmbeddingModel() {
		...
		// Index summary chunk
		knowledgeDomainInfo, err := s.knowledgeDomainRepo.GetKnowledgeDomainByID(ctx, payload.KnowledgeDomainID)
		if err != nil {
			logger.Errorf(ctx, "Failed to get knowledgeDomain info: %v", err)
			summaryErr = err
			return fmt.Errorf("failed to get knowledgeDomain info: %w", err)
		}
		ctx = context.WithValue(ctx, types.KnowledgeDomainInfoContextKey, knowledgeDomainInfo)
...
func (s *knowledgeService) processQuestionGenerationForKnowledge(ctx context.Context, t *asynq.Task, payload types.QuestionGenerationPayload) (retErr error) {
	...
	knowledgeDomainInfo, err := s.knowledgeDomainRepo.GetKnowledgeDomainByID(ctx, payload.KnowledgeDomainID)
	if err != nil {
		exitStatus = "get_knowledge_domain_failed"
		logger.Errorf(ctx, "Failed to get knowledgeDomain info: %v", err)
		return fmt.Errorf("failed to get knowledgeDomain info: %w", err)
	}
	ctx = context.WithValue(ctx, types.KnowledgeDomainInfoContextKey, knowledgeDomainInfo)
	...
	imageInfoMap := searchutil.CollectImageInfoByChunkIDs(ctx, s.chunkRepo, payload.KnowledgeDomainID, textChunkIDs)
...
func (s *knowledgeService) processQuestionGenerationForChunks(ctx context.Context, t *asynq.Task, payload types.QuestionGenerationPayload) (retErr error) {
	...
	getChunk := func(id string) *types.Chunk {
		...
		c, gerr := s.chunkRepo.GetChunkByID(ctx, payload.KnowledgeDomainID, id)
		if gerr != nil {
			return nil
		}
		return c
	...
	imageInfoMap := searchutil.CollectImageInfoByChunkIDs(ctx, s.chunkRepo, payload.KnowledgeDomainID, infoIDs)
...
func (s *knowledgeService) ReparseKnowledge(
	...
	existing, err := s.repo.GetKnowledgeByID(ctx, knowledgeDomainID, knowledgeID)
	if err != nil {
	...
	if existing.FilePath != "" {
		...
		if slices.Contains([]string{"csv", "xlsx", "xls"}, getFileType(existing.FileName)) {
			NewDataTableSummaryTask(ctx, s.task, knowledgeDomainID, existing.ID, kb.SummaryModelID, kb.EmbeddingModelID)
		}
...
func (s *knowledgeService) updateChunkVector(ctx context.Context, kbID string, chunks []*types.Chunk) error {
	...
	// Initialize composite retrieve engine from knowledgeDomain configuration
	...
	retrieveEngine, err := retriever.CreateRetrieveEngineForKB(
		ctx, s.retrieveEngine, s.ownership, types.MustKnowledgeDomainIDFromContext(ctx), sourceKB.VectorStoreID)
	if err != nil {
		return err
	}
...
func (s *knowledgeService) UpdateImageInfo(
	...
	knowledgeDomainID := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64)
	chunkChildren, err := s.chunkService.ListChunkByParentID(ctx, knowledgeDomainID, chunkID)
	if err != nil {
		logger.ErrorWithFields(ctx, err, map[string]interface{}{
	...
	knowledge, err := s.repo.GetKnowledgeByID(ctx, knowledgeDomainID, knowledgeID)
	if err != nil {
		logger.Errorf(ctx, "Failed to get knowledge: %v", err)
		return err
	}
...
func (s *knowledgeService) ProcessManualUpdate(ctx context.Context, t *asynq.Task) error {
	...
	knowledgeDomainInfo, err := s.knowledgeDomainRepo.GetKnowledgeDomainByID(ctx, payload.KnowledgeDomainID)
	if err != nil {
		logger.Errorf(ctx, "ProcessManualUpdate: failed to get knowledgeDomain: %v", err)
		return nil
	}
	ctx = context.WithValue(ctx, types.KnowledgeDomainInfoContextKey, knowledgeDomainInfo)
	...
	// this run's summary subspans onto the previous attempt's trace.
	attempt := 0
...
func (s *knowledgeService) ProcessDocument(ctx context.Context, t *asynq.Task) error {
	...
	knowledgeDomainInfo, err := s.knowledgeDomainRepo.GetKnowledgeDomainByID(ctx, payload.KnowledgeDomainID)
	if err != nil {
		logger.Errorf(ctx, "failed to get knowledgeDomain: %v", err)
		return nil
	}
	ctx = context.WithValue(ctx, types.KnowledgeDomainInfoContextKey, knowledgeDomainInfo)
	...
	if payload.FileURL != "" {
		...
		filePath, err := fileSvc.SaveBytes(ctx, contentBytes, payload.KnowledgeDomainID, resolvedFileName, true)
		if err != nil {
			if isLastRetry {
				knowledge.ParseStatus = "failed"
				knowledge.ErrorMessage = err.Error()
	...
	if s.imageResolver != nil && convertResult != nil {
		fileSvc := s.resolveFileService(ctx, kb)
		knowledgeDomainID, _ := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64)
		updatedMarkdown, images, resolveErr := s.imageResolver.ResolveAndStore(ctx, convertResult, fileSvc, knowledgeDomainID)
		if resolveErr != nil {
		...
		updatedContent, remoteImages, remoteErr := s.imageResolver.ResolveRemoteImages(ctx, convertResult.MarkdownContent, fileSvc, knowledgeDomainID)
		if remoteErr != nil {
	...
	if eff.ChunkingConfig.EnableParentChild {
		...
		// Parse page mapping (if available)
		var pageMapping []types.PagePosition
		if pageMappingStr, ok := convertResult.Metadata["page_mapping"]; ok && pageMappingStr != "" {
			json.Unmarshal([]byte(pageMappingStr), &pageMapping)
		}
		...
		for i, c := range splitChunks {
			...
			if len(pageMapping) > 0 {
				calculatePageForMapping(&chunks[i], pageMapping)
			} else if totalPages, ok := convertResult.Metadata["pages"]; ok {
				pages, _ := strconv.Atoi(totalPages)
				if pages > 0 {
					estimatePageByPosition(&chunks[i], pages, len(splitChunks))
				}
			}
		}
...
func (s *knowledgeService) convert(
	...
	knowledgeDomainOverrides := s.getParserEngineOverridesFromContext(ctx)
	...
	mergedOverrides := MergeParserEngineOverrides(knowledgeDomainOverrides, uploadOverrides)
...
// calculatePageForMapping calculates page number based on precise mapping
func calculatePageForMapping(chunk *types.ParsedChunk, pageMapping []types.PagePosition) {
	var bestMatch *types.PagePosition
	maxOverlap := 0

	for i := range pageMapping {
		page := &pageMapping[i]

		// Calculate overlap
		overlapStart := max(chunk.Start, page.StartPos)
		overlapEnd := min(chunk.End, page.EndPos)
	...
	if bestMatch != nil {
		chunk.PageNumber = &bestMatch.PageNumber

		// Handle cross-page situation
		if maxOverlap < (chunk.End - chunk.Start) {
			rangePages := []string{fmt.Sprintf("%d", bestMatch.PageNumber)}

			// Check if spans across other pages
			for _, page := range pageMapping {
				if page.PageNumber == bestMatch.PageNumber {
					continue
				}
				if chunk.Start < page.EndPos && chunk.End > page.StartPos {
					rangePages = append(rangePages, fmt.Sprintf("%d", page.PageNumber))
				}
			}

			if len(rangePages) > 1 {
				rangeStr := strings.Join(rangePages, "-")
				chunk.PageRange = &rangeStr
			}
		}
	}
}

// estimatePageByPosition estimates page number based on position (fallback)
func estimatePageByPosition(chunk *types.ParsedChunk, totalPages, totalChunks int) {
	if totalChunks == 0 {
		return
	}

	// Estimate page number based on chunk sequence
	estimatedPage := (chunk.Seq * totalPages / totalChunks) + 1
	if estimatedPage < 1 {
		estimatedPage = 1
	}
	if estimatedPage > totalPages {
		estimatedPage = totalPages
	}

	chunk.PageNumber = &estimatedPage
}
