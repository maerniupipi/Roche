...
func (s *knowledgeService) CreateKnowledgeFromFile(ctx context.Context,
	kbID string, file *multipart.FileHeader, metadata map[string]string, enableMultimodel *bool, customFileName string, folderPath string, tagIDs []string, channel string,
	...
	fileName, folderPath, err := normalizeUploadedFilePlacement(fileName, folderPath)
	if err != nil {
		return nil, err
	}
	...
	knowledgeDomainInfo, err := s.resolveKnowledgeDomainForKB(ctx, kb)
	if err != nil {
		logger.Errorf(ctx, "Failed to load knowledge domain for knowledge base %s: %v", kbID, err)
		return nil, err
	}
	knowledgeDomainID := knowledgeDomainInfo.ID

	// Check if file already exists
	folder, err := s.repo.ResolveKnowledgeFolderPath(ctx, knowledgeDomainID, kbID, folderPath)
	if err != nil {
		logger.Errorf(ctx, "Failed to resolve knowledge folder %q: %v", folderPath, err)
		return nil, err
	}
	folderID := ""
	if folder != nil {
	...
	logger.Infof(ctx, "Checking if file exists, knowledgeDomain ID: %d", knowledgeDomainID)
	exists, existingKnowledge, err := s.repo.CheckKnowledgeExists(ctx, knowledgeDomainID, kbID, &types.KnowledgeCheckParams{
		Type:     "file",
		FileName: fileName,
		FileSize: file.Size,
		FileHash: hash,
	...
	// Check storage quota
	if knowledgeDomainInfo.StorageQuota > 0 && knowledgeDomainInfo.StorageUsed >= knowledgeDomainInfo.StorageQuota {
	...
	if processOverrides != nil {
		...
		if IsImageType(getFileType(safeFilename)) {
			...
			knowledgeDomain := knowledgeDomainInfo
			if provider == "" && knowledgeDomain != nil && knowledgeDomain.StorageEngineConfig != nil {
				provider = strings.ToLower(strings.TrimSpace(knowledgeDomain.StorageEngineConfig.DefaultProvider))
			}

			switch provider {
			case "cos":
				if knowledgeDomain == nil || knowledgeDomain.StorageEngineConfig == nil || knowledgeDomain.StorageEngineConfig.COS == nil ||
					knowledgeDomain.StorageEngineConfig.COS.SecretID == "" || knowledgeDomain.StorageEngineConfig.COS.SecretKey == "" ||
					knowledgeDomain.StorageEngineConfig.COS.Region == "" || knowledgeDomain.StorageEngineConfig.COS.BucketName == "" {
			...
			case "minio":
				ok := false
				if knowledgeDomain != nil && knowledgeDomain.StorageEngineConfig != nil && knowledgeDomain.StorageEngineConfig.MinIO != nil {
					m := knowledgeDomain.StorageEngineConfig.MinIO
					if m.Mode == "remote" {
	...
	filePath, err := fileSvc.SaveFile(ctx, file, knowledge.KnowledgeDomainID, knowledge.ID)
	if err != nil {
	...
	// Audit: knowledge document creation
	s.businessAudit.RecordKnowledgeCreated(ctx,
		knowledge.ID, knowledge.Title, safeFilename, getFileType(safeFilename),
		file.Size, kbID, kb.Name)

	// Set tag relations
	if err := s.setAndAttachKnowledgeTags(ctx, knowledgeDomainID, kbID, knowledge, tagIDs); err != nil {
	...
	if slices.Contains([]string{"csv", "xlsx", "xls"}, getFileType(safeFilename)) {
		NewDataTableSummaryTask(ctx, s.task, knowledgeDomainID, knowledge.ID, kb.SummaryModelID, kb.EmbeddingModelID)
	}
...
const (
	maxKnowledgeFolderDepth = 32
	maxKnowledgeFolderPath  = 2048
	maxKnowledgeFolderName  = 255
)

// normalizeUploadedFilePlacement separates a legacy path-bearing filename from
// the actual basename and validates the requested folder hierarchy.
func normalizeUploadedFilePlacement(fileName, folderPath string) (string, string, error) {
	fileName = strings.TrimSpace(strings.ReplaceAll(fileName, "\\", "/"))
	folderPath = strings.TrimSpace(strings.ReplaceAll(folderPath, "\\", "/"))

	// Older web clients sent "folder/file.ext" through fileName. Keep that
	// contract working while storing the path in the new folder model.
	if strings.Contains(fileName, "/") {
		legacyFolder := strings.Trim(path.Dir(fileName), "/")
	...
	if fileName == "" || fileName == "." || fileName == ".." {
		return "", "", werrors.NewValidationError("file name cannot be empty")
	}

	normalizedFolder, err := normalizeKnowledgeFolderPath(folderPath)
	if err != nil {
		return "", "", err
	}
	return fileName, normalizedFolder, nil
}

func knowledgeDomainIDForKB(ctx context.Context, kb *types.KnowledgeBase) (uint64, error) {
	if kb != nil && kb.KnowledgeDomainID != 0 {
		return kb.KnowledgeDomainID, nil
	}
	if knowledgeDomainID, ok := types.KnowledgeDomainIDFromContext(ctx); ok {
		return knowledgeDomainID, nil
	}
	return 0, werrors.NewValidationError("knowledge domain is missing for knowledge base")
}

func (s *knowledgeService) resolveKnowledgeDomainForKB(
	ctx context.Context,
	kb *types.KnowledgeBase,
) (*types.KnowledgeDomain, error) {
	knowledgeDomainID, err := knowledgeDomainIDForKB(ctx, kb)
	if err != nil {
		return nil, err
	}

	if s.knowledgeDomainRepo != nil {
		return s.knowledgeDomainRepo.GetKnowledgeDomainByID(ctx, knowledgeDomainID)
	}

	// Lightweight service tests may omit the repository and provide a hydrated
	// domain through context instead.
	if knowledgeDomain, ok := types.KnowledgeDomainInfoFromContext(ctx); ok {
		if knowledgeDomain.ID == 0 {
			knowledgeDomain.ID = knowledgeDomainID
		}
		return knowledgeDomain, nil
	}
	return &types.KnowledgeDomain{ID: knowledgeDomainID}, nil
}

func normalizeKnowledgeFolderPath(raw string) (string, error) {
	raw = strings.TrimSpace(strings.ReplaceAll(raw, "\\", "/"))
	raw = strings.Trim(raw, "/")
	if raw == "" {
		return "", nil
	}
	if len(raw) > maxKnowledgeFolderPath {
		return "", werrors.NewValidationError("folder path is too long")
	}

	parts := strings.Split(raw, "/")
	if len(parts) > maxKnowledgeFolderDepth {
		return "", werrors.NewValidationError("folder path is too deep")
	}
	for i, part := range parts {
		part = strings.TrimSpace(part)
		if part == "" || part == "." || part == ".." {
			return "", werrors.NewValidationError("folder path contains an invalid segment")
		}
		if len([]rune(part)) > maxKnowledgeFolderName {
			return "", werrors.NewValidationError("folder name is too long")
		}
		for _, char := range part {
			if char < 0x20 || char == 0x7f {
				return "", werrors.NewValidationError("folder path contains control characters")
			}
		}
		parts[i] = part
	}
...
// resolveTargetKnowledgeFolder recreates the source document's relative folder
// path inside another knowledge base. Folder IDs are scoped to a KB and must
// never be copied directly across knowledge bases.
func (s *knowledgeService) resolveTargetKnowledgeFolder(
	ctx context.Context,
	source *types.Knowledge,
	targetKB *types.KnowledgeBase,
) (*string, string, error) {
	if source.FolderID == nil || *source.FolderID == "" {
		return nil, "", nil
	}

	sourceFolder, err := s.repo.GetKnowledgeFolderByID(ctx, source.KnowledgeDomainID, *source.FolderID)
	if err != nil {
		return nil, "", fmt.Errorf("failed to load source knowledge folder: %w", err)
	}
	targetFolder, err := s.repo.ResolveKnowledgeFolderPath(
		ctx,
		targetKB.KnowledgeDomainID,
		targetKB.ID,
		sourceFolder.RelativePath,
	)
	if err != nil {
		return nil, "", fmt.Errorf("failed to resolve target knowledge folder: %w", err)
	}
	if targetFolder == nil {
		return nil, "", nil
	}
	id := targetFolder.ID
	return &id, targetFolder.RelativePath, nil
}
...
func (s *knowledgeService) CreateKnowledgeFromURL(ctx context.Context,
	...
	logger.Infof(ctx, "Checking if URL exists, knowledgeDomain ID: %d", knowledgeDomainID)
...
func (s *knowledgeService) CreateKnowledgeFromManual(ctx context.Context,
	...
	// Set tag relations
	if err := s.setAndAttachKnowledgeTags(ctx, knowledgeDomainID, kbID, knowledge, payload.TagIDs); err != nil {
		logger.Errorf(ctx, "Failed to set knowledge tags, knowledge ID: %s, error: %v", knowledge.ID, err)
		return nil, err
	}
...
func (s *knowledgeService) triggerManualProcessing(ctx context.Context,
	...
	if s.imageResolver != nil {
		fileSvc := s.resolveFileService(ctx, kb)
		afterDataURI, fromDataURI, _ := s.imageResolver.ResolveDataURIImages(ctx, clean, fileSvc, knowledge.KnowledgeDomainID)
		...
		updatedContent, storedImages, resolveErr := s.imageResolver.ResolveRemoteImages(ctx, clean, fileSvc, knowledge.KnowledgeDomainID)
		if resolveErr != nil {
