...
func (s *knowledgeBaseService) ResolveEmbeddingModelKeys(ctx context.Context, kbs []*types.KnowledgeBase) map[string]string {
	...
	for _, kb := range kbs {
		ref := modelRef{ModelID: kb.EmbeddingModelID, KnowledgeDomainID: kb.KnowledgeDomainID}
		uniqueRefs[ref] = struct{}{}
		kbRefs[kb.ID] = ref
	}

	// Resolve each unique (modelID, knowledgeDomainID) to a model identity key
	resolvedKeys := make(map[modelRef]string, len(uniqueRefs))
	for ref := range uniqueRefs {
		knowledgeDomainCtx := context.WithValue(ctx, types.KnowledgeDomainIDContextKey, ref.KnowledgeDomainID)
		model, err := s.modelService.GetModelByID(knowledgeDomainCtx, ref.ModelID)
		if err != nil || model == nil {
			logger.Warnf(ctx, "ResolveEmbeddingModelKeys: cannot resolve model %s for knowledgeDomain %d: %v", ref.ModelID, ref.KnowledgeDomainID, err)
...
func (s *knowledgeBaseService) HybridSearch(ctx context.Context,
	...
	requestKnowledgeDomainID := types.MustKnowledgeDomainIDFromContext(ctx)

	// Batch-load every KB in scope. Required for store grouping,
	// embedding-model consistency validation, and FAQ type detection.
	// GetKnowledgeBaseByIDs is knowledgeDomain-agnostic at the repository layer;
	// authorization for every returned row is enforced explicitly below.
	...
	// Authorize every KB the caller asked for. Without this guard, a
	// caller could pass arbitrary KB UUIDs in params.KnowledgeBaseIDs
	// and reach foreign knowledgeDomains' bound vector stores via the per-group
	// engine resolution downstream.
	if err := s.authorizeKBAccess(ctx, kbs, requestKnowledgeDomainID, params); err != nil {
		return nil, err
	}
	...
	// meaningless cross-model scores. Non-embedding graph KBs are
	...
	// Group KBs by (storeID, owner knowledgeDomain), resolve the bound engine for
	...
	if len(groups) == 0 || allBaseParamsEmpty(groups) {
		// Graph-only fan-out: every KB is non-retrievable.
	...
	if knowledgeDomainInfo != nil {
		retrievalCfg = knowledgeDomainInfo.RetrievalConfig
	}
...
// buildRetrievalParams. If the caller selects a graph-only
...
// BaseParams slice. True only when every KB in scope has neither vector nor
// keyword indexing — HybridSearch then
...
func (s *knowledgeBaseService) buildRetrievalParams(
	...
	// vector indexing enabled (for example, graph-only KBs) has no
