...
// The registry holds provider type factories; the providerRepo loads knowledgeDomain-specific configurations.
...
func (s *WebSearchService) resolveProvider(
	...
	if providerID != "" {
		knowledgeDomainID, ok := types.KnowledgeDomainIDFromContext(ctx)
		if !ok {
			return nil, fmt.Errorf("knowledgeDomain ID not found in context")
		}

		entity, err := s.providerRepo.GetByID(ctx, knowledgeDomainID, providerID)
		if err != nil {
