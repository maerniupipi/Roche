...
func (s *webSearchProviderService) CreateProvider(ctx context.Context, provider *types.WebSearchProviderEntity) error {
	if provider.KnowledgeDomainID == 0 {
		return fmt.Errorf("knowledgeDomain ID is required")
	}
	...
	if provider.IsDefault {
		if err := s.repo.ClearDefault(ctx, provider.KnowledgeDomainID, ""); err != nil {
			logger.Warnf(ctx, "Failed to clear default providers: %v", err)
		}
	}

	logger.Infof(ctx, "Creating web search provider: knowledgeDomain=%d, name=%s, type=%s", provider.KnowledgeDomainID, provider.Name, provider.Provider)
...
func (s *webSearchProviderService) UpdateProvider(ctx context.Context, provider *types.WebSearchProviderEntity) error {
	...
	if provider.IsDefault {
		if err := s.repo.ClearDefault(ctx, provider.KnowledgeDomainID, provider.ID); err != nil {
			logger.Warnf(ctx, "Failed to clear default providers: %v", err)
		}
	}
	...
	logger.Infof(ctx, "Updating web search provider: knowledgeDomain=%d, id=%s", provider.KnowledgeDomainID, provider.ID)
...
func (s *webSearchProviderService) UpdateProviderCredentials(
	...
	existing, err := s.repo.GetByID(ctx, knowledgeDomainID, id)
	if err != nil {
		return nil, err
	}
	if existing == nil {
	...
	if apiKey != nil && *apiKey != "" && *apiKey != existing.Parameters.APIKey {
		...
		logger.Infof(ctx, "WebSearch provider credentials updated: knowledgeDomain=%d id=%s", knowledgeDomainID, id)
	}
	return existing, nil
}
...
func (s *webSearchProviderService) ClearProviderCredential(
	ctx context.Context, knowledgeDomainID uint64, id, field string,
) error {
	if field != "api_key" {
	...
	logger.Infof(ctx, "WebSearch provider credential cleared by user: knowledgeDomain=%d id=%s field=%s", knowledgeDomainID, id, field)
	return nil
}

// DeleteProvider deletes a provider by knowledgeDomain + id.
func (s *webSearchProviderService) DeleteProvider(ctx context.Context, knowledgeDomainID uint64, id string) error {
	logger.Infof(ctx, "Deleting web search provider: knowledgeDomain=%d, id=%s", knowledgeDomainID, id)
