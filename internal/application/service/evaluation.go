...
func (e *EvaluationService) EvaluationResult(ctx context.Context, taskID string) (*types.EvaluationDetail, error) {
	...
	logger.Infof(
		ctx,
		"Checking knowledgeDomain ID match, task knowledgeDomain ID: %d, current knowledgeDomain ID: %d",
		detail.Task.KnowledgeDomainID, knowledgeDomainID,
	)

	if knowledgeDomainID != detail.Task.KnowledgeDomainID {
		logger.Error(ctx, "KnowledgeDomain ID mismatch")
		return nil, errors.New("knowledgeDomain ID does not match")
	}
...
func (e *EvaluationService) Evaluation(ctx context.Context,
	...
	// Get knowledgeDomain ID from context for multi-tenancy support
	knowledgeDomainID := types.MustKnowledgeDomainIDFromContext(ctx)
	...
	taskID := utils.GenerateTaskID("evaluation", knowledgeDomainID, datasetID)
