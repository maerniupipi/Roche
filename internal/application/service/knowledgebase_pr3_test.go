...
type fakeOwnership struct {
	owned map[string]uint64 // storeID -> knowledgeDomainID owner
	err   error
}

func (f *fakeOwnership) StoreOwnedBy(_ context.Context, storeID string, knowledgeDomainID uint64) (bool, error) {
	if f.err != nil {
		return false, f.err
	}
	owner, ok := f.owned[storeID]
	return ok && owner == knowledgeDomainID, nil
}
...
// keyed by ID. No knowledgeDomain scoping is applied — the tested paths already
...
func (r *fakeKBRepo) GetKnowledgeBaseByIDAndKnowledgeDomain(_ context.Context, id string, knowledgeDomainID uint64) (*types.KnowledgeBase, error) {
	kb := r.rows[id]
	if kb == nil || kb.KnowledgeDomainID != knowledgeDomainID {
...
func (r *fakeKBRepo) ListKnowledgeBasesByKnowledgeDomainID(_ context.Context, _ uint64) ([]*types.KnowledgeBase, error) {
	return nil, nil
}
...
func (r *fakeKBRepo) CountByModelID(_ context.Context, _ string) (int64, error) {
	return 0, nil
}
...
func ctxWithKnowledgeDomain(knowledgeDomainID uint64) context.Context {
	return context.WithValue(context.Background(), types.KnowledgeDomainIDContextKey, knowledgeDomainID)
}

func ctxWithKnowledgeDomainStorage(knowledgeDomainID uint64, defaultProvider string) context.Context {
	ctx := ctxWithKnowledgeDomain(knowledgeDomainID)
	...
	return context.WithValue(ctx, types.KnowledgeDomainInfoContextKey, knowledgeDomain)
}

func TestCreateKnowledgeBase_DoesNotReadStorageProviderFromRequestContext(t *testing.T) {
	repo := newFakeKBRepo()
	svc := newPR3KBService(repo, &fakeRegistry{registered: map[string]struct{}{}}, &fakeOwnership{})

	kb, err := svc.CreateKnowledgeBase(ctxWithKnowledgeDomainStorage(1, "minio"), &types.KnowledgeBase{
		Name:              "kb",
		KnowledgeDomainID: 1,
	})
	...
	kbExplicit, err := svc.CreateKnowledgeBase(ctxWithKnowledgeDomainStorage(1, "minio"), &types.KnowledgeBase{
		Name:                  "kb2",
		KnowledgeDomainID:     1,
...
func TestCreateKnowledgeBase_VectorStoreBinding(t *testing.T) {
	...
	t.Run("nil vector_store_id persists unchanged", func(t *testing.T) {
		...
		kb, err := svc.CreateKnowledgeBase(ctxWithKnowledgeDomain(1), &types.KnowledgeBase{Name: "kb", KnowledgeDomainID: 1})
		require.NoError(t, err)
	...
	t.Run("empty string vector_store_id normalizes to nil", func(t *testing.T) {
		...
		empty := ""
		kb, err := svc.CreateKnowledgeBase(ctxWithKnowledgeDomain(1), &types.KnowledgeBase{
			Name: "kb", KnowledgeDomainID: 1, VectorStoreID: &empty,
		})
		require.NoError(t, err)
		assert.Nil(t, kb.VectorStoreID)
	})
	...
	t.Run("malformed UUID rejected with BindingInvalid code", func(t *testing.T) {
		...
		bad := "not-a-uuid"
		_, err := svc.CreateKnowledgeBase(ctxWithKnowledgeDomain(1), &types.KnowledgeBase{
			Name: "kb", KnowledgeDomainID: 1, VectorStoreID: &bad,
		})
		require.Error(t, err)
	...
	t.Run("valid UUID + owned + registered → persist OK", func(t *testing.T) {
		...
		id := validKBStoreUUID
		kb, err := svc.CreateKnowledgeBase(ctxWithKnowledgeDomain(1), &types.KnowledgeBase{
			Name: "kb", KnowledgeDomainID: 1, VectorStoreID: &id,
		})
		require.NoError(t, err)
	...
	t.Run("valid UUID + cross-knowledgeDomain → BindingInvalid", func(t *testing.T) {
		repo := newFakeKBRepo()
		svc := newPR3KBService(repo,
			&fakeRegistry{registered: registered},
			&fakeOwnership{owned: map[string]uint64{validKBStoreUUID: 2}}, // owned by knowledgeDomain 2
		)
		id := validKBStoreUUID
		_, err := svc.CreateKnowledgeBase(ctxWithKnowledgeDomain(1), &types.KnowledgeBase{
			Name: "kb", KnowledgeDomainID: 1, VectorStoreID: &id,
...
func TestCopyKnowledgeBase_Defenses(t *testing.T) {
	mkKB := func(id string, knowledgeDomain uint64, embed string, vsid *string) *types.KnowledgeBase {
		return &types.KnowledgeBase{
			ID: id, Name: id, KnowledgeDomainID: knowledgeDomain, EmbeddingModelID: embed, VectorStoreID: vsid,
		}
	}
	storeA := "store-A"
	storeB := "store-B"
	...
	t.Run("dstKB empty -> new KB inherits VectorStoreID", func(t *testing.T) {
		...
		src, tgt, err := svc.CopyKnowledgeBase(ctxWithKnowledgeDomain(1), "src", "")
		require.NoError(t, err)
	...
	t.Run("dstKB empty + source nil VectorStoreID -> new target nil too", func(t *testing.T) {
		...
		_, tgt, err := svc.CopyKnowledgeBase(ctxWithKnowledgeDomain(1), "src", "")
		require.NoError(t, err)
	...
	t.Run("dstKB set + same embedding model + both nil stores -> OK", func(t *testing.T) {
		...
		_, _, err := svc.CopyKnowledgeBase(ctxWithKnowledgeDomain(1), "src", "dst")
		require.NoError(t, err)
	})
