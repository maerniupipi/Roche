...
// TestBuildStorageConfig_KnowledgeDomainMergeAllProviders pins the knowledgeDomain-merge branch
...
// fully-populated DocParserStorageConfig when the knowledgeDomain carries the matching
...
func TestBuildStorageConfig_KnowledgeDomainMergeAllProviders(t *testing.T) {
	t.Parallel()

	// Each table entry sets one engine config on a fresh knowledgeDomain and checks that
	...
	tests := []struct {
	...
	}{
		{
			...
			knowledgeDomain: &types.KnowledgeDomain{StorageEngineConfig: &types.StorageEngineConfig{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.WithValue(context.Background(), types.KnowledgeDomainInfoContextKey, tt.knowledgeDomain)
...
// resolve through the knowledgeDomain-merge path, so a populated legacy struct on those
...
func TestBuildStorageConfig_LegacyPathOnlyForCOSAndMinIO(t *testing.T) {
	...
	// up; everything else falls through to the knowledgeDomain merge.
	...
	tests := []struct {
		...
		wantSecret           string
		wantAppID            string
		wantKnowledgeDomains bool // true when the test should fall through to knowledgeDomain merge (empty result)
	}{
		...
		{name: "local skips legacy", provider: "local", wantKnowledgeDomains: true},
		{name: "tos skips legacy", provider: "tos", wantKnowledgeDomains: true},
		{name: "s3 skips legacy", provider: "s3", wantKnowledgeDomains: true},
		{name: "oss skips legacy", provider: "oss", wantKnowledgeDomains: true},
		{name: "ks3 skips legacy", provider: "ks3", wantKnowledgeDomains: true},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background() // no knowledgeDomain config → knowledgeDomain-merge yields empty
...
// TestBuildStorageConfig_NoKnowledgeDomainFallsThroughToEmpty pins the no-knowledgeDomain case:
...
func TestBuildStorageConfig_NoKnowledgeDomainFallsThroughToEmpty(t *testing.T) {
	t.Parallel()
