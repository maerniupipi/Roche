...
func (s *mcpServiceService) GetMCPServiceByID(
	...
	service, err := s.mcpServiceRepo.GetByID(ctx, knowledgeDomainID, id)
	if err != nil {
...
func (s *mcpServiceService) ListMCPServices(ctx context.Context, knowledgeDomainID uint64) ([]*types.MCPService, error) {
	services, err := s.mcpServiceRepo.List(ctx, knowledgeDomainID)
	if err != nil {
...
func (s *mcpServiceService) ListMCPServicesByIDs(
	...
	services, err := s.mcpServiceRepo.ListByIDs(ctx, knowledgeDomainID, ids)
	if err != nil {
...
func (s *mcpServiceService) UpdateMCPService(ctx context.Context, service *types.MCPService) error {
	...
	existing, err := s.mcpServiceRepo.GetByID(ctx, service.KnowledgeDomainID, service.ID)
	if err != nil {
...
func (s *mcpServiceService) DeleteMCPService(ctx context.Context, knowledgeDomainID uint64, id string) error {
	// Check if service exists
	existing, err := s.mcpServiceRepo.GetByID(ctx, knowledgeDomainID, id)
	if err != nil {
		return fmt.Errorf("failed to get MCP service: %w", err)
	}
	if existing == nil {
	...
	if err := s.mcpServiceRepo.Delete(ctx, knowledgeDomainID, id); err != nil {
...
func (s *mcpServiceService) TestMCPService(
	...
	if service.AuthConfig.IsOAuth() {
		...
		config.KnowledgeDomainID, _ = types.KnowledgeDomainIDFromContext(ctx)
