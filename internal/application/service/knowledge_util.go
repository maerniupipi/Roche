...
func (s *knowledgeService) buildStorageConfig(ctx context.Context, kb *types.KnowledgeBase) *types.DocParserStorageConfig {
	...
	// resolve via the knowledgeDomain-merge path below. Listing them here keeps the fall-through
	...
	// Merge from knowledgeDomain's StorageEngineConfig.
	...
	if knowledgeDomain != nil && knowledgeDomain.StorageEngineConfig != nil {
		sec := knowledgeDomain.StorageEngineConfig
	...
	logger.Infof(ctx, "[storage] buildStorageConfig use merged knowledgeDomain/global config: kb=%s provider=%s bucket=%s path_prefix=%s endpoint=%s",
...
// based on the KB's StorageProviderConfig (or legacy StorageConfig.Provider) and the knowledgeDomain's StorageEngineConfig.
// Falls back to the global fileSvc when no knowledgeDomain-level storage config is found.
func (s *knowledgeService) resolveFileService(ctx context.Context, kb *types.KnowledgeBase) interfaces.FileService {
	...
	if provider == "" || knowledgeDomain == nil || knowledgeDomain.StorageEngineConfig == nil {
		logger.Infof(ctx, "[storage] resolveFileService fallback default: kb=%s provider=%q knowledge_domain_cfg=%v",
			kb.ID, provider, knowledgeDomain != nil && knowledgeDomain.StorageEngineConfig != nil)
		return s.fileSvc
	}
	...
	if err != nil {
		logger.Errorf(ctx, "Failed to create %s file service from knowledgeDomain config: %v, falling back to default", provider, err)
		return s.fileSvc
	}
...
// knowledgeDomain/KB config changes but files were stored under the old provider.
func (s *knowledgeService) resolveFileServiceForPath(ctx context.Context, kb *types.KnowledgeBase, filePath string) interfaces.FileService {
	...
	if configured == "" {
		knowledgeDomain, _ := ctx.Value(types.KnowledgeDomainInfoContextKey).(*types.KnowledgeDomain)
		if knowledgeDomain != nil && knowledgeDomain.StorageEngineConfig != nil {
			configured = strings.ToLower(strings.TrimSpace(knowledgeDomain.StorageEngineConfig.DefaultProvider))
		}
	}
	if configured == "" {
...
func downloadFileFromURL(ctx context.Context, fileURL string, payloadFileName, payloadFileType *string) ([]byte, error) {
	...
	tmpFile, err := os.CreateTemp("", "rochekap-fileurl-*")
	if err != nil {
