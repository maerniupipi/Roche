...
func (s *ChunkExtractService) Handle(ctx context.Context, t *asynq.Task) error {
	...
	ctx = context.WithValue(ctx, types.KnowledgeDomainIDContextKey, p.KnowledgeDomainID)
	...
	chunk, err := s.chunkRepo.GetChunkByID(ctx, p.KnowledgeDomainID, p.ChunkID)
	if err != nil {
...
func (s *DataTableSummaryService) prepareResources(ctx context.Context, payload DataTableSummaryPayload) (*extractionResources, error) {
	...
	// 获取知识域信息
	knowledgeDomainInfo, err := s.knowledgeDomainService.GetKnowledgeDomainByID(ctx, payload.KnowledgeDomainID)
	if err != nil {
	...
	// route to the bound store (or fall back to knowledgeDomain engines if unbound).
	...
	// The factory's unbound path reads KnowledgeDomainInfo from ctx.
	...
	retrieveEngine, err := retriever.CreateRetrieveEngineForKB(
		ctx, s.retrieveEngine, s.ownership, payload.KnowledgeDomainID, vectorStoreID)
	if err != nil {
...
// It falls back to the global service when knowledgeDomain storage config is unavailable.
func (s *DataTableSummaryService) resolveFileServiceForKnowledge(ctx context.Context, resources *extractionResources) interfaces.FileService {
	...
	if resources.knowledgeDomain == nil || resources.knowledgeDomain.StorageEngineConfig == nil {
		return s.fileService
	}

	provider := types.InferStorageFromFilePath(resources.knowledge.FilePath)
	if provider == "" {
		provider = strings.ToLower(strings.TrimSpace(resources.knowledgeDomain.StorageEngineConfig.DefaultProvider))
	}
	if provider == "" {
		return s.fileService
	}
	...
	resolvedSvc, resolvedProvider, err := filesvc.NewFileServiceFromStorageConfig(
		provider,
		resources.knowledgeDomain.StorageEngineConfig,
		baseDir,
	)
	if err != nil {
...
func (s *DataTableSummaryService) processTableData(ctx context.Context, resources *extractionResources) ([]*types.Chunk, error) {
	...
	duckdbTool := tools.NewDataAnalysisTool(s.knowledgeBaseService, s.knowledgeService, s.knowledgeDomainService, fileSvc, s.sqlDB, sessionID)
...
func (s *DataTableSummaryService) buildChunks(resources *extractionResources, tableDescription string, columnDescription string) []*types.Chunk {
	...
	summaryChunk := &types.Chunk{
		ID:                uuid.New().String(),
		KnowledgeDomainID: resources.knowledge.KnowledgeDomainID,
		KnowledgeID:       resources.knowledge.ID,
