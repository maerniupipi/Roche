...
// (knowledgeDomainID, knowledgeID) using the destination FileService, returning the new
...
func copyOwnedObject(
	...
	return dstSvc.CopyFile(ctx, srcPath, knowledgeDomainID, knowledgeID)
}

// cloneChunkImageInfo parses a chunk's image_info JSON, copies every referenced
// object into a NEW object owned by (knowledgeDomainID, knowledgeID), and returns the
...
func cloneChunkImageInfo(
	...
	for _, img := range images {
		...
		if !cached {
			newURL, err = copyOwnedObject(ctx, dstSvc, dstSvc, img.URL, knowledgeDomainID, knowledgeID)
			if err != nil {
...
func (s *knowledgeService) CloneKnowledgeBase(ctx context.Context, srcID, dstID string) error {
	...
	addKnowledge, err := s.repo.AminusB(ctx, srcKB.KnowledgeDomainID, srcKB.ID, dstKB.KnowledgeDomainID, dstKB.ID)
	if err != nil {
		logger.Errorf(ctx, "Failed to get knowledge: %v", err)
		return err
	}

	delKnowledge, err := s.repo.AminusB(ctx, dstKB.KnowledgeDomainID, dstKB.ID, srcKB.KnowledgeDomainID, srcKB.ID)
	if err != nil {
		logger.Errorf(ctx, "Failed to get knowledge: %v", err)
		return err
	}
	...
	for _, knowledge := range addKnowledge {
		g.Go(func() error {
			srcKn, err := s.repo.GetKnowledgeByID(gctx, srcKB.KnowledgeDomainID, knowledge)
			if err != nil {
...
func (s *knowledgeService) CloneChunk(ctx context.Context, src, dst *types.Knowledge) (err error) {
		...
		for _, sourceChunk := range sourceChunks {
			...
			if sourceChunk.TagID != "" {
				if mappedTagID, ok := tagIDMapping[sourceChunk.TagID]; ok {
					...
					targetTagID = s.getOrCreateTagInTarget(ctx, src.KnowledgeDomainID, dst.KnowledgeDomainID, dst.KnowledgeBaseID, sourceChunk.TagID, tagIDMapping)
				}
			}
			...
			newImageInfo, copied, copyErr := cloneChunkImageInfo(
				ctx, dstSvc, sourceChunk.ImageInfo, dst.KnowledgeDomainID, dst.ID, urlCache)
			if copyErr != nil {
			...
			targetChunk := &types.Chunk{
				ID:                uuid.New().String(),
				KnowledgeDomainID: dst.KnowledgeDomainID,
				KnowledgeID:       dst.ID,
	...
	retrieveEngine, err := retriever.CreateRetrieveEngineForKB(
		ctx, s.retrieveEngine, s.ownership, knowledgeDomainID, sourceStoreID)
	if err != nil {
		return err
	}
...
func (s *knowledgeService) cloneFAQKnowledgeBase(
	...
	srcKnowledgeList, err := s.repo.ListKnowledgeByKnowledgeBaseID(ctx, srcKB.KnowledgeDomainID, srcKB.ID)
	if err != nil {
	...
	chunksToAdd, chunksToDelete, err := s.chunkRepo.FAQChunkDiff(ctx, srcKB.KnowledgeDomainID, srcKB.ID, dstKB.KnowledgeDomainID, dstKB.ID)
	if err != nil {
	...
	retrieveEngine, err := retriever.CreateRetrieveEngineForKB(
		ctx, s.retrieveEngine, s.ownership, types.MustKnowledgeDomainIDFromContext(ctx), sourceStoreID)
	if err != nil {
	...
	if len(chunksToDelete) > 0 {
		...
		if err := s.chunkRepo.DeleteChunks(ctx, dstKB.KnowledgeDomainID, chunksToDelete); err != nil {
	...
	for i := 0; i < len(chunksToAdd); i += batch {
		...
		// Get source chunks
		srcChunks, err := s.chunkRepo.ListChunksByID(ctx, srcKB.KnowledgeDomainID, batchIDs)
		if err != nil {
		...
		for _, srcChunk := range srcChunks {
			...
			if srcChunk.TagID != "" {
				if mappedTagID, ok := tagIDMapping[srcChunk.TagID]; ok {
					...
					// Try to find or create the tag in target knowledge base
					targetTagID = s.getOrCreateTagInTarget(ctx, srcKB.KnowledgeDomainID, dstKB.KnowledgeDomainID, dstKB.ID, srcChunk.TagID, tagIDMapping)
				}
			}
			...
			newImageInfo, copied, copyErr := cloneChunkImageInfo(
				ctx, dstSvc, srcChunk.ImageInfo, dstKB.KnowledgeDomainID, dstKnowledge.ID, imageURLCache)
			if copyErr != nil {
			...
			newChunk := &types.Chunk{
				ID:                uuid.New().String(),
				KnowledgeDomainID: dstKB.KnowledgeDomainID,
...
func (s *knowledgeService) getOrCreateFAQKnowledge(ctx context.Context, kb *types.KnowledgeBase, srcKnowledge *types.Knowledge) (*types.Knowledge, error) {
	...
	knowledgeList, err := s.repo.ListKnowledgeByKnowledgeBaseID(ctx, kb.KnowledgeDomainID, kb.ID)
	if err != nil {
		return nil, err
	}
...
func (s *knowledgeService) ProcessKnowledgeMove(ctx context.Context, t *asynq.Task) error {
	...
	knowledgeDomainInfo, err := s.knowledgeDomainRepo.GetKnowledgeDomainByID(ctx, payload.KnowledgeDomainID)
	if err != nil {
		logger.Errorf(ctx, "ProcessKnowledgeMove: failed to get knowledgeDomain info: %v", err)
		return fmt.Errorf("failed to get knowledgeDomain info: %w", err)
	}
	ctx = context.WithValue(ctx, types.KnowledgeDomainInfoContextKey, knowledgeDomainInfo)
...
func (s *knowledgeService) moveKnowledgeReuseVectors(
	...
	oldChunks, err := s.chunkRepo.ListChunksByKnowledgeID(ctx, knowledgeDomainID, knowledge.ID)
	if err != nil {
	...
	if err := s.chunkRepo.MoveChunksByKnowledgeID(ctx, knowledgeDomainID, knowledge.ID, targetKB.ID); err != nil {
		return fmt.Errorf("failed to move chunks: %w", err)
	}

	// 4. Update knowledge record. Tags and folder IDs are KB-scoped, so clear
	// tags and recreate the same relative folder path in the target KB.
	...
	targetFolderID, targetFolderPath, err := s.resolveTargetKnowledgeFolder(ctx, knowledge, targetKB)
	if err != nil {
		return err
	}
	knowledge.KnowledgeBaseID = targetKB.ID
	knowledge.FolderID = targetFolderID
	knowledge.FolderPath = targetFolderPath
...
func (s *knowledgeService) moveKnowledgeReparse(
	...
	// 2. Update knowledge to belong to target KB. Tags and folder IDs are
	// KB-scoped, so clear tags and recreate the relative folder path.
	if err := s.repo.DeleteKnowledgeTagRelations(ctx, knowledge.ID); err != nil {
		return fmt.Errorf("failed to clear knowledge tag relations: %w", err)
	}
	targetFolderID, targetFolderPath, err := s.resolveTargetKnowledgeFolder(ctx, knowledge, targetKB)
...
func (s *knowledgeService) getOrCreateTagInTarget(
	ctx context.Context,
	srcKnowledgeDomainID, dstKnowledgeDomainID uint64,
	dstKnowledgeBaseID string,
	srcTagID string,
	tagIDMapping map[string]string,
) string {
	// Get source tag
	srcTag, err := s.tagRepo.GetByID(ctx, srcKnowledgeDomainID, srcTagID)
	...
	dstTag, err := s.tagRepo.GetByName(ctx, dstKnowledgeDomainID, dstKnowledgeBaseID, srcTag.Name)
	...
	newTag := &types.KnowledgeTag{
		ID:                uuid.New().String(),
		KnowledgeDomainID: dstKnowledgeDomainID,
