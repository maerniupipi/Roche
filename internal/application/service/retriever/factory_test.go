...
// fakeOwnership is an in-memory KnowledgeDomainStoreOwnership implementation.
...
type fakeOwnership struct {
	mu sync.Mutex
	// owned maps storeID → knowledgeDomainID that owns it. Absence means "not owned".
	owned map[string]uint64
	// err, when non-nil, is returned from every StoreOwnedBy call
	// (used to simulate infrastructure failures).
	err error
	// calls records every (storeID, knowledgeDomainID) pair asked. Tests use this
...
func (f *fakeOwnership) StoreOwnedBy(ctx context.Context, storeID string, knowledgeDomainID uint64) (bool, error) {
	f.mu.Lock()
	f.calls = append(f.calls, ownershipCall{storeID: storeID, knowledgeDomainID: knowledgeDomainID})
	err := f.err
	ownedKnowledgeDomain, ok := f.owned[storeID]
	f.mu.Unlock()
	if err != nil {
		return false, err
	}
	return ok && ownedKnowledgeDomain == knowledgeDomainID, nil
}
...
// knowledgeDomain's effective engines.
...
// newKnowledgeDomainCtx returns a context carrying a KnowledgeDomain with the given
// EffectiveEngines. Factory's unbound path consumes this via
// types.KnowledgeDomainInfoFromContext.
func newKnowledgeDomainCtx(engines []types.RetrieverEngineParams) context.Context {
	knowledgeDomain := &types.KnowledgeDomain{
		RetrieverEngines: types.RetrieverEngines{Engines: engines},
	}
	return context.WithValue(context.Background(), types.KnowledgeDomainInfoContextKey, knowledgeDomain)
}

// ----- CreateRetrieveEngineForKB -----

func TestCreateRetrieveEngineForKB_Unbound(t *testing.T) {
	milvusEngine := &fakeEngine{
	...
	registry := registryWithStores(t, nil, map[types.RetrieverEngineType]*fakeEngine{
		types.MilvusRetrieverEngineType: milvusEngine,
	})
	ownership := &fakeOwnership{}

	knowledgeDomainCtx := newKnowledgeDomainCtx([]types.RetrieverEngineParams{
		{RetrieverEngineType: types.MilvusRetrieverEngineType, RetrieverType: types.VectorRetrieverType},
	})
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			engine, err := CreateRetrieveEngineForKB(knowledgeDomainCtx, registry, ownership, 1, tc.store)
			require.NoError(t, err)
			require.NotNil(t, engine)
			assert.Len(t, engine.engineInfos, 1, "unbound path uses knowledgeDomain effective engines")
			assert.Same(t, milvusEngine, engine.engineInfos[0].retrieveEngine)
...
func TestCreateRetrieveEngineForKB_UnboundMissingKnowledgeDomain(t *testing.T) {
	registry := registryWithStores(t, nil, nil)
	ownership := &fakeOwnership{}

	_, err := CreateRetrieveEngineForKB(context.Background(), registry, ownership, 1, nil)
	require.Error(t, err)
	assert.True(t, errors.Is(err, ErrKnowledgeDomainInfoMissing),
		"err %q must wrap ErrKnowledgeDomainInfoMissing", err)
}
...
func TestCreateRetrieveEngineForKB_CrossKnowledgeDomain(t *testing.T) {
	esEngine := &fakeEngine{
		engineType: types.ElasticsearchRetrieverEngineType,
	...
	// store-A is owned by knowledgeDomain 2, not knowledgeDomain 1
	...
	assert.True(t, errors.Is(err, ErrVectorStoreForbidden),
		"cross-knowledgeDomain access must yield ErrVectorStoreForbidden; got %q", err)
...
func TestCreateRetrieveEngineForKB_StoreNotRegistered(t *testing.T) {
	// Ownership says the knowledgeDomain owns the store, but registry has not
...
func TestCreateRetrieveEngineForKB_OwnershipLookupError(t *testing.T) {
	registry := registryWithStores(t,
		map[string]*fakeEngine{"store-A": {engineType: types.MilvusRetrieverEngineType}},
		nil,
	)
...
func TestCreateRetrieveEngineFromPayload_LegacyUnbound(t *testing.T) {
	...
	cases := []struct {
		...
		{"missing field", decode(t, `{"knowledge_domain_id":1}`)},
		{"explicit null", decode(t, `{"knowledge_domain_id":1,"vector_store_id":null}`)},
		{"empty string", decode(t, `{"knowledge_domain_id":1,"vector_store_id":""}`)},
...
func TestCreateRetrieveEngineFromPayload_TamperedCrossKnowledgeDomain(t *testing.T) {
	esEngine := &fakeEngine{engineType: types.ElasticsearchRetrieverEngineType,
		support: []types.RetrieverType{types.VectorRetrieverType}}
	registry := registryWithStores(t,
		map[string]*fakeEngine{"store-A": esEngine}, nil)
	// Store is owned by knowledgeDomain 99, but the (possibly tampered) payload
	// claims knowledgeDomain 1. Factory must reject.
...
func TestVerifyBinding(t *testing.T) {
	...
	t.Run("cross-knowledgeDomain returns Forbidden (not Found)", func(t *testing.T) {
		// store owned by knowledgeDomain 2, queried by knowledgeDomain 1
		registry := registryWithStores(t,
			map[string]*fakeEngine{"store-A": esEngine},
			nil,
		)
