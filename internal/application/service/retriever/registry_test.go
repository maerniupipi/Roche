...
func TestRegistry_Register(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		err := reg.Register(newMock(types.MilvusRetrieverEngineType))
...
func TestRegistry_GetRetrieveEngineService(t *testing.T) {
	reg := NewRetrieveEngineRegistry().(*RetrieveEngineRegistry)
	_ = reg.Register(newMock(types.MilvusRetrieverEngineType))

	t.Run("found", func(t *testing.T) {
		svc, err := reg.GetRetrieveEngineService(types.MilvusRetrieverEngineType)
		assert.NoError(t, err)
		assert.NotNil(t, svc)
	})
...
func TestRegistry_RegisterWithStoreID(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		reg.RegisterWithStoreID("store-1", newMock(types.MilvusRetrieverEngineType))
...
func TestRegistry_DualMapIsolation(t *testing.T) {
	...
	_ = reg.Register(newMock(types.MilvusRetrieverEngineType))
	reg.RegisterWithStoreID("store-milvus", newMock(types.MilvusRetrieverEngineType))
	...
	t.Run("unregister byStoreID does not affect byEngineType", func(t *testing.T) {
		reg.UnregisterByStoreID("store-milvus")
		svc, err := reg.GetRetrieveEngineService(types.MilvusRetrieverEngineType)
		assert.NoError(t, err)
		assert.NotNil(t, svc)
	})
...
func TestRegistry_ConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < goroutines; i++ {
		...
		go func() {
			defer wg.Done()
			reg.RegisterWithStoreID(storeID, newMock(types.MilvusRetrieverEngineType))
		}()
		go func() {
			defer wg.Done()
