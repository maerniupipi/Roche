...
func (m *mockVectorStoreRepo) GetByID(_ context.Context, knowledgeDomainID uint64, id string) (*types.VectorStore, error) {
	for _, s := range m.stores {
		if s.ID == id && s.KnowledgeDomainID == knowledgeDomainID {
			return s, nil
		}
	}
	return nil, nil
...
func (m *mockVectorStoreRepo) List(_ context.Context, knowledgeDomainID uint64) ([]*types.VectorStore, error) {
	var result []*types.VectorStore
	for _, s := range m.stores {
		if s.KnowledgeDomainID == knowledgeDomainID {
...
func TestCreateStore_ValidationError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "empty name",
			store: &types.VectorStore{KnowledgeDomainID: 1, EngineType: types.MilvusRetrieverEngineType},
		},
		{
			name:  "invalid engine type",
			store: &types.VectorStore{KnowledgeDomainID: 1, Name: "test", EngineType: "unknown"},
		},
		{
			name:  "zero knowledgeDomain ID",
			store: &types.VectorStore{Name: "test", EngineType: types.MilvusRetrieverEngineType},
		},
	}
...
func TestCreateStore_ConnectionConfigValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "retired postgres retriever is rejected",
			store: &types.VectorStore{
				KnowledgeDomainID: 1, Name: "test",
				EngineType: types.RetrieverEngineType("postgres"),
			},
			wantError: true,
		},
		{
...
func TestCreateStore_DifferentEndpointSameIndex_Allowed(t *testing.T) {
	// Every registerable engine performs a network probe, so this service-level
	// case needs an injectable connection tester or a real backend.
	//
	// The underlying invariant (same endpoint + different index name is
	// allowed) is still indirectly exercised by the live Milvus/Qdrant
...
func TestUpdateStore_ValidationError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "empty name",
			store: &types.VectorStore{ID: "id", KnowledgeDomainID: 1, Name: ""},
		},
	}
...
func TestSaveDetectedVersion_RepoError(t *testing.T) {
	...
	store := &types.VectorStore{ID: "store-1", KnowledgeDomainID: 1}
...
func TestTestConnection_RetiredPostgresRetrieverRejected(t *testing.T) {
	repo := &mockVectorStoreRepo{}
	svc := NewVectorStoreService(repo, nil, nil, nil, nil)

	_, err := svc.TestConnection(context.Background(), types.RetrieverEngineType("postgres"),
...
func TestValidateConnectionConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			config:     types.ConnectionConfig{Addr: "doris-fe:9030", Database: "rochekap"},
			wantError:  false,
		},
		{
			name:       "doris missing addr",
			engineType: types.DorisRetrieverEngineType,
			config:     types.ConnectionConfig{Database: "rochekap"},
			wantError:  true,
		},
		{
...
func (r *realStoreRepo) GetByID(ctx context.Context, knowledgeDomainID uint64, id string) (*types.VectorStore, error) {
	_ = knowledgeDomainID
...
func (r *realStoreRepo) List(ctx context.Context, knowledgeDomainID uint64) ([]*types.VectorStore, error) {
	_ = knowledgeDomainID
...
func (r *realStoreRepo) Delete(ctx context.Context, knowledgeDomainID uint64, id string) error {
	_ = knowledgeDomainID
...
func (r *realKBRepo) CountByVectorStoreID(ctx context.Context, db *gorm.DB, knowledgeDomainID uint64, storeID string) (int64, error) {
	_ = knowledgeDomainID
	if db == nil {
		db = r.db
	}
...
func (r *realKBRepo) CountByModelID(_ context.Context, _ string) (int64, error) {
	return 0, nil
}
...
func (r *realKBRepo) GetKnowledgeBaseByIDAndKnowledgeDomain(_ context.Context, _ string, _ uint64) (*types.KnowledgeBase, error) {
	return nil, nil
}
...
func (r *realKBRepo) ListKnowledgeBasesByKnowledgeDomainID(_ context.Context, _ uint64) ([]*types.KnowledgeBase, error) {
	return nil, nil
}
...
func insertGuardStore(t *testing.T, db *gorm.DB, id string, knowledgeDomainID uint64) {
	t.Helper()
	require.NoError(t, db.Create(&types.VectorStore{
		ID: id, Name: id, EngineType: types.QdrantRetrieverEngineType, KnowledgeDomainID: knowledgeDomainID,
	}).Error)
}

func insertGuardKB(t *testing.T, db *gorm.DB, knowledgeDomainID uint64, vsid *string) string {
	t.Helper()
	id := "kb-" + knowledgeDomainID2s(knowledgeDomainID) + "-" + ptrOrEmpty(vsid)
...
func knowledgeDomainID2s(t uint64) string {
	if t == 1 {
		return "t1"
	}
	if t == 2 {
...
func TestDeleteStore_Guard_BlocksBindingFromAnyKnowledgeDomain(t *testing.T) {
	ctx := context.Background()
	svc, db, registry := newGuardTestService(t)
	insertGuardStore(t, db, "store-A", 1)
	storeID := "store-A"
...
func TestDeleteStore_Guard_RejectsCount3(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		kb := &types.KnowledgeBase{
			ID:   "kb-multi-" + knowledgeDomainID2s(1) + "-" + ptrOrEmpty(&storeID) + "-" + string(rune('a'+i)),
			Name: "kb", KnowledgeDomainID: 1, EmbeddingModelID: "e", SummaryModelID: "s",
...
func TestResolveStoreView(t *testing.T) {
	...
	t.Run("global store is available across knowledge domains", func(t *testing.T) {
