...
func TestClassifyFactoryError_KnowledgeDomainInfoMissingMaps(t *testing.T) {
	t.Parallel()
	err := classifyFactoryError(context.Background(),
		retriever.ErrKnowledgeDomainInfoMissing, 0, "")
	app, ok := apperrors.IsAppError(err)
	require.True(t, ok)
	assert.Equal(t, apperrors.ErrVectorStoreBindingInvalid, app.Code)
}
...
func TestValidateSameEmbeddingModel_SingleKB_NoOp(t *testing.T) {
	...
	err := svc.validateSameEmbeddingModel(context.Background(), []*types.KnowledgeBase{
		{ID: "kb-1", EmbeddingModelID: "m-1", KnowledgeDomainID: 1},
	})
...
func TestValidateSameEmbeddingModel_SameIdentity_OK(t *testing.T) {
	...
	err := svc.validateSameEmbeddingModel(context.Background(), []*types.KnowledgeBase{
		{ID: "kb-1", EmbeddingModelID: "m-1", KnowledgeDomainID: 1},
		{ID: "kb-2", EmbeddingModelID: "m-2", KnowledgeDomainID: 2}, // cross-knowledgeDomain
	})
	assert.NoError(t, err)
}
...
func TestValidateSameEmbeddingModel_DifferentIdentity_400(t *testing.T) {
	...
	err := svc.validateSameEmbeddingModel(context.Background(), []*types.KnowledgeBase{
		{ID: "kb-1", EmbeddingModelID: "m-1", KnowledgeDomainID: 1},
		{ID: "kb-2", EmbeddingModelID: "m-2", KnowledgeDomainID: 1},
	})
	app, ok := apperrors.IsAppError(err)
	require.True(t, ok, "expected AppError, got %T", err)
...
func TestValidateSameEmbeddingModel_NonEmbeddingKB_Tolerated(t *testing.T) {
	t.Parallel()
	models := map[string]*types.Model{
		"m-1": buildModel("m-1", "bge-m3", "http://embed-a"),
	}
	svc := newKBSvcForValidate(&fakeModelSvcForKeys{byID: models})
	// One vector KB plus one non-embedding KB is tolerated.
	err := svc.validateSameEmbeddingModel(context.Background(), []*types.KnowledgeBase{
		{ID: "kb-vec", EmbeddingModelID: "m-1", KnowledgeDomainID: 1},
		{ID: "kb-graph", EmbeddingModelID: "", KnowledgeDomainID: 1},
	})
	assert.NoError(t, err)
}
...
func TestRetrieveFromStores_SingleGroupFastPath(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, res, 1)
	assert.Equal(t, types.SQLiteRetrieverEngineType, res[0].RetrieverEngineType)
...
func TestRetrieveFromStores_MultiGroupParallel_Concat(t *testing.T) {
	...
	fakeB := &fakeRetrieveEngineService{
		engineType: types.SQLiteRetrieverEngineType, // same engine type
		support:    []types.RetrieverType{types.VectorRetrieverType},
...
func TestRetrieveFromStores_MixedEngine_Normalizes(t *testing.T) {
	...
	fakeSQLite := &fakeRetrieveEngineService{
		engineType: types.SQLiteRetrieverEngineType,
		support:    []types.RetrieverType{types.VectorRetrieverType},
		canned:     []*types.IndexWithScore{{ChunkID: "sqlite1", Score: 0.9}},
	}
	groups := []*storeGroup{
		{Engine: buildBoundComposite(t, fakeES), BaseParams: vectorParams("q"), TopK: 50, KBIDs: []string{"kb-es"}},
		{Engine: buildBoundComposite(t, fakeSQLite), BaseParams: vectorParams("q"), TopK: 50, KBIDs: []string{"kb-sqlite"}},
	}
	...
	assert.InDelta(t, 0.9, scoresByChunk["sqlite1"], 1e-9)
	...
	fakeSQLite2 := &fakeRetrieveEngineService{
		engineType: types.SQLiteRetrieverEngineType,
		support:    []types.RetrieverType{types.VectorRetrieverType},
		canned:     []*types.IndexWithScore{{ChunkID: "sqlite2", Score: 0.8}},
	}
	groups2 := []*storeGroup{
		{Engine: buildBoundComposite(t, fakeES2), BaseParams: vectorParams("q"), TopK: 50, KBIDs: []string{"kb-es2"}},
		{Engine: buildBoundComposite(t, fakeSQLite2), BaseParams: vectorParams("q"), TopK: 50, KBIDs: []string{"kb-sqlite2"}},
	}
	...
	assert.InDelta(t, 0.8, scoresByChunk2["sqlite2"], 1e-9)
	...
	fakeSQLite3 := &fakeRetrieveEngineService{
		engineType: types.SQLiteRetrieverEngineType,
		support:    []types.RetrieverType{types.VectorRetrieverType},
		canned:     []*types.IndexWithScore{{ChunkID: "sqlite3", Score: 0.8}},
	}
	groups3 := []*storeGroup{
		{Engine: buildBoundComposite(t, fakeMilvus), BaseParams: vectorParams("q"), TopK: 50, KBIDs: []string{"kb-mv"}},
		{Engine: buildBoundComposite(t, fakeSQLite3), BaseParams: vectorParams("q"), TopK: 50, KBIDs: []string{"kb-sqlite3"}},
	}
	...
	assert.InDelta(t, 0.8, scoresByChunk3["sqlite3"], 1e-9)
}
...
func TestRetrieveFromStores_KeywordPassthroughOnMixed(t *testing.T) {
	...
	fakeSQLiteKW := &fakeRetrieveEngineService{
		engineType: types.SQLiteRetrieverEngineType,
	...
	groups := []*storeGroup{
		{Engine: buildBoundComposite(t, fakeES), BaseParams: vectorParams("q"), TopK: 50, KBIDs: []string{"kb-es"}},
		{Engine: buildBoundComposite(t, fakeSQLiteKW), BaseParams: []types.RetrieveParams{{Query: "q", TopK: 50, RetrieverType: types.KeywordsRetrieverType}}, TopK: 50, KBIDs: []string{"kb-sqlite"}},
	}
	s := &knowledgeBaseService{}
...
func ctxWithKnowledgeDomainForAuth(knowledgeDomainID uint64) context.Context {
	return context.WithValue(context.Background(), types.KnowledgeDomainIDContextKey, knowledgeDomainID)
}

func TestAuthorizeKBAccess_SameKnowledgeDomainAllPass(t *testing.T) {
	t.Parallel()
	s := &knowledgeBaseService{}
	kbs := []*types.KnowledgeBase{
		{ID: "kb-1", KnowledgeDomainID: 7},
		{ID: "kb-2", KnowledgeDomainID: 7},
	}
	err := s.authorizeKBAccess(ctxWithKnowledgeDomainForAuth(7), kbs, 7, types.SearchParams{})
	require.NoError(t, err)
}

func TestAuthorizeKBAccess_ForeignKnowledgeDomain_NotFound(t *testing.T) {
	t.Parallel()
	s := &knowledgeBaseService{}
	kbs := []*types.KnowledgeBase{
		{ID: "kb-foreign", KnowledgeDomainID: 99},
	}
	err := s.authorizeKBAccess(ctxWithKnowledgeDomainForAuth(7), kbs, 7, types.SearchParams{})
	require.Error(t, err)
	app, ok := apperrors.IsAppError(err)
...
func TestAuthorizeKBAccess_EmptyKBs_OK(t *testing.T) {
	t.Parallel()
	s := &knowledgeBaseService{}
	err := s.authorizeKBAccess(ctxWithKnowledgeDomainForAuth(7), nil, 7, types.SearchParams{})
	require.NoError(t, err)
}
...
func TestApplyFAQPostProcessing_PropagatesError(t *testing.T) {
	...
	kb := &types.KnowledgeBase{
		...
		EmbeddingModelID: "m-1", KnowledgeDomainID: 1,
	}
