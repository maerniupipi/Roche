...
func (s *knowledgeService) UpsertFAQEntries(ctx context.Context,
	...
	if taskID == "" {
		taskID = secutils.GenerateTaskID("faq_import", knowledgeDomainID, kbID)
	}
	...
	if entryCount > entryCountThreshold {
		...
		entriesURL, err := s.fileSvc.SaveBytes(ctx, entriesData, knowledgeDomainID, fileName, false)
		if err != nil {
...
func (s *knowledgeService) generateFailedEntriesCSV(ctx context.Context,
	knowledgeDomainID uint64, taskID string, failedEntries []types.FAQFailedEntry,
) (string, error) {
	// 生成 CSV 内容
	...
	filePath, err := s.fileSvc.SaveBytes(ctx, []byte(buf.String()), knowledgeDomainID, fileName, true)
	if err != nil {
...
func (s *knowledgeService) saveFAQImportResultToDatabase(ctx context.Context,
	...
	// 获取FAQ知识库实例
	knowledgeDomainID := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64)
	knowledge, err := s.repo.GetKnowledgeByID(ctx, knowledgeDomainID, payload.KnowledgeID)
	if err != nil {
...
func (s *knowledgeService) executeFAQDryRunValidation(ctx context.Context,
	...
	if payload.Mode == types.FAQBatchModeAppend {
		validEntryIndices = s.validateEntriesForAppendModeWithProgress(ctx, payload.KnowledgeDomainID, payload.KBID, entries, progress)
	} else {
...
func (s *knowledgeService) validateEntriesForAppendModeWithProgress(ctx context.Context,
	knowledgeDomainID uint64, kbID string, entries []types.FAQEntryPayload, progress *types.FAQImportProgress,
) []int {
	validIndices := make([]int, 0, len(entries))

	// 查询知识库中已有的所有FAQ chunks的metadata
	existingChunks, err := s.chunkRepo.ListAllFAQChunksWithMetadataByKnowledgeBaseID(ctx, knowledgeDomainID, kbID)
	if err != nil {
...
func (s *knowledgeService) calculateAppendOperations(ctx context.Context,
	knowledgeDomainID uint64, kbID string, entries []types.FAQEntryPayload,
...
func (s *knowledgeService) calculateReplaceOperations(ctx context.Context,
	knowledgeDomainID uint64, knowledgeID string, newEntries []types.FAQEntryPayload,
	...
	// 查询所有已存在的chunks
	allExistingChunks, err := s.chunkRepo.ListAllFAQChunksByKnowledgeID(ctx, knowledgeDomainID, knowledgeID)
	if err != nil {
...
func (s *knowledgeService) executeFAQImport(ctx context.Context, taskID string, kbID string,
	payload *types.FAQBatchUpsertPayload, knowledgeDomainID uint64, processedCount int,
	progress *types.FAQImportProgress,
) (err error) {
	...
	if payload.Mode == types.FAQBatchModeReplace {
		...
		if len(chunksToDelete) > 0 {
			...
			if err := s.chunkRepo.DeleteChunks(ctx, knowledgeDomainID, chunkIDsToDelete); err != nil {
		...
		entriesToProcess, skippedCount, err = s.calculateAppendOperations(ctx, knowledgeDomainID, kb.ID, payload.Entries)
		if err != nil {
	...
	for i := 0; i < remainingEntries; i += faqImportBatchSize {
		...
		for idx, chunk := range chunks {
			...
			tagName := ""
			if chunk.TagID != "" {
				if tag, err := s.tagRepo.GetByID(ctx, knowledgeDomainID, chunk.TagID); err == nil && tag != nil {
					tagID = tag.SeqID
					tagName = tag.Name
				}
			}
...
func (s *knowledgeService) indexFAQChunks(ctx context.Context,
	...
	if adjustStorage {
		...
		if knowledgeDomainInfo.StorageQuota > 0 && knowledgeDomainInfo.StorageUsed+size > knowledgeDomainInfo.StorageQuota {
	...
	if adjustStorage && size > 0 {
		...
		if err := s.knowledgeDomainRepo.AdjustStorageUsed(ctx, knowledgeDomainInfo.ID, size); err == nil {
			knowledgeDomainInfo.StorageUsed += size
		}
...
func (s *knowledgeService) ProcessFAQImport(ctx context.Context, t *asynq.Task) error {
	...
	chunksDeleted, err := s.chunkRepo.DeleteUnindexedChunks(ctx, payload.KnowledgeDomainID, payload.KnowledgeID)
	if err != nil {
	...
	if err := s.executeFAQImport(ctx, payload.TaskID, payload.KBID, faqPayload, payload.KnowledgeDomainID, progress.FailedCount+processedCount, progress); err != nil {
...
func (s *knowledgeService) finalizeFAQValidation(ctx context.Context, payload *types.FAQImportPayload,
	...
	if len(progress.FailedEntries) > 0 {
		csvURL, err := s.generateFailedEntriesCSV(ctx, payload.KnowledgeDomainID, payload.TaskID, progress.FailedEntries)
		if err != nil {
	...
	if !payload.DryRun {
		...
		if payload.Mode == types.FAQBatchModeReplace {
			deletedTags, err := s.tagRepo.DeleteUnusedTags(ctx, payload.KnowledgeDomainID, payload.KBID)
			if err != nil {
...
func (s *knowledgeService) GetFAQImportProgress(ctx context.Context, taskID string) (*types.FAQImportProgress, error) {
	...
	if progress.Status == types.FAQImportStatusCompleted && progress.KnowledgeID != "" {
		knowledgeDomainID := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64)
		knowledge, err := s.repo.GetKnowledgeByID(ctx, knowledgeDomainID, progress.KnowledgeID)
...
func (s *knowledgeService) UpdateLastFAQImportResultDisplayStatus(ctx context.Context, kbID string, displayStatus string) error {
	...
	// 查找FAQ类型的knowledge
	knowledgeList, err := s.repo.ListKnowledgeByKnowledgeBaseID(ctx, knowledgeDomainID, kbID)
	if err != nil {
