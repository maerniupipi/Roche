...
func (s *knowledgeService) ListFAQEntries(ctx context.Context,
	...
	if kb.KnowledgeDomainID != knowledgeDomainID {
		return nil, werrors.NewForbiddenError("knowledge base is outside the current knowledgeDomain")
	}
	effectiveKnowledgeDomainID := knowledgeDomainID

	faqKnowledge, err := s.findFAQKnowledge(ctx, effectiveKnowledgeDomainID, kb.ID)
	if err != nil {
		return nil, err
	}
	if faqKnowledge == nil {
	...
	var tagID string
	if tagSeqID > 0 {
		tag, err := s.tagRepo.GetBySeqID(ctx, effectiveKnowledgeDomainID, tagSeqID)
		if err != nil {
	...
	chunks, total, err := s.chunkRepo.ListPagedChunksByKnowledgeID(
		ctx, effectiveKnowledgeDomainID, faqKnowledge.ID, page, chunkType, tagID, keyword, searchField, sortOrder, types.KnowledgeTypeFAQ,
	)
	if err != nil {
		return nil, err
	}
	...
	if len(tagIDs) > 0 {
		tags, err := s.tagRepo.GetByIDs(ctx, effectiveKnowledgeDomainID, tagIDs)
		if err == nil {
...
func (s *knowledgeService) CreateFAQEntry(ctx context.Context,
	...
	// 检查标准问和相似问是否与其他条目重复
	if err := s.checkFAQQuestionDuplicate(ctx, knowledgeDomainID, kb.ID, "", meta); err != nil {
		return nil, err
	}

	// 确保FAQ Knowledge存在
	faqKnowledge, err := s.ensureFAQKnowledge(ctx, knowledgeDomainID, kb)
	if err != nil {
	...
	tagSeqIDMap := make(map[string]int64)
	if chunk.TagID != "" {
		tag, tagErr := s.tagRepo.GetByID(ctx, knowledgeDomainID, chunk.TagID)
...
func (s *knowledgeService) GetFAQEntry(ctx context.Context,
	...
	// 获取chunk by seq_id
	chunk, err := s.chunkRepo.GetChunkBySeqID(ctx, knowledgeDomainID, entrySeqID)
	if err != nil {
		return nil, werrors.NewNotFoundError("FAQ条目不存在")
	}

	// 验证chunk属于当前知识库
	if chunk.KnowledgeBaseID != kb.ID || chunk.KnowledgeDomainID != knowledgeDomainID {
		return nil, werrors.NewNotFoundError("FAQ条目不存在")
	}
...
func (s *knowledgeService) UpdateFAQEntry(ctx context.Context,
	...
	// 检查标准问和相似问是否与其他条目重复
	if err := s.checkFAQQuestionDuplicate(ctx, knowledgeDomainID, kb.ID, chunk.ID, meta); err != nil {
		return nil, err
	}
	...
	if payload.TagID > 0 {
		tag, tagErr := s.tagRepo.GetBySeqID(ctx, knowledgeDomainID, payload.TagID)
		if tagErr != nil {
			return nil, werrors.NewNotFoundError("标签不存在")
		}
		chunk.TagID = tag.ID
	...
	faqKnowledge, err := s.repo.GetKnowledgeByID(ctx, knowledgeDomainID, chunk.KnowledgeID)
	if err != nil {
		return nil, err
	}
	...
	if questionIndexMode == types.FAQQuestionIndexModeSeparate && len(oldSimilarQuestions) > 0 {
		...
		if questionIndexMode == types.FAQQuestionIndexModeSeparate && oldSimilarQuestionCount > newSimilarQuestionCount {
			retrieveEngine, engineErr := retriever.CreateRetrieveEngineForKB(
				ctx, s.retrieveEngine, s.ownership, types.MustKnowledgeDomainIDFromContext(ctx), kb.VectorStoreID)
			if engineErr == nil {
...
func (s *knowledgeService) AddSimilarQuestions(ctx context.Context,
	...
	if err := s.checkFAQQuestionDuplicate(ctx, knowledgeDomainID, kb.ID, chunk.ID, tempMeta); err != nil {
		return nil, err
	}
...
func (s *knowledgeService) UpdateFAQEntryStatus(ctx context.Context,
	...
	knowledgeDomainID := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64)
	chunk, err := s.chunkRepo.GetChunkByID(ctx, knowledgeDomainID, entryID)
	if err != nil {
		return err
	}
	...
	retrieveEngine, err := retriever.CreateRetrieveEngineForKB(
		ctx, s.retrieveEngine, s.ownership, knowledgeDomainID, kb.VectorStoreID)
	if err != nil {
		return err
	}
...
func (s *knowledgeService) UpdateFAQEntryFieldsBatch(ctx context.Context,
	...
	if len(req.ExcludeIDs) > 0 {
		excludeChunks, err := s.chunkRepo.ListChunksBySeqID(ctx, knowledgeDomainID, req.ExcludeIDs)
		if err == nil {
	...
	if len(req.ByTag) > 0 {
		for tagSeqID, update := range req.ByTag {
			// Convert tag seq_id to UUID
			tag, err := s.tagRepo.GetBySeqID(ctx, knowledgeDomainID, tagSeqID)
			if err != nil {
			...
			if update.TagID != nil {
				if *update.TagID > 0 {
					newTag, err := s.tagRepo.GetBySeqID(ctx, knowledgeDomainID, *update.TagID)
					if err != nil {
			...
			affectedIDs, err := s.chunkRepo.UpdateChunkFieldsByTagID(
				ctx, knowledgeDomainID, kb.ID, tag.ID,
	...
	if len(req.ByID) > 0 {
		...
		chunks, err := s.chunkRepo.ListChunksBySeqID(ctx, knowledgeDomainID, entrySeqIDs)
		if err != nil {
			return err
		}
		...
		if len(setFlags) > 0 || len(clearFlags) > 0 {
			if err := s.chunkRepo.UpdateChunkFlagsBatch(ctx, knowledgeDomainID, kb.ID, setFlags, clearFlags); err != nil {
				return err
			}
		}
	}
...
func (s *knowledgeService) UpdateFAQEntryTag(ctx context.Context, kbID string, entryID string, tagID *string) error {
	...
	if tagID != nil && *tagID != "" {
		tag, err := s.tagRepo.GetByID(ctx, knowledgeDomainID, *tagID)
		if err != nil {
			return err
		}
...
func (s *knowledgeService) UpdateFAQEntryTagBatch(ctx context.Context, kbID string, updates map[int64]*int64) error {
	...
	if len(tagSeqIDSet) > 0 {
		...
		tags, err := s.tagRepo.GetBySeqIDs(ctx, knowledgeDomainID, tagSeqIDs)
		if err != nil {
			return err
		}
		for _, tag := range tags {
...
func (s *knowledgeService) SearchFAQEntries(ctx context.Context,
	...
	if len(req.FirstPriorityTagIDs) > 0 {
		tags, err := s.tagRepo.GetBySeqIDs(ctx, knowledgeDomainID, req.FirstPriorityTagIDs)
		if err == nil {
	...
	if len(req.SecondPriorityTagIDs) > 0 {
		tags, err := s.tagRepo.GetBySeqIDs(ctx, knowledgeDomainID, req.SecondPriorityTagIDs)
		if err == nil {
	...
	// Batch fetch chunks
	chunks, err := s.chunkRepo.ListChunksByID(ctx, knowledgeDomainID, chunkIDs)
	if err != nil {
		return nil, err
	}
	...
	if len(tagIDs) > 0 {
		tags, err := s.tagRepo.GetByIDs(ctx, knowledgeDomainID, tagIDs)
		if err == nil {
			for _, tag := range tags {
				tagSeqIDMap[tag.ID] = tag.SeqID
			}
...
func (s *knowledgeService) DeleteFAQEntries(ctx context.Context,
	...
	for _, seqID := range entrySeqIDs {
		...
		chunk, err := s.chunkRepo.GetChunkBySeqID(ctx, knowledgeDomainID, seqID)
		if err != nil {
...
func (s *knowledgeService) ExportFAQEntries(ctx context.Context, kbID string) ([]byte, error) {
	...
	knowledgeDomainID := ctx.Value(types.KnowledgeDomainIDContextKey).(uint64)
	faqKnowledge, err := s.findFAQKnowledge(ctx, knowledgeDomainID, kb.ID)
	if err != nil {
		return nil, err
	}
	if faqKnowledge == nil {
	...
	// Get all FAQ chunks
	chunks, err := s.chunkRepo.ListAllFAQChunksForExport(ctx, knowledgeDomainID, faqKnowledge.ID)
	if err != nil {
		return nil, fmt.Errorf("failed to list FAQ chunks: %w", err)
	}

	// Build tag map for tag_id -> tag_name conversion
	tagMap, err := s.buildTagMap(ctx, knowledgeDomainID, kbID)
	if err != nil {
...
func (s *knowledgeService) buildTagMap(ctx context.Context, knowledgeDomainID uint64, kbID string) (map[string]string, error) {
	const pageSize = 1000
	tagMap := make(map[string]string)

	for pageNum := 1; ; pageNum++ {
		page := &types.Pagination{Page: pageNum, PageSize: pageSize}
		tags, _, err := s.tagRepo.ListByKB(ctx, knowledgeDomainID, kbID, page, "")
		if err != nil {
			return nil, err
		}
		for _, tag := range tags {
...
func (s *knowledgeService) ensureFAQKnowledge(
	...
	existing, err := s.findFAQKnowledge(ctx, knowledgeDomainID, kb.ID)
	if err != nil {
		return nil, err
	}
	if existing != nil {
...
func (s *knowledgeService) checkFAQQuestionDuplicate(
	...
	dupChunk, err := s.chunkRepo.FindFAQChunkWithDuplicateQuestion(ctx, knowledgeDomainID, kbID, excludeChunkID, allQuestions)
	if err != nil {
...
func (s *knowledgeService) resolveTagID(ctx context.Context, kbID string, payload *types.FAQEntryPayload) (string, error) {
	...
	if payload.TagID != 0 {
		tag, err := s.tagRepo.GetBySeqID(ctx, knowledgeDomainID, payload.TagID)
		if err != nil {
