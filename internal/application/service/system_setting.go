...
// suffix ROCHE_KAP_REDIS_NAMESPACE so two deployments sharing one Redis
// instance don't cross-talk.
const pubsubChannelBase = "rochekap:system_settings:changed"
...
var registry = map[string]settingSpec{
	...
	// knowledge_domain.default_storage_quota_gb is the default storage quota (in GB)
	// applied to a newly-created knowledgeDomain when the caller doesn't specify
	// one explicitly. Read at create time only — changing the value does
	// NOT retroactively resize already-existing knowledgeDomains (they keep the
	// quota stored on their row at creation; superusers can edit
	// individual knowledgeDomains via the existing knowledgeDomain-update path).
	// 0 or negative = use the in-code default (10 GB).
	"knowledge_domain.default_storage_quota_gb": {
		Type:     "int",
		EnvName:  "ROCHE_KAP_KNOWLEDGE_DOMAIN_DEFAULT_STORAGE_QUOTA_GB",
		Default:  int64(10),
		Category: "knowledgeDomain",
		Description: "新建知识域时默认分配的存储配额（GB），包含向量、原文、文本、索引等。" +
	...
	// ROCHE_KAP_ASYNQ_CONCURRENCY (default 32).
	"asynq.concurrency": {
		Type:            "int",
		EnvName:         "ROCHE_KAP_ASYNQ_CONCURRENCY",
		Default:         int64(32),
		Category:        "worker",
...
// (matches the access-audit convention — tests that don't care
...
// This mirrors the access-audit emitter's failure semantics: the
