...
func TestOIDCRejectsUnverifiedIDTokenWithoutUserInfo(t *testing.T) {
	svc := &userService{}
	_, err := svc.resolveOIDCUserInfo(
		context.Background(),
		...
		&oidcTokenResponse{
			IDToken: "header.eyJzdWIiOiJhdHRhY2tlciJ9.signature",
		},
	)
	if err == nil || !strings.Contains(err.Error(), "userinfo endpoint is required") {
