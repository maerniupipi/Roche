...
// Service-layer chain helper retained for legacy creator lookups. The
// handler-level adapters in internal/handler/rbac_lookups.go
...
type stubKnowledgeRepoForChain struct {
	...
	getByID func(ctx context.Context, knowledgeDomainID uint64, id string) (*types.Knowledge, error)
}

func (s *stubKnowledgeRepoForChain) GetKnowledgeByID(
	ctx context.Context, knowledgeDomainID uint64, id string,
) (*types.Knowledge, error) {
	return s.getByID(ctx, knowledgeDomainID, id)
}
...
func chainCtx(knowledgeDomainID uint64) context.Context {
...
func TestGetOwningKBCreatorID_HappyPathReturnsKBCreator(t *testing.T) {
	s := newKnowledgeServiceForChain(
		&stubKnowledgeRepoForChain{
			getByID: func(_ context.Context, knowledgeDomainID uint64, _ string) (*types.Knowledge, error) {
				if knowledgeDomainID != 1 {
					t.Fatalf("expected knowledgeDomain context to flow into repo (got %d)", knowledgeDomainID)
				}
				return &types.Knowledge{ID: "kn-1", KnowledgeBaseID: "kb-1"}, nil
			},
		},
		&stubKBServiceForChain{
			getByID: func(_ context.Context, id string) (*types.KnowledgeBase, error) {
				...
				return &types.KnowledgeBase{ID: "kb-1", KnowledgeDomainID: 1, CreatorID: "u-creator"}, nil
			},
		},
	)
...
func TestGetOwningKBCreatorID_LegacyKBHasEmptyCreatorID(t *testing.T) {
	...
	// middleware's "" -> knowledgeDomain-owned branch fires (Admin+ only).
	s := newKnowledgeServiceForChain(
		&stubKnowledgeRepoForChain{
			getByID: func(_ context.Context, _ uint64, _ string) (*types.Knowledge, error) {
		...
		&stubKBServiceForChain{
			getByID: func(_ context.Context, _ string) (*types.KnowledgeBase, error) {
				return &types.KnowledgeBase{ID: "kb-legacy", KnowledgeDomainID: 1, CreatorID: ""}, nil
			},
		},
	)
	creator, err := s.GetOwningKBCreatorID(chainCtx(1), "kn-1")
