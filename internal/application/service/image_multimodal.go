...
type ImageMultimodalService struct {
	...
	// when the knowledgeDomain-scoped storage config cannot produce a usable service
	// (e.g. images were saved using the global MINIO_* env vars while the
	// knowledgeDomain's StorageEngineConfig.MinIO is empty). Mirrors the write-side
...
func (s *ImageMultimodalService) indexChunks(ctx context.Context, payload types.ImageMultimodalPayload, chunks []*types.Chunk) {
	...
	// (e.g. graph-only KBs). Without this check, GetEmbeddingModel would fail because
	...
	if err != nil {
		logger.Warnf(ctx, "[ImageMultimodal] Failed to get knowledgeDomain for indexing: %v", err)
		return
	}
	// The factory's unbound path reads KnowledgeDomainInfo from ctx; make sure it's there.
	...
	// (nil → knowledgeDomain effective engines fallback; verified knowledgeDomain ownership otherwise).
	engine, err := retriever.CreateRetrieveEngineForKB(
		ctx, s.retrieveEngine, s.ownership, payload.KnowledgeDomainID, kb.VectorStoreID)
	if err != nil {
...
func (s *ImageMultimodalService) resolveVLM(ctx context.Context, kbID, knowledgeID string) (vlm.VLM, types.VLMConfig, error) {
	...
	model, err := vlm.NewVLMFromLegacyConfig(vlmCfg)
	return model, vlmCfg, err
}

// resolveFileServiceForPayload resolves knowledgeDomain/KB scoped file service for reading provider:// URLs.
// Falls back to the globally configured default FileService when the knowledgeDomain's
...
// env vars while knowledgeDomain.StorageEngineConfig.MinIO is left empty (issue #1282).
func (s *ImageMultimodalService) resolveFileServiceForPayload(ctx context.Context, payload types.ImageMultimodalPayload) interfaces.FileService {
	...
	if err != nil || knowledgeDomain == nil {
		logger.Warnf(ctx, "[ImageMultimodal] GetKnowledgeDomainByID failed: knowledgeDomain=%d err=%v", payload.KnowledgeDomainID, err)
		return s.fileSvc
	}
	...
	fileSvc, _, svcErr := filesvc.NewFileServiceFromStorageConfig(provider, knowledgeDomain.StorageEngineConfig, baseDir)
	if svcErr != nil {
		logger.Warnf(ctx, "[ImageMultimodal] resolve file service failed (falling back to default): knowledgeDomain=%d provider=%s err=%v",
			payload.KnowledgeDomainID, provider, svcErr)
		return s.fileSvc
	}
	return fileSvc
}
