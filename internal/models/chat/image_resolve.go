...
// local:// storage URL to its bytes using the owning knowledgeDomain's storage config.
// Stored local:// URLs are relative to the storage base dir and do NOT encode
// the knowledgeDomain's configured PathPrefix, so a plain env-based join would miss the
