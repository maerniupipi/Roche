...
//   - ROCHE_KAP_LLM_CHAT_TIMEOUT_SECONDS    非流式调用兜底超时（默认 600s）
//   - ROCHE_KAP_LLM_STREAM_TIMEOUT_SECONDS  流式调用兜底超时（默认 1800s）
var (
	defaultChatTimeout   = envDurationSeconds("ROCHE_KAP_LLM_CHAT_TIMEOUT_SECONDS", 300*time.Second)
	defaultStreamTimeout = envDurationSeconds("ROCHE_KAP_LLM_STREAM_TIMEOUT_SECONDS", 600*time.Second)
)
