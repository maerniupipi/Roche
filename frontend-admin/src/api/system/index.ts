import { get, post, put, del, getDownWithName } from '@/utils/request'
...
export function getPromptTemplates(): Promise<{ data: PromptTemplatesConfig }> {
  return get('/api/v1/system/runtime-config/prompt-templates')
}
...
export function getParserEngineConfig(): Promise<{ data: ParserEngineConfig }> {
  return get('/api/v1/system/runtime-config/parser-engine-config')
}

export function updateParserEngineConfig(config: ParserEngineConfig): Promise<{ data: ParserEngineConfig }> {
  return put('/api/v1/system/runtime-config/parser-engine-config', config)
}
...
// ---- 平台级存储引擎配置（供文档/图片存储与 docreader 使用） ----
...
export function getStorageEngineConfig(): Promise<{ data: StorageEngineConfig }> {
  return get('/api/v1/system/runtime-config/storage-engine-config')
}

export function updateStorageEngineConfig(config: StorageEngineConfig): Promise<{ data: StorageEngineConfig }> {
  return put('/api/v1/system/runtime-config/storage-engine-config', config)
}
...
/**
 * List every system setting row (system-scope, not knowledgeDomain-scope).
...
/**
 * Result of POST /system/admin/knowledge-domains/apply-default-storage-quota.
 * `affected` is the count of knowledgeDomain rows whose storage_quota was
...
/**
 * Apply the current `knowledge_domain.default_storage_quota_gb` setting to every
 * existing knowledgeDomain. Reads the resolved setting server-side (DB > ENV >
 ...
 */
export async function applyDefaultStorageQuotaToAllKnowledgeDomains(): Promise<ApplyDefaultStorageQuotaResult> {
  const response = await post('/api/v1/system/admin/knowledge-domains/apply-default-storage-quota')
  return response as unknown as ApplyDefaultStorageQuotaResult
}

// ---- Platform Audit Log (system-scope) ----

// We reuse the AuditLog / ListAuditLogParams types from the knowledgeDomain
// audit-log module — the row shape is identical, only the route
// differs (knowledge_domain_id=0 rows aren't visible via the per-knowledgeDomain endpoint).
// Re-exported here so SystemSettings.vue doesn't need to cross-import
// from a knowledgeDomain-specific module to consume system-scope feeds.
export type {
  AuditLog,
  AuditAction,
  AuditOutcome,
...
} from '@/api/knowledge-domain/audit-log'

import type { ListAuditLogParams, ListAuditLogResponse } from '@/api/knowledge-domain/audit-log'

/**
 * List the platform-wide audit log (system-scope, knowledge_domain_id=0).
 *
...
export type AnswerRecordChannel = 'web' | 'app' | ''

export type AnswerRecordFeedbackRating = 'like' | 'dislike' | 'none'

export interface AnswerRecordFeedbackDetail {
  rating: AnswerRecordFeedbackRating
  reason?: string
  reason_zh?: string
  reason_en?: string
  comment?: string
...
export interface AnswerRecordListParams {
...
function buildAnswerRecordsQuery(params: AnswerRecordListParams): string {
  const qs = new URLSearchParams()
  if (params.channel) qs.append('channel', params.channel)
  if (params.username) qs.append('username', params.username)
  if (params.feedback) qs.append('feedback', params.feedback)
...
/**
 * 管理端问答记录列表分页。
 * 后端：GET /api/v1/system/admin/answer-records
 */
export async function listAnswerRecords(
  params: AnswerRecordListParams = {},
): Promise<AnswerRecordListResponse> {
  const tail = buildAnswerRecordsQuery(params)
  const url = `/api/v1/system/admin/answer-records${tail ? '?' + tail : ''}`
  return (await get(url)) as unknown as AnswerRecordListResponse
}

/**
 * 导出问答记录（CSV），从响应头 Content-Disposition 解析真实文件名。
 * 后端：GET /api/v1/system/admin/answer-records/export
 */
export async function exportAnswerRecords(
  params: AnswerRecordListParams = {},
): Promise<{ blob: Blob; filename: string }> {
  const tail = buildAnswerRecordsQuery(params)
  const url = `/api/v1/system/admin/answer-records/export${tail ? '?' + tail : ''}`
  return getDownWithName(url)
}
...
/**
 * 读取当前生效的汇率配置。
 * 后端：GET /api/v1/exchange-rate
 * - 任意已登录用户可调用
 * - 后端无配置时返回的 data 可能为 undefined，前端需兼容空表单
 */
export async function getExchangeRate(): Promise<ExchangeRateConfig | null> {
  const res = (await get('/api/v1/exchange-rate')) as unknown as {
    success?: boolean
