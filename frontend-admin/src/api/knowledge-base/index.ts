...
export function listKnowledgeBases(params?: {
   ...
   *   - "others" → only KBs created by someone else in this knowledgeDomain
...
//
//   - source 'env'    → KB uses the knowledgeDomain's env-configured store
...
//                       reports the underlying engine (e.g. "milvus").
//   - source 'user'   → KB is bound to a knowledgeDomain-owned VectorStore.
...
export function createKnowledgeBase(data: {
  ...
  // Opt-in binding to a specific knowledgeDomain-owned VectorStore. Omit (or
...
export function createKnowledgeFromURL(
  kbId: string,
  data: { url: string; enable_multimodel?: boolean; process_config?: KnowledgeProcessOverrides },
) {
