...
// 知识库是否被智能体锁定（kb_selection_mode = 'none' 时完全禁用知识库）
// @ 提及是否被智能体完全禁用（流式模式下仅在所有可选项都被禁用时才显示锁定提示）
const isMentionDisabled = computed(() => {
  if (!settingsStore.isAgentStreamMode || !hasAgentConfig.value) return false;
  return agentSkillsSelectionMode.value === 'none';
});

const isKnowledgeBaseDisabledByAgent = computed(() => false);
  ...
  <div class="answers-input" @drop="onDrop" @dragover="onDragOver">
    ...
    <div class="rich-input-container" data-guide="chat-input">
      ...
      <div class="control-bar">
        ...
        <div class="control-left">
          <!-- Agent 模式切换按钮 -->
          <div ref="agentModeButtonRef" v-if="isDebugger" class="control-btn agent-mode-btn"
            :class="{ 'is-normal': !isCustomAgent && !isAgentEnabled, 'is-agent': !isCustomAgent && isAgentEnabled, 'is-custom': isCustomAgent }"
          ...
          <!-- 图片上传按钮（智能体未启用时不显示） -->
          <t-tooltip v-if="showImageUploadButton && isDebugger" placement="top" theme="light"
          ...
          <!-- 附件上传按钮 -->
          <t-tooltip v-if="isDebugger" placement="top" theme="light"
            :popupProps="{ overlayClassName: 'input-field-tooltip' }">
            <template #content>
          ...
          <t-tooltip v-if="isDebugger" placement="top" theme="light"
            ...
            <template #content>
              ...
              <span v-else>
                {{ allSelectedItems.length > 0 ? $t('input.knowledgeBaseWithCount', { count: allSelectedItems.length })
          ...
          <!-- 模型显示 -->
          <t-tooltip v-if="isDebugger" :content="isModelLockedByAgent ? $t('input.modelLockedByAgent') : ''"
    ...
    <Teleport to="body">
      <MentionSelector ref="mentionSelectorRef" :visible="showMention" :style="mentionStyle" :items="mentionItems"
        :hasMore="mentionHasMore" :loading="mentionLoading" :emptyHint="mentionEmptyHint" :query="mentionQuery"
        :group-counts="mentionGroupCounts" v-model:activeIndex="mentionActiveIndex" @select="onMentionSelect"
