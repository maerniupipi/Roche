...
/**
 * 下载场景：在 blob 之外从响应头 Content-Disposition 解析出文件名，
 * 让浏览器下载时展示正确文件名（中文走 RFC 5987 的 filename* 编码）。
 * 不修改既有 getDown，避免影响其他调用方。
 */
export async function getDownWithName(
  url: string,
  fallbackName = 'answer-records.csv',
): Promise<{ blob: Blob; filename: string }> {
  const res = (await instance.get<Blob>(url, { responseType: 'blob' })) as unknown as {
    data: Blob
    headers: Record<string, string> | undefined
  }
  const blob = res.data
  const raw = (res.headers || {})['content-disposition'] || (res.headers || {})['Content-Disposition']
  if (!raw) return { blob, filename: fallbackName }
  const utf8 = /filename\*=(?:UTF-8|utf-8)''([^;]+)/i.exec(raw)
  if (utf8?.[1]) {
    try {
      return { blob, filename: decodeURIComponent(utf8[1]) }
    } catch {
      return { blob, filename: utf8[1] }
    }
  }
  const ascii = /filename\s*=\s*"?([^";]+)"?/i.exec(raw)
  if (ascii?.[1]) return { blob, filename: ascii[1].trim() }
