...
import {
  getKnowledgeDomainRetrievalConfig,
  updateKnowledgeDomainRetrievalConfig,
...
// PUT /system/runtime-config/retrieval-config requires SystemAdmin on the server. Hide the
// banner + lock all controls for non-Admins so they can read the
// configuration without tripping a 403 mid-edit.
const canEdit = computed(() => authStore.isSystemAdmin)
...
const loadConfig = async () => {
  try {
    const response = await getKnowledgeDomainRetrievalConfig()
    if (response.data) {
...
const saveConfig = async () => {
  ...
  try {
    const response = await updateKnowledgeDomainRetrievalConfig({ ...localConfig })
    if (response.data) {
      initialConfig = { ...localConfig }
    }
