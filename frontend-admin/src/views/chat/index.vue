    ...
    <div class="chat" :class="{
        ...
        <div ref="scrollContainer" class="chat_scroll_box" @scroll="handleScroll">
            <div class="msg_list">
                ...
                <SuggestedQuestions v-if="messagesList.length === 0 && !loading&&!historyLoading" :enabled="!historyLoading"  @select="handleSuggestedQuestionClick" />
                <!--
                ...
                <div v-for="(session, index) in messagesList"
                    ...
                    <div v-if="session.role == 'assistant' && shouldRenderAssistantMessage(session)">
                        <botmsg :content="session.content" :session="session" :session-id="session_id"
                            ...
                            :isFirstEnter="isFirstEnter" @regenerate="handleRegenerate"></botmsg>
                    </div>
                </div>
        ...
        <div class="input-container">
            <!--
              按钮放在 .input-container 内部（而不是作为 .chat 的直接子元素），
              让输入框成为它的定位上下文。bottom: calc(100% + 85px) 让按钮
              始终位于输入框顶部上方 85px，无论输入框撑高多少行都不会重叠。
              同时 .input-container 在 .chat 内被 align-items: center 居中，
              所以 left: 50% 自动落在可见内容区中心，引用面板打开时不需要
              额外的 left 补偿。
            -->
...
// 按钮放在 .input-container 内部（而不是作为 .chat 的直接子元素），让
// .input-container 成为它的定位上下文。这样：
//   - bottom: calc(100% + 85px) 让按钮始终位于输入框顶部上方 85px，无论
//     输入框撑高多少行（多行文本、附件预览、@mention 标签等）都不会重叠。
//   - .input-container 在 .chat 内被 align-items: center 居中，所以 left: 50%
//     自动落在可见内容区中心；引用面板打开时 .chat 的 padding-right 变化会
//     把输入框连带按钮一起往左移动，按钮始终跟着内容区走，无需额外补偿。
