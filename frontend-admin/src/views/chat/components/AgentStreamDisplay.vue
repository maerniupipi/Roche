            ...
            <div v-else-if="event.type === 'answer' && (event.done || (event.content && event.content.trim()))"
              ...
              <div v-if="event.done && event.content && event.content.trim()" class="answer-toolbar">
                ...
                <t-button size="small" variant="outline" shape="round" class="feedback-btn feedback-btn--like"
                  :class="{ 'is-active': currentFeedback === 'like', 'is-pulsing': pulseRating === 'like' }" :disabled="feedbackSubmitting" :title="currentFeedback === 'like'
                ...
                <t-button size="small" variant="outline" shape="round" class="feedback-btn feedback-btn--dislike"
                  :class="{ 'is-active': currentFeedback === 'dislike', 'is-pulsing': pulseRating === 'dislike' }" :disabled="feedbackSubmitting" :title="currentFeedback === 'dislike'
                ...
                <t-button size="small" variant="outline" shape="round" :loading="exportingAnswer"
                  @click.stop="handleExportAnswerHtml(event)" :title="$t('agent.exportHtml')">
                  <t-icon name="download" />
                </t-button>
                <t-button v-if="isDebugger"  size="small" variant="outline" shape="round" @click.stop="handleAddToKnowledge(event)"
                ...
                <ChatRequestInfoButton v-if="showRequestInfo && isConversationDone && isDebugger" :session="session"
