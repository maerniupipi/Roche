...
// FAQ mutations follow the parent KB's effective management permission.
// creator_id is audit/display metadata and does not grant write access.
const canEdit = computed(() => {
  if (authStore.canManageKnowledge) return true
  return kbInfo.value?.my_permission === 'manage'
})
...
const exampleData: FAQEntryPayload[] = [
  {
    standard_question: '什么是 Roche Knowledge Agent Platform？',
    answers: ['Roche Knowledge Agent Platform 是一个智能知识库管理系统', '它支持多种知识库类型和导入方式'],
    similar_questions: ['Roche Knowledge Agent Platform 是什么？', '介绍一下 Roche Knowledge Agent Platform'],
    negative_questions: ['这不是 Roche Knowledge Agent Platform', '与 Roche Knowledge Agent Platform 无关'],
    tag_name: '产品介绍',
  },
  {
...
.faq-card {
  ...
  &:hover {
    border-color: var(--td-brand-color);
    box-shadow: 0 2px 8px rgba(11, 65, 205, 0.1);
  }

  &.selected {
    border-color: var(--td-brand-color);
    background: var(--td-success-color-light);
    box-shadow: 0 2px 8px rgba(11, 65, 205, 0.15);
  }
}
...
.preview-item {
  ...
  &:hover {
    border-color: var(--td-brand-color);
    box-shadow: 0 2px 4px rgba(11, 65, 205, 0.08);
  }
}
...
.full-width-input-wrapper {
  ...
  :deep(.add-item-btn) {
    ...
    &:hover:not(:disabled) {
      ...
      box-shadow: 0 2px 8px rgba(11, 65, 205, 0.3);
    }
...
.item-row {
  ...
  &:hover {
    background: var(--td-bg-color-secondarycontainer);
    border-color: var(--td-brand-color);
    box-shadow: 0 2px 8px rgba(11, 65, 205, 0.12);
    transform: translateY(-1px);
  }
...
.search-button {
  ...
  &:hover:not(:disabled) {
    transform: translateY(-1px);
    box-shadow: 0 4px 12px rgba(11, 65, 205, 0.3);
  }
