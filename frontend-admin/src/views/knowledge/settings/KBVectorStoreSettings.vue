...
import { listVectorStoreOptions, type VectorStoreOption } from '@/api/vector-store'
...
const allStores = ref<VectorStoreOption[]>([])
...
// KBs (e.g. "milvus"). When the env-store entry is missing or its
...
onMounted(async () => {
  ...
  loading.value = true
  try {
    const resp = await listVectorStoreOptions()
