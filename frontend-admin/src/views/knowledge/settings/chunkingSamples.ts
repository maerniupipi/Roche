...
const MARKDOWN_SAMPLE = `# Roche Knowledge Agent Platform 知识框架

Roche Knowledge Agent Platform 是一个基于 LLM 的企业知识平台，集 RAG 问答、ReAct 智能体和知识图谱于一体。本文介绍其设计动机、架构与典型用法。

## 设计动机

企业内部知识散落在 Confluence、飞书、Notion、Git 仓库等多种系统，传统全文检索难以理解语义，单一 LLM 又缺少可信赖的上下文来源。Roche Knowledge Agent Platform 的目标是：
...
兼容 20+ 主流 LLM：OpenAI、Anthropic、DeepSeek、Qwen、智谱、混元和 Gemini。模型层抽象 \`Provider\` 接口，新增厂商只需实现 \`Chat()\` 方法并注册即可。

### 向量检索

平台使用 Milvus 保存 Chunk 向量并执行语义检索，PostgreSQL 专门保存业务数据、权限和 Chunk 正文。
...
make dev-start             # 启动 postgres / redis / milvus
...
Roche Knowledge Agent Platform 后端采用清晰的分层架构：
...
const FAQ_SAMPLE = `# Roche Knowledge Agent Platform 部署与使用 FAQ
...
\`\`\`
docker pull roche/knowledge-agent-platform-app:v0.5.0
docker pull roche/knowledge-agent-platform-docreader:v0.5.0
docker pull roche/knowledge-agent-platform-ui:v0.5.0
\`\`\`
...
### Q6: 当前使用什么向量库？
当前产品部署使用 \`RETRIEVE_DRIVER=milvus\`。变更向量引擎需要同步调整部署和代码，并重新嵌入存量数据。
...
### Q13: 调用 OpenAI 超时
检查代理网络与 \`OPENAI_BASE_URL\` 配置；网络受限时可切换到可访问的模型服务。

### Q14: token 用量在哪里看？
开启 Langfuse 后，每个会话、每次推理、每个工具调用的 token 消耗都有完整 trace，可按时间、知识域、模型聚合。
...
### Q17: 嵌入吞吐瓶颈
使用模型服务的批量端点（如 OpenAI 的 batch API），并根据服务能力调整 \`EMBED_BATCH_SIZE\`，可显著提升吞吐。
...
用户、部门、角色与权限规则集中在 user-service 中。RBAC 模型支持继承与覆盖，企业 SAML 接入通过独立认证服务完成。
...
Agent 编排器负责 ReAct 循环，工具调用通过内置注册表分发。所有 LLM 调用统一经过模型代理层，方便切换、限流与计费。
