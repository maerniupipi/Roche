...
/** KnowledgeDomain-wide default from Settings → Storage engine (used when creating a KB). */
const knowledgeDomainDefaultStorageProvider = ref('local')
...
const handleStorageProviderUpdate = (value: string) => {
  if (formData.value) {
    formData.value.storageProvider = value || knowledgeDomainDefaultStorageProvider.value || 'local'
  }
}

async function loadKnowledgeDomainDefaultStorageProvider(force = false) {
  try {
    await editorResources.ensureStorageEngine(force)
    knowledgeDomainDefaultStorageProvider.value = editorResources.storageConfig?.default_provider || 'local'
  } catch {
    knowledgeDomainDefaultStorageProvider.value = 'local'
  }
}

/** Resolved storage provider for create payload (never silently default to local before knowledgeDomain config loads). */
function resolvedStorageProvider(): string {
  const explicit = formData.value?.storageProvider?.trim()
  if (explicit) return explicit
  return knowledgeDomainDefaultStorageProvider.value || 'local'
}
...
const validateForm = (): boolean => {
  ...
  if (props.mode === 'create' && !props.knowledgeDomainId) {
  ...
  if (formData.value.type !== 'faq') {
    const s = formData.value.indexingStrategy
    if (s && !s.vectorEnabled && !s.keywordEnabled && !s.graphEnabled) {
...
const buildSubmitData = () => {
  ...
  const data: any = {
    ...(props.mode === 'create' ? { knowledge_domain_id: props.knowledgeDomainId } : {}),
...
const doSubmit = async () => {
  ...
  try {
    ...
    // is invalid" (e.g. unknown id, foreign knowledgeDomain), 2201 is "the
...
watch(() => props.visible, async (newVal) => {
  if (newVal) {
    ...
    // 加载模型列表与平台默认存储引擎（创建 KB 时即使用，不依赖是否打开「存储引擎」Tab）
    await Promise.all([loadAllModels(), loadKnowledgeDomainDefaultStorageProvider()])
    
    // 根据模式加载数据
    if (props.mode === 'edit' && props.kbId) {
      await loadKBData()
    } else {
      // 创建模式：初始化空表单，并预填平台默认存储引擎
      formData.value = initFormData(props.initialType || 'document')
      formData.value.storageProvider = knowledgeDomainDefaultStorageProvider.value
      hasFiles.value = false
