        ...
        <div class="form-card" v-else>
          ...
          <div class="form-content">
            <t-form ref="registerFormRef" :data="registerData" :rules="registerRules" @submit="handleRegister"
              ...
              <t-form-item v-if="registrationRoleSelectionEnabled" :label="$t('auth.registrationRoleLabel')"
                name="role">
                <t-select v-model="registerData.role" :options="registrationRoleOptions" size="large" />
...
const persistLoginResponse = async (response: any) => {
  ...
  await nextTick()
  router.replace(authStore.canManageKnowledge ? '/platform/dashboard' : '/platform/creatChat')
}
...
// Check if already logged in; mount-time SSO / SAML config probing
onMounted(async () => {
