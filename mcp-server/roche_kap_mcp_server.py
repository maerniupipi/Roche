...
ROCHE_KAP_BASE_URL = os.getenv("ROCHE_KAP_BASE_URL", "http://localhost:8080/api/v1")
ROCHE_KAP_API_KEY = os.getenv("ROCHE_KAP_API_KEY", "")
# Chat SSE read timeout in seconds. LLM responses can be slow; default 300s.
try:
    ROCHE_KAP_CHAT_TIMEOUT = int(os.getenv("ROCHE_KAP_CHAT_TIMEOUT", "300"))
except ValueError:
    logger.warning("ROCHE_KAP_CHAT_TIMEOUT is not a valid integer; falling back to 300s.")
    ROCHE_KAP_CHAT_TIMEOUT = 300
...
class RocheKAPClient:
    """Client for interacting with RocheKAP API"""

    def __init__(self, base_url: str, api_key: str):
        """Initialize the RocheKAP API client with base URL and authentication"""
        self.base_url = base_url
        self.api_key = api_key
        # SSL verification: enabled by default. Set ROCHE_KAP_VERIFY_SSL=false to disable
        # (e.g. for self-signed certs in dev environments — NOT recommended for production).
        self.verify_ssl = os.getenv("ROCHE_KAP_VERIFY_SSL", "true").lower() != "false"
        if not self.verify_ssl:
            logger.warning(
                "SSL certificate verification is DISABLED (ROCHE_KAP_VERIFY_SSL=false). "
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        """Make a request to the RocheKAP API
    ...
    def _consume_sse_stream(self, url: str, body: Dict[str, Any]) -> Dict:
        ...
        Timeout: (10s connect, ROCHE_KAP_CHAT_TIMEOUT read) — configurable via env var.
        ...
        try:
            ...
            # Timeout: 10s to establish connection, ROCHE_KAP_CHAT_TIMEOUT for reading response
            response = self.session.post(
                url, json=body, stream=True,
                timeout=(10, ROCHE_KAP_CHAT_TIMEOUT),
            )
            response.raise_for_status()
    ...
    def agent_chat(
        ...
        *agent_id* is required — the backend uses the CustomAgent config for
        ...
        Knowledge access is derived exclusively from the authenticated user's
        effective grants; the agent has no independent knowledge authorization.
        """
    ...
    def list_agents(self, page: int = 1, page_size: int = 50) -> Dict:
        """List all custom agents available on the platform."""
...
app = Server("roche-kap-server")
# Initialize RocheKAP API client with configuration
client = RocheKAPClient(ROCHE_KAP_BASE_URL, ROCHE_KAP_API_KEY)


# Tool definitions - Register all available tools for the MCP protocol
@app.list_tools()
async def handle_list_tools() -> list[types.Tool]:
    """List all available RocheKAP tools with their schemas"""
    return [
        ...
        types.Tool(
            ...
            description=(
                ...
                "Knowledge access is always derived from the authenticated user's effective grants; "
                "agents do not have independent knowledge-base permissions."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    ...
                    "agent_id": {
                        "type": "string",
                        "description": "REQUIRED. Custom agent UUID or name. Use list_agents to discover agents.",
                    },
        ...
        types.Tool(
            name="list_agents",
            description="List all custom agents available on the platform. Use this to discover agent IDs and names before calling agent_chat.",
            inputSchema={
                "type": "object",
                "properties": {
        ...
        types.Tool(
            ...
            description=(
                ...
                "The response contains model, prompt, and tool configuration; "
                "knowledge authorization comes from the authenticated user."
            ),
            inputSchema={
                "type": "object",
                "properties": {
...
def main():
    ...
    Transport selection (in priority order):
      ...
      3. Default: stdio
    """
    parser = argparse.ArgumentParser(description="RocheKAP MCP Server")
    parser.add_argument(
        "--transport",
