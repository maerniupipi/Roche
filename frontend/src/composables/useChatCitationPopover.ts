...
type FloatState = {
  ...
  knowledgeId: string
  fileName: string
  // Reserved for backend-driven thumbnail rendering; remains empty until the
  // citation enrichment pipeline starts populating `image_info`.
  imageInfo: { thumbnailUrl: string } | null
}
...
export function useChatCitationPopover(
  ...
  const openKb = async (el: HTMLElement) => {
    ...
    float.value.fileName =
      matchedRef?.knowledge_filename || matchedRef?.knowledge_title || title
    float.value.visible = true
    positionFor(el, 4)
