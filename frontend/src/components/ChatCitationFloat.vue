  ...
  <Teleport to="body">
    <div v-if="float.visible" class="chat-citation-float" :style="{ top: `${float.top}px`, left: `${float.left}px` }"
      ...
      <template v-else>
        ...
        <!-- Reserved thumbnail slot: only renders when the backend supplies
             image_info.thumbnail_url. The slot is intentionally gated so the
             float stays text-only until the upstream contract lands. -->
        <img v-if="float.imageInfo?.thumbnailUrl" class="chat-citation-float__thumbnail"
          :src="float.imageInfo.thumbnailUrl" :alt="float.title" loading="lazy" />
