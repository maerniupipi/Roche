...
const props = withDefaults(
  defineProps<{
    ...
    //           late-arriving post-processing subspans
    ...
    //           /spans the instant parsing finishes — async summary
    //           work doesn't keep a headless poller alive
...
// Async post-pipeline tasks (summary, question, graph.chunk[*]) open
// their postprocess.* subspans AFTER the parse pipeline has finalised.
// At that exact moment parse_status is already
...
// automatically. Sized generously for a typical post-processing run,
// but short enough that opening an
...
//     the tree quiesces so late work surfaces without a manual refresh.
//
//   gracePoll = false (for background mounts e.g. doc-content's
//     hidden badge driver):
//     Strict mode. Poll ONLY while the parse pipeline itself is
//     non-terminal. Post-pipeline async work (summary / question /
//     graph subspans) is intentionally ignored —
...
async function fetchSpans(opts: { manual?: boolean } = {}) {
  ...
  try {
    ...
    if (res?.success && res.data) {
      ...
      // nested post-processing subspans whose children appear later
      // in the run surface automatically. Without this the user could
      // click a post-processing chevron and see
...
const flatRows = computed<FlatRow[]>(() => {
  ...
  for (const stage of root.children || []) {
    ...
    const walk = (n: SpanNode, depth: number, idxPath: string, parentKey: string) => {
      ...
      // this gate, clicking the chevron on a nested subspan only
      // rotated the icon but the children
...
function toggleTree(row: FlatRow, ev?: MouseEvent) {
  ...
  // who collapsed a noisy subspan with many children would see it
  // pop back open on the
  // next 2s tick.
...
// post-pipeline window (summary / question / graph), the latest
// attempt's ROOT span closes — so trace.status reads 'done' — while
// those subspans keep running and the row is still 'finalizing'.
// Trusting trace.status there flashes "已完成" mid-processing even though the
...
/* Status palette — NOT all green. The project brand color happens
   ...
   running goes amber + striped (the familiar CI "in progress" style).
   The two are now
...
.kp-bar-wrap-done {
  border-color: rgba(11, 65, 205, 0.35);
}
