<template>
  <div :class="[
    ...
    <span class="submenu_title" :class="batchMode ? 'submenu_title--batch' : ''" :title="item.title">
      <img v-if="item.is_pinned"
        :src="activePath === item.path ? pinActiveIcon : pinIcon"
        class="submenu_pin_icon" alt="" />
      <span class="submenu_title-text">{{ item.title }}</span>
    </span>
    <div v-if="!batchMode" class="session-row-menu-wrap" @click.stop>
      <button ref="triggerRef" type="button" class="menu-more-wrap" aria-haspopup="menu" :aria-expanded="menuOpen"
        @click="toggleMenu">
        <t-icon name="ellipsis" class="menu-more" />
      </button>
      <Teleport to="body">
        <div v-if="menuOpen" class="session-row-menu" role="menu" :style="menuStyle" @click.stop>
          <button v-for="option in menuOptions" :key="option.value" type="button" class="session-row-menu__item"
            :class="{ 'session-row-menu__item--error': option.theme === 'error' }" role="menuitem"
            @click="handleMenuClick(option)">
            <component :is="option.prefixIcon" v-if="option.prefixIcon" class="session-row-menu__icon" />
...
import pinActiveIcon from '@/assets/img/pin-active.svg'
