...
interface Settings {
  ...
  selectedFileKbMap: Record<string, string>; // 文件ID -> 知识库ID，用于刷新后带 kb_id 拉取文件
...
interface ModelItem {
  ...
  interfaceType?: 'openai';  // VLLM专用：接口类型
...
export const useSettingsStore = defineStore("settings", {
  ...
  actions: {
    ...
    saveSettings(settings: Settings) {
      ...
      // 保存到localStorage
      localStorage.setItem("RocheKAP_settings", JSON.stringify(this.settings));
    },
    ...
    // 让调用方（例如 t-switch 组件）可以提示并把开关复位。
    ...
    // 调用方：authStore.setUser（每次登录或刷新 user 后都会触发）。
    ...
    selectAgent(agentId: string) {
      ...
      // 智能体没有独立知识范围；切换智能体不得修改普通 RAG 的知识选择。
    ...
    // —— 会话级输入态恢复 —— //
    //
    // 输入栏的 agent / 模型 / KB / 联网 等选择由本 store 持有，跨会话共享。
