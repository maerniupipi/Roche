...
// 保密确认状态查询
export interface ConfidentialityAckStatus {
  acknowledged: boolean
  acknowledged_at: string | null
}

export async function getConfidentialityAck(): Promise<ConfidentialityAckStatus> {
  return unwrapAuthResponse<ConfidentialityAckStatus>(
    await get('/api/v1/auth/me/confidentiality-ack'),
  )
}

// 标记保密确认（POST 无 body，返回更新后的状态）
export async function acknowledgeConfidentiality(): Promise<ConfidentialityAckStatus> {
  return unwrapAuthResponse<ConfidentialityAckStatus>(
    await post('/api/v1/auth/me/confidentiality-ack', {}),
  )
}
