...
export type CitationKnowledgeRef = {
  ...
  // Optional thumbnail metadata for the citation hover float. Backend field
  // is not yet finalized; the UI renders only when both values are present.
  image_info?: {
...
export function preprocessCitationTags(
  ...
  return contentStr
    ...
    .replace(KB_TAG_ATTR_RE, (_m, attrString: string) => {
      ...
      return `<span class="citation citation-kb" data-kb-id="${safeKbId}" data-chunk-id="${safeChunkId}" data-doc="${safeDoc}" role="button" tabindex="0"><span class="citation-text">${rawChunkId}</span><span class="citation-tip"><span class="tip-loading">…</span></span></span>`
    })
}
