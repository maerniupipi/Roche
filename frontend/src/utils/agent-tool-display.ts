...
function getRetrievalStatusKeys(failed: boolean) {
  return {
...
export function getKnowledgeSearchSummaryHtml(
  ...
  const count = Number(toolData.count) || (Array.isArray(toolData.results) ? toolData.results.length : 0) || 0
...
export function getRagPipelineStepTitle(t: ComposerTranslation, event: RagPipelineEvent): string {
  ...
  if (toolName === 'knowledge_search' || toolName === 'search_knowledge') {
    const labels = getRetrievalStatusKeys(event.success === false)
    if (pending) {
      return query
