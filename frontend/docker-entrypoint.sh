...
envsubst '${MAX_FILE_SIZE} ${APP_HOST} ${APP_PORT} ${APP_SCHEME} ${FRONTEND_DEFAULT_ENABLED} ${FRONTEND_ADMIN_ENABLED} ${FRONTEND_APP_ENABLED}' < /etc/nginx/templates/default.conf.template > /etc/nginx/conf.d/default.conf
