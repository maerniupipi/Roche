...
DocReader 是 RocheKAP 项目中负责文档解析和处理的 gRPC 服务。它支持多种文档格式的读取、OCR 识别、多模态处理等功能。

## Docker Compose 环境变量配置

DocReader 在本地由 `docker-compose.local.yml`、在服务器开发环境由
`docker-compose.server-dev.yml` 配置；正式部署通过
`docker-compose.production.yml` 覆盖为固定版本镜像。

```yaml
docreader:
  image: roche/knowledge-agent-platform-docreader:local
  environment:
...
- **重要提示**: 
  ...
  - 可以在 `.env.local`、`.env.server-dev` 或 `.env.production` 中配置 `MINIO_PORT`
- **配置示例**:
  ```bash
  # 对应场景的环境文件
  MINIO_PORT=9000
  ```
  或直接在对应的 Compose 文件中修改：
  ```yaml
...
除了 Compose 已配置的变量外，DocReader 还支持以下环境变量（可按场景加入
`.env.local`、`.env.server-dev` 或 `.env.production`）：
...
- `MINIO_BUCKET_NAME`: MinIO 存储桶名称（默认：RocheKAP）
...
```yaml
healthcheck:
  test: ["CMD", "python", "-c", "import grpc; from grpc_health.v1 import health_pb2, health_pb2_grpc; health_pb2_grpc.HealthStub(grpc.insecure_channel('127.0.0.1:50051')).Check(health_pb2.HealthCheckRequest(), timeout=3)"]
  interval: 30s
  timeout: 10s
  retries: 3
  start_period: 60s
...
docker logs roche-kap-docreader
```

## 更多信息

- 服务端口：50051（gRPC）
- 容器名称：roche-kap-docreader
- 网络：roche-kap-network
- 重启策略：unless-stopped
